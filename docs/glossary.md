...
| 术语 | 当前定义 | 状态 |
| --- | --- | --- |
| 桌面客户端 | Super 的图形化第一方访问面；通过 Automation Command Gateway 使用领域能力，并承载 Console 与保存脚本的交互。 | 已确认 |
...
| 自动化脚本 | 使用受控 `super` API 组合领域命令的 JS/TS 程序或交互式代码；不等同于 Node.js 脚本，默认不能访问任意模块、进程环境、文件系统、网络或数据库。 | 已确认 |
...
| 资产 | 通过导入过滤规则、被 Super 纳入管理的文件。首版不把依赖文件组合成复合资产。 | 已确认 |
...
| 磁盘目录 | 操作系统文件系统中的真实目录；Super 可在用户确认后移动、重命名或删除其中的文件。 | 已确认 |
| 复制导入 | 将原文件复制进由 Super 管理的资源库存储。 | 已确认 |
| 链接文件夹 | 用户通过“以链接方式导入”挂载到 Super 的真实磁盘目录；内容不复制，Super 中的文件操作会作用于源目录。 | 已确认 |
| 链接过滤规则 | 通过图形界面配置的路径过滤；命中的文件或目录不被 Super 索引、展示或操作。 | 已确认 |
...
| 非受限插件（unrestricted） | 由用户明确选择、运行在独立 Node.js 进程并具有完整系统能力的插件；权限清单用于风险披露和 Super API 管理，不承诺拦截全部系统行为。 | 已确认 |
| 用户级插件 | Package 安装在当前 Super 用户数据中的插件；代码不随资源库同步，可选择在哪些资源库启用。 | 已确认 |
| 资源库级插件 | Package 和非秘密资源库配置保存在 `.super/` 并随资源库同步的插件；每台设备首次执行前都需要本机信任。 | 已确认 |
...
| 输入捕获会话 | 插件在限定视图、查看页或 Super 应用内暂时独占键盘/指针事件的有界会话；不同于普通快捷键和操作系统全局 Hook。 | 已确认 |
