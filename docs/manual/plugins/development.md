...
本文面向准备实际开发、联调和发布 Super 插件的人类开发者。文档以当前仓库的
...
领域读写仍通过 `super` API 进入 Automation Gateway；Renderer 不加载插件后端代码，插件也不能获得任意
SQL 或宿主 DOM。
...
```text
my-plugin/
  super-plugin.json       # 必需，Manifest v1
...
```json
{
  ...
  "version": "1.0.0",
  "name": "Hello",
  "description": "A small Super plugin.",
  "author": "Example",
  "license": "MIT",
  "engines": {
    "super": ">=0.1.0 <1.0.0",
...
| `engines` | 是 | `super` 是显式比较符 SemVer range；`pluginApi` 当前只能为 `1` |
| `runtime` | 是 | 见下一节 |
...
原生文件系统、数据库或宿主 DOM。插件只能通过声明权限后可用的 `super.*` API 工作。适合一般菜单、领域操作、
Provider 和 Job。

### unrestricted

`unrestricted` 运行在每个实例独立的 Node.js UtilityProcess 中，拥有 Node、文件系统、网络、子进程和依赖的完整能力。
它仍应优先通过 `super.*` 操作 Super 领域对象，但权限清单不能拦截插件绕过 Gateway 直接调用 Node 的行为；权限同时
是 Super API 的能力控制和对用户的风险披露，不是沙箱承诺。需要原生模块时可声明：
...
`--super-plugin-ref-accent`，`tokens.badge` 变成 `--super-plugin-token-badge`。插件不得依赖 Host 内部旧变量（如
...
Host 当前提供的外观预设包括 `vscode-dark`、`super-dark`、`super-light` 和 `soft-light`。插件不需要识别预设 ID，
...
exports.setup = async function setup(context) {
  ...
  await context.super.storage.set('initialized', true);
};
...
`context` 包含 `pluginId`、`pluginInstanceId`、`installationScope`、`instanceScope`、`super`、`subscriptions` 和 `signal`。
...
`super.storage` 是插件命名空间 KV，小配置使用 `scope: "user" | "library"`；按权限 `storage.read`/`storage.write` 控制。
`super.data.getDirectory({ scope })` 返回插件数据根：用户级为 `{userData}/plugin-files/<pluginId>/`，库级为
`<库根>/.super/plugin-files/<pluginId>/`。库未打开时不能取得库级目录。受限模式没有 Node fs；unrestricted 才能在该目录下用 fs，
...
```js
const library = context.super.forLibrary(libraryId);
...
commands 或 handler 注册；handler 必须在实例级 `super.jobs` 注册。library 实例不能借此越过自己的库边界。
...
Hook 在 Manifest 中声明 `id`、`event`、`blocking`。当前实现首发事件是 `asset.trash`；运行时 `super.hooks.onWill` 返回：
...
超时默认 fail-open。事件订阅使用 `super.events.on('library.changed' | 'asset.changed' | '*', handler)`；事件至少一次投递，
请按 `eventId` 去重。
...
- Electron E2E 必须后台运行并使用临时 `SUPER_E2E_USER_DATA_PATH`；持久化必须以完整退出并重新启动为边界。
- 发布前重新打包当前 HEAD；不能用旧包证明当前包可运行。macOS/Windows 和 Computer Use 未执行时必须明确标记“未验证”。

### 发布清单

1. `super-plugin.json` 通过当前 schema，权限最小化，所有 Contribution ID 唯一。
...
6. 发布版本保持稳定 `id`，并记录支持的 Super SemVer 与 Plugin API 版本。
