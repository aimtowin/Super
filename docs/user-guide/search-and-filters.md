...
点击顶部「颜色」可打开调色盘；点击色块选择条件，按住 Shift 可多选，「排除」用于排除对应颜色。

![颜色筛选调色盘](../assets/ui/live-2.2.8/color-filter.png)
...
筛选入口位于资源区上方，分别为「颜色」「标签」「形状」「评分」「AI 分析」「仅喜欢」「格式」「更多」；多个维度同时启用时按 AND 组合。同一维度的多个值按 OR 组合。当前维度包括：

- 颜色：18 个色相区间与黑、灰、白，共 21 个色块；匹配素材色卡中的代表色，不只匹配单一主色
- 标签：可搜索标签、常用标签和最近筛选
