...
1. 先打开一个资源库。
2. 打开「更多工具」→「自动化脚本」。
3. 在编辑器中输入或粘贴脚本，或点击「打开脚本」选择已有文件。先阅读内容，再点击「运行」。
4. 运行后回到资源浏览界面检查结果；不确定时先用少量资产试运行。

脚本只通过 Super 提供的操作来修改资源库，不会替你执行电脑上的任意程序。运行前仍应确认脚本来自可信来源，并备份重要资源库。

![当前自动化脚本面板；编辑器为空，未运行脚本](../assets/ui/live-2.2.8/automation-scripts.png)

## 连接外部 AI 工具（MCP）

MCP 是一种让外部 AI 工具读取和整理 Super 资产的连接方式。它只在本机工作，不会自动把资源库公开到互联网。
...
5. 将剪贴板中的**全部文本**直接粘贴给 Agent，不要手动拆分地址或授权信息；其中包含 Agent 连接 Super 所需的内容和使用提示。
...
![当前 MCP 设置；服务原已开启，未新增客户端](../assets/ui/live-2.2.8/mcp-settings.png)
