...
async function setup(superApi) {
  superApi.providers.register('preview', {
    id: 'probe-preview',
    async compute(batch) {
      return mediaResult(batch);
    },
  });
  superApi.providers.register('thumbnail', {
    id: 'probe-thumbnail',
    async compute(batch) {
      return mediaResult(batch);
    },
  });
  superApi.providers.register('metadata', {
    id: 'probe-metadata',
    async compute(batch) {
      return metadataResult(batch);
    },
  });
  superApi.providers.register('import', {
    id: 'probe-import',
    async compute(batch) {
      return importPlanResult(batch);
    },
  });
  superApi.providers.register('export', {
    id: 'probe-export',
    async compute(batch) {
      return exportDescriptorResult(batch);
    },
  });
  superApi.providers.register('ai', {
    id: 'probe-ai',
    async compute(batch) {
