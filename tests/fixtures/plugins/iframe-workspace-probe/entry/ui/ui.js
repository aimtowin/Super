...
const contributionId = new URLSearchParams(location.search).get('contributionId')
  ?? 'com.super.iframe-workspace-probe.workspace-probe';
...
window.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'plugin-ui.theme-changed') {
    for (const [name, value] of Object.entries(event.data.tokens || {})) {
      document.documentElement.style.setProperty(`--superApi-${name.slice(2)}`, value);
    }
    const accent = event.data.tokens?.['--superApi-plugin-ref-accent'] ?? 'unset';
