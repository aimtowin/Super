...
describe('Automation Command Registry', () => {
  it('contains complete read/write descriptors and exports JSON/TypeScript contracts', () => {
    ...
    // Super-10lo: folder/collection parent ids are strict UUIDs — non-UUID
    ...
    for (const command of automationCommandRegistry) {
      ...
      if (command.commandId === 'asset.rating.set'
        ...
        // Super-8b5b.2: the dangerous command is a write with no plan
      ...
      expect(command.mcp.toolName).toMatch(/^super_/u);
    ...
    const declaration = generateAutomationTypeDeclaration('@super/test-api');
    expect(declaration).toContain('const super: SuperAutomationApi');
    expect(declaration).toContain('interface SuperScriptAssetSearchPage');
    ...
    expect(declaration).toContain('create(name: string): Promise<SuperScriptTag>');
    ...
    expect(declaration).toContain('getAiContent(assetId: string): Promise<SuperAiContent>');
  ...
  it('keeps capability risk metadata complete and separates critical operations', () => {
    ...
    for (const command of automationCommandRegistry) {
      ...
      // Super-8b5b.2: the single MCP-exposed dangerous command is critical.
...
describe('Automation Command Gateway', () => {
  ...
  it('fires the import-completed hook for a successful automation import (Super-ihpx)', async () => {
    const plan = {
      planHash: 'a'.repeat(64),
...
describe('file.import VERSION_CONFLICT auto re-plan (Super-xdt8)', () => {
...
describe('file.import VERSION_CONFLICT retry negative paths (Super-xdt8)', () => {
  const plan = {
    planHash: 'a'.repeat(64),
    expectedChangeSequence: 0,
    assetStates: [],
