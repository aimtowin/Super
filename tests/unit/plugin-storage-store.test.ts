...
describe('PluginStorageStore', () => {
  it('stores library-scoped values under .super and keeps user scope in userData', async () => {
    const userData = temporaryRoot('super-plugin-storage-user-');
    const library = temporaryRoot('super-plugin-storage-library-');
  ...
  it('rejects storage writes without storage.write', async () => {
    const userData = temporaryRoot('super-plugin-storage-denied-');
    const library = temporaryRoot('super-plugin-storage-denied-library-');
    const store = new PluginStorageStore(userData);
