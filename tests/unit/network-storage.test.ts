...
describe('network storage detection', () => {
  it('recognizes a macOS SMB mount and prefers the longest matching mount point', () => {
    ...
    expect(classifyLibraryStorage('/Volumes/Working/Project/.super/library.db', {
      platform: 'darwin',
      mountOutput,
    })).toBe('network');
    ...
    expect(classifyLibraryStorage('/Users/test/library/.super/library.db', {
      platform: 'darwin',
      mountOutput,
    })).toBe('local');
  });
  ...
  it('unescapes mount points containing spaces', () => {
    expect(classifyLibraryStorage('/Volumes/NAS Work/Library/.super/library.db', {
      platform: 'darwin',
  ...
  it('recognizes Linux mount entries with an explicit type token', () => {
    ...
    expect(classifyLibraryStorage('/mnt/assets/Library/.super/library.db', {
      platform: 'linux',
      mountOutput,
    })).toBe('network');
    expect(classifyLibraryStorage('/var/lib/super/library/.super/library.db', {
      platform: 'linux',
      mountOutput,
    })).toBe('local');
  });
  ...
  it('recognizes Windows UNC and extended UNC paths', () => {
    ...
    expect(classifyLibraryStorage('\\\\server\\share\\Library\\.super\\library.db', {
      platform: 'win32',
    })).toBe('network');
  });

  it('recognizes mapped Windows network drives through GetDriveTypeW', () => {
    expect(classifyLibraryStorage('Z:\\Library\\.super\\library.db', {
      platform: 'win32',
      getDriveType: (rootPath) => rootPath === 'Z:\\' ? 4 : 3,
    })).toBe('network');
    expect(classifyLibraryStorage('C:\\Library\\.super\\library.db', {
      platform: 'win32',
