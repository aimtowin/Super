...
import {
  SUPER_GUEST_ASSET_METHODS,
  SUPER_GUEST_COLLECTION_METHODS,
  SUPER_GUEST_FILE_METHODS,
  SUPER_GUEST_FOLDER_METHODS,
  SUPER_GUEST_LIBRARY_METHODS,
  SUPER_GUEST_LINKED_FOLDER_METHODS,
  SUPER_GUEST_PALETTE_METHODS,
  SUPER_GUEST_SMART_COLLECTION_METHODS,
  SUPER_GUEST_TAG_METHODS,
  SUPER_GUEST_TRASH_METHODS,
} from '../../src/scripting/super-guest-api';
...
describe('Plugin Trusted Host handler', () => {
  it('exposes the shared Guest API Gateway method sets', async () => {
    const packageDirectory = mkdtempSync(path.join(tmpdir(), 'super-trusted-guest-api-'));
    ...
    writeFileSync(path.join(packageDirectory, 'dist', 'main.js'), `
        ...
        pluginContext = superApi;
        superApi.subscriptions.add(() => superApi.console.log('subscription-disposed'));
        superApi.console.log('signal-aborted:' + superApi.signal.aborted);
        superApi.console.log(JSON.stringify({
          assets: Object.keys(superApi.assets).sort(),
          library: Object.keys(superApi.library).sort(),
          folders: Object.keys(superApi.folders).sort(),
          tags: Object.keys(superApi.tags).sort(),
          collections: Object.keys(superApi.collections).sort(),
          smartCollections: Object.keys(superApi.smartCollections).sort(),
          linkedFolders: Object.keys(superApi.linkedFolders).sort(),
          files: Object.keys(superApi.files).sort(),
          trash: Object.keys(superApi.trash).sort(),
          palettes: Object.keys(superApi.palettes).sort()
        }));
      };
    ...
    expect(JSON.parse(consoleMessage.message)).toEqual({
      assets: [...SUPER_GUEST_ASSET_METHODS].sort(),
      library: [...SUPER_GUEST_LIBRARY_METHODS].sort(),
      folders: [...SUPER_GUEST_FOLDER_METHODS].sort(),
      tags: [...SUPER_GUEST_TAG_METHODS].sort(),
      collections: [...SUPER_GUEST_COLLECTION_METHODS].sort(),
      smartCollections: [...SUPER_GUEST_SMART_COLLECTION_METHODS].sort(),
      linkedFolders: [...SUPER_GUEST_LINKED_FOLDER_METHODS].sort(),
      files: [...SUPER_GUEST_FILE_METHODS].sort(),
      trash: [...SUPER_GUEST_TRASH_METHODS].sort(),
      palettes: [...SUPER_GUEST_PALETTE_METHODS].sort(),
    });
  ...
  it('binds trusted jobs to the same explicit forLibrary target as domain calls', async () => {
    const packageDirectory = mkdtempSync(path.join(tmpdir(), 'super-trusted-targeted-jobs-'));
    roots.push(packageDirectory);
    mkdirSync(path.join(packageDirectory, 'dist'), { recursive: true });
    writeFileSync(path.join(packageDirectory, 'dist', 'main.js'), `
      exports.setup = async function setup(superApi) {
  ...
  it('loads a CommonJS entry from a package directory and parks until deactivate', async () => {
    const packageDirectory = mkdtempSync(path.join(tmpdir(), 'super-trusted-plugin-'));
    roots.push(packageDirectory);
    mkdirSync(path.join(packageDirectory, 'dist'), { recursive: true });
    writeFileSync(path.join(packageDirectory, 'dist', 'main.js'), `
      let bridge;
  ...
  it('rejects a pending Host call during deactivation before disposing with its reason', async () => {
    const packageDirectory = mkdtempSync(path.join(tmpdir(), 'super-trusted-pending-plugin-'));
    roots.push(packageDirectory);
    mkdirSync(path.join(packageDirectory, 'dist'), { recursive: true });
    writeFileSync(path.join(packageDirectory, 'dist', 'main.js'), `
      let bridge;
      exports.setup = async function setup(superApi) {
        bridge = superApi;
        void superApi.assets.search({ query: null, limit: 1 }).catch((error) => {
  ...
  it('exposes bounded content read and replacement through the Gateway bridge', async () => {
    const packageDirectory = mkdtempSync(path.join(tmpdir(), 'super-trusted-content-plugin-'));
    roots.push(packageDirectory);
    mkdirSync(path.join(packageDirectory, 'dist'), { recursive: true });
    writeFileSync(path.join(packageDirectory, 'dist', 'main.js'), `
        exports.setup = async function setup(superApi) {
        await superApi.assets.readContent('asset-1', { maxBytes: 4 });
        await superApi.assets.replaceContent('asset-1', 'AQID', { expectedRevisionId: 'revision-1' });
      };
        exports.dispose = async function dispose() {};
    `);
