...
describe("plugin job display", () => {
  it("turns a reverse-domain plugin id into a readable fallback name", () => {
    expect(formatPluginJobPluginName("com.dolag.super.image-upscaler")).toBe("Image Upscaler");
  });
  ...
  it("redacts paths from the live progress message", () => {
    expect(
      formatPluginJobProgressMessage({
        ...
        status: "running",
        phase: "spawn",
        message: "/Users/testuser/Library/Application Support/Super/plugin.js",
      }),
    ).toBe("spawn · <path>");
  });
  ...
  it("redacts host paths and bounds plugin failure text", () => {
    expect(
      formatPluginJobError(
        "Failed to read /Users/testuser/Library/Application Support/Super/cache/image.png",
        "READ_FAILED",
      ),
