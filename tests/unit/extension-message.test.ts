...
const validUploadHeaders = {
  'X-Super-Kind': 'image',
  'X-Super-Source-Page-Url': 'https://example.com/gallery',
  'X-Super-Media-Url': 'https://example.com/photo.png',
  'X-Super-Content-Type': 'image/png',
  'X-Super-Filename': 'photo.png',
};
...
describe('createExtensionServer', () => {
  ...
  it('starts on the requested port and responds to GET /ping', async () => {
    ...
    expect(ping.body).toEqual({ app: 'Super' });
  });
  ...
  it('accepts a valid Firefox extension Origin (Super-54122a)', async () => {
    server = await startServer({ port: TEST_PORT });
    const res = await post(
      server.port,
      {
  ...
  describe('POST /save-upload', () => {
    it('returns 202 for a valid upload and calls onSaveUpload', async () => {
      ...
      const stagingRoot = path.join(os.tmpdir(), `super-ext-test-${randomUUID()}`);
    ...
    it('accepts a local-file upload without URL metadata', async () => {
      ...
      try {
        ...
        const res = await postUpload(server.port, Buffer.from('local-png'), {
          'X-Super-Kind': 'image',
          'X-Super-Local-File': 'true',
          'X-Super-Content-Type': 'image/png',
          'X-Super-Filename': 'shot.png',
        });
        expect(res.status).toBe(202);
        expect(uploads[0]).toMatchObject({
          kind: 'image',
