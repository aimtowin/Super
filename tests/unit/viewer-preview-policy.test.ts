...
const nativeImage = {
  ...
  mediaType: 'image',
  playbackMode: 'source',
  url: 'super://source/lib/asset?revision=r1',
  kind: 'thumbnail',
};
...
describe('viewer preview policy (REQ-VIEW-002)', () => {
  ...
  it('opens model assets into the media surface, never unsupported (slice A)', () => {
    // Model resolution is a ready source URL (super://source with revision);
  ...
  it('presents mip placeholder while full preview is still loading (Super-eh07)', () => {
    expect(
      resolveViewerPrimarySurface({
        loading: true,
        resolution: null,
