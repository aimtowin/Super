...
describe('createToastNotifications', () => {
  ...
  it('keeps error visible when a later info notice arrives (Super-99lv)', () => {
    const toast = createToastNotifications();
  ...
  it('keeps warning visible when a later info notice arrives (Super-99lv)', () => {
    const toast = createToastNotifications();
  ...
  it('keeps fatal open while lower toast channels change (Super-99lv)', () => {
    const toast = createToastNotifications();
    toast.setFatal('开库失败。');
    toast.setError('普通错误。');
    toast.setWarning('警告。');
