...
describe('pathIsWithin', () => {
  const root = path.resolve(path.sep, 'super-path-root');
  ...
  it('rejects siblings, parents, and prefix-colliding directories', () => {
    ...
    // 'super-path-root-other' shares a string prefix but is a sibling.
    expect(pathIsWithin(root, `${root}-other`)).toBe(false);
    expect(pathIsWithin(root, path.join(path.dirname(root), 'super-path-root-other'))).toBe(false);
  });
  ...
  it.runIf(process.platform === 'win32')(
    ...
    () => {
      // Regression for Super-59f: exporting a library on E: to
