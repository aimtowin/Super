...
describe("coverSrc", () => {
  it("builds the super preview URL", () => {
    expect(coverSrc("lib-a", "art-b")).toBe("super://preview/lib-a/art-b");
  });
});
...
describe("resolveLivePreviewMedia (Super-a9n)", () => {
  it("plays a ready GIF (image mediaType) as 'gif' when active", () => {
    expect(
      resolveLivePreviewMedia(true, {
        status: "ready",
        url: "super://source/lib/gif-1",
        mediaType: "image",
      }),
    ).toEqual({ url: "super://source/lib/gif-1", kind: "gif" });
  });
  ...
  it("plays a ready video as 'video' when active", () => {
    expect(
      resolveLivePreviewMedia(true, {
        status: "ready",
        url: "super://source/lib/vid-1",
        mediaType: "video",
        posterUrl: "super://preview/lib/poster-1",
      }),
    ).toEqual({ url: "super://source/lib/vid-1", kind: "video" });
  });

  it("plays a ready audio as 'audio' when active (Super hover 音频工单)", () => {
    expect(
      resolveLivePreviewMedia(true, {
        status: "ready",
        url: "super://source/lib/aud-1",
        mediaType: "audio",
      }),
    ).toEqual({ url: "super://source/lib/aud-1", kind: "audio" });
  });
...
describe("resolveLiveVideoMuted (Super hover 音频工单)", () => {
...
describe("shouldPlayLiveAudio (Super hover 音频工单)", () => {
