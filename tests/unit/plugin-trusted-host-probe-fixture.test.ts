...
describe('fixed trusted Host probe fixture', () => {
  it('installs and activates through the trusted Node Host into library storage', async () => {
    const userData = temporaryRoot('super-trusted-probe-user-');
    const library = temporaryRoot('super-trusted-probe-library-');
    const source = temporaryRoot('super-trusted-probe-source-');
    ...
    expect(installed.package.manifest.id).toBe('com.super.trusted-host-probe');
        ...
        if (message.type === 'plugin-trusted.storage-request') {
          queueMicrotask(() => {
            void storage.execute({
              ...
              pluginId: 'com.super.trusted-host-probe',
    ...
    const document = JSON.parse(readFileSync(
      path.join(library, PLUGIN_LIBRARY_DATA_DIRECTORY, 'com.super.trusted-host-probe.json'),
      'utf8',
