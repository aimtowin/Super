...
describe('MCP credential pepper ACL hardening (Super-8b5b.7)', () => {
  it('builds an inheritance-free, current-user-only icacls command shape', () => {
    expect(buildPepperFileAclArgs('/Users/testuser/AppData/Roaming/Super/pepper', 'testuser')).toEqual([
      '/Users/testuser/AppData/Roaming/Super/pepper',
      '/inheritance:r',
      '/grant:r',
      'testuser:F',
    ]);
...
describe('MCP credential token persistence', () => {
  it('keeps the client and token stable when copying it again', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-mcp-credential-persistence-'));
    try {
  ...
  it('does not return a missing or revoked credential token', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-mcp-credential-persistence-missing-'));
    try {
      const store = new McpClientCredentialStore(root);
