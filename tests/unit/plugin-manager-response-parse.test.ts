...
describe('plugin manager response parse', () => {
  ...
  it.each(viewTargets)('parses view contribution target %s without duplicate discriminator errors', (target) => {
    const parsed = parsePluginManagerResponse({
      ...
      contributions: [{
        ...
        url: `super-plugin://com.example.probe/59847245-d394-4012-ad75-35f837393a8f/entry/ui/index.html?libraryId=library-a&contributionId=com.example.probe.${target}`,
      }],
    });
    ...
    const parsed = parsePluginManagerResponse({
      ...
      contributions: [
        ...
        ...viewTargets.map((target) => ({
          ...
          target,
          entryPath: 'entry/ui/index.html',
          url: `super-plugin://com.example.probe/${instanceId}/entry/ui/index.html?libraryId=library-a&contributionId=com.example.probe.${target}`,
        })),
      ],
    });
    expect(parsed.ok).toBe(true);
  ...
  it('parses menus.asset contributions', () => {
    const parsed = parsePluginManagerResponse({
      ...
      contributions: [{
        kind: 'menu',
        id: 'com.dolag.super.image-upscaler.upscale.selection',
        pluginId: 'com.dolag.super.image-upscaler',
        pluginInstanceId: '59847245-d394-4012-ad75-35f837393a8f',
