...
function writePlugin(directory: string): void {
  ...
  writeFileSync(path.join(directory, 'super-plugin.json'), `${JSON.stringify(manifestFixture, null, 2)}\n`);
...
describe('PluginSettingsStore', () => {
  it('synchronizes only the library layer when a library is copied to another device', async () => {
    const pluginSource = temporaryRoot('super-plugin-settings-source-');
    const sourceLibrary = temporaryRoot('super-plugin-settings-library-a-');
    const copiedLibrary = temporaryRoot('super-plugin-settings-library-b-');
    const userA = temporaryRoot('super-plugin-settings-user-a-');
    const userB = temporaryRoot('super-plugin-settings-user-b-');
    ...
    expect(readFileSync(path.join(copiedLibrary, '.super', 'plugin-lock.json'), 'utf8'))
  ...
  it.each([
    ...
    const library = temporaryRoot('super-plugin-settings-library-');
    const userData = temporaryRoot('super-plugin-settings-user-');
  ...
  it('rejects select values outside the declared options', async () => {
    const library = temporaryRoot('super-plugin-select-library-');
    const userData = temporaryRoot('super-plugin-select-user-');
    const manifest = pluginManifestSchema.parse({
      ...manifestFixture,
      contributes: {
        ...manifestFixture.contributes,
  ...
  it('rejects numbers outside the declared range', async () => {
    const library = temporaryRoot('super-plugin-number-range-library-');
    const userData = temporaryRoot('super-plugin-number-range-user-');
    const manifest = pluginManifestSchema.parse({
      ...manifestFixture,
      contributes: {
        ...manifestFixture.contributes,
  ...
  it('skips stale invalid select values when reading an existing settings document', async () => {
    const library = temporaryRoot('super-plugin-select-stale-library-');
    const userData = temporaryRoot('super-plugin-select-stale-user-');
    const manifest = pluginManifestSchema.parse({
      ...manifestFixture,
      contributes: {
        ...manifestFixture.contributes,
  ...
  it('falls back per field, reports diagnostics, and preserves undeclared values on later writes', async () => {
    const library = temporaryRoot('super-plugin-settings-diagnostics-library-');
    const userData = temporaryRoot('super-plugin-settings-diagnostics-user-');
    const manifest = pluginManifestSchema.parse({
      ...manifestFixture,
      contributes: {
        ...manifestFixture.contributes,
    ...
    const settingsDirectory = path.join(library, '.super', 'plugin-settings');
