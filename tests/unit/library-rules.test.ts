...
describe('library path rules', () => {
  it('accepts an absolute path and derives a child target', () => {
    const parentPath = path.resolve('/tmp', 'Super tests');
  ...
  it('strips trailing separators so Windows folder-picker paths still join (Super-sq4i)', () => {
    const parentPath = path.resolve('/tmp', 'Super tests');
  ...
  it('allows a filesystem root as the library parent (Super-qn6k)', () => {
