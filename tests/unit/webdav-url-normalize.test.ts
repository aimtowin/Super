...
describe('normalizeWebDAVBaseUrl (Super-fatf)', () => {
  it('keeps valid http/https URLs unchanged', () => {
    expect(normalizeWebDAVBaseUrl('http://10.96.192.48:5005/Share/Super')).toEqual({
      ok: true,
      value: 'http://10.96.192.48:5005/Share/Super',
    });
  ...
  it('auto-prepends http:// when the protocol is missing (browser-style input)', () => {
    expect(normalizeWebDAVBaseUrl('10.96.192.48:5005/Share/Super')).toEqual({
      ok: true,
      value: 'http://10.96.192.48:5005/Share/Super',
    });
...
describe('publicErrorForWorkerFailure INVALID_URL mapping (Super-fatf)', () => {
...
describe('publicErrorForWorkerFailure ENOSPC mapping (Super-xxx)', () => {
