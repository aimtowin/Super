...
describe('requestTimeoutForCommand', () => {
  ...
  it('does not impose a wall-clock timeout on disk-bound library transfers (Super-4s8b)', () => {
  ...
  it('names worker timeouts so Main can map them to a public reason (Super-sq4i)', () => {
  ...
  it('lets an unbounded Eagle conversion finish after the old 30-minute deadline (Super-4s8b)', async () => {
    vi.useFakeTimers();
    const previousFfmpegPath = process.env.SUPER_FFMPEG_PATH;
    const previousOiioPath = process.env.SUPER_OIIO_PATH;
    process.env.SUPER_FFMPEG_PATH = '/usr/bin/true';
    process.env.SUPER_OIIO_PATH = '/usr/bin/true';
    try {
      ...
      vi.useRealTimers();
      if (previousFfmpegPath === undefined) delete process.env.SUPER_FFMPEG_PATH;
      else process.env.SUPER_FFMPEG_PATH = previousFfmpegPath;
      if (previousOiioPath === undefined) delete process.env.SUPER_OIIO_PATH;
      else process.env.SUPER_OIIO_PATH = previousOiioPath;
    }
  });
  ...
  it('gives AI queue processing a long timeout (Super-iokf)', () => {
  ...
  it('does not kill the Worker when a malformed model render event arrives', async () => {
    ...
    child.emit('message', {
      ...
      format: 'glb',
      renderUrl: 'super://source/library-1/asset-1',
      companionMap: [],
  ...
  it('passes an explicit missing FFmpeg path to the Worker unchanged', async () => {
    ...
    const previousFfmpegPath = process.env.SUPER_FFMPEG_PATH;
    const previousOiioPath = process.env.SUPER_OIIO_PATH;
    const missingFfmpegPath = '/tmp/super-missing-ffmpeg';
    process.env.SUPER_FFMPEG_PATH = missingFfmpegPath;
    process.env.SUPER_OIIO_PATH = '/tmp/super-oiiotool';
    ...
    try {
      ...
      expect(utilityProcessFork).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          env: expect.objectContaining({ SUPER_FFMPEG_PATH: missingFfmpegPath }),
        }),
      );
      ...
      expect(forkOptions?.env?.SUPER_FFMPEG_PATH).toBe(missingFfmpegPath);
