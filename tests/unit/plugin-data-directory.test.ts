...
describe('plugin data directories', () => {
  it('creates a user-scoped directory below userData and validates containment', () => {
    const userDataDirectory = mkdtempSync(path.join(tmpdir(), 'super-plugin-user-data-'));
  ...
  it('creates a library-scoped directory below the library root', () => {
    const userDataDirectory = mkdtempSync(path.join(tmpdir(), 'super-plugin-user-data-'));
    const libraryDirectory = mkdtempSync(path.join(tmpdir(), 'super-plugin-library-'));
    ...
    expect(resolved).toBe(path.join(libraryDirectory, '.super', PLUGIN_FILES_DIRECTORY_NAME, 'com.example.model'));
    expect(existsSync(resolved)).toBe(true);
