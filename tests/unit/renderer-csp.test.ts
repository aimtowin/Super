...
describe('Renderer content security policy', () => {
  it('permits only the application media scheme at the image and video sinks', () => {
    ...
    expect(directive('img-src')).toContain('super:');
    expect(directive('media-src')).toContain('super:');
    expect(directive('script-src')).not.toContain("'unsafe-eval'");
    expect(directive('script-src')).not.toContain('*');
  });

  it('permits super:// fetches (3D viewer + model pipeline, slice C)', () => {
    // three r185 loaders use fetch: GLB artifacts, companion textures, .hdr
    // environment maps and .gltf/.obj/.mtl text all load over super:// in
    ...
    // allowance.
    expect(directive('connect-src')).toContain('super:');
