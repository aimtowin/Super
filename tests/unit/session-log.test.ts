...
function tempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'super-session-log-'));
  tempDirs.push(dir);
  return dir;
}
afterEach(() => {
...
describe('formatSessionTimestamp (Super-wgmy)', () => {
...
describe('sessionLogPathFor', () => {
  it('builds the plain snake/underscore-free session path', () => {
    ...
    expect(sessionLogPathFor('/tmp/logs', d)).toBe(
      path.join('/tmp/logs', 'super-20260816T221412.log'),
    );
  });
});
...
describe('chooseUniqueSessionLogPath', () => {
  ...
  it('falls back to millisecond precision on a same-second collision', () => {
    ...
    const exists = (filePath: string) =>
      filePath === path.join('/tmp/logs', 'super-20260816T221412.log');
    expect(chooseUniqueSessionLogPath('/tmp/logs', d, exists, 42)).toBe(
      path.join('/tmp/logs', 'super-20260816T221412345.log'),
    );
  });
  ...
  it('falls back to pid when the millisecond file also collides', () => {
    ...
    const exists = (filePath: string) =>
      filePath === path.join('/tmp/logs', 'super-20260816T221412.log') ||
      filePath === path.join('/tmp/logs', 'super-20260816T221412345.log');
    expect(chooseUniqueSessionLogPath('/tmp/logs', d, exists, 99)).toBe(
      path.join('/tmp/logs', 'super-20260816T221412-99.log'),
    );
  });
});
...
describe('pruneSessionLogs', () => {
  it('keeps at most MAX_SESSION_LOGS newest files and removes the rest', () => {
    ...
    for (let i = 0; i < MAX_SESSION_LOGS + 3; i += 1) {
      const name = `super-20260816T${String(100000 + i * 7).padStart(6, '0')}.log`;
      const filePath = path.join(dir, name);
      writeFileSync(filePath, 'entry');
      utimesSync(filePath, new Date(2026, 7, 16, 0, 0, i), new Date(2026, 7, 16, 0, 0, i));
    }
    // 最旧的一个，必然被先删。
    writeFileSync(path.join(dir, 'super-20260815T000000.log'), 'oldest');

    const files = readdirSync(dir).filter((n) => n.startsWith('super-') && n.endsWith('.log'));
    ...
    const remaining = readdirSync(dir).filter((n) => n.startsWith('super-') && n.endsWith('.log'));
    expect(remaining).toHaveLength(MAX_SESSION_LOGS);
  });

  it('leaves non-session logs and the legacy superApi.log untouched', () => {
    const dir = tempDir();
    writeFileSync(path.join(dir, 'superApi.log'), 'legacy');
    writeFileSync(path.join(dir, 'other.txt'), 'other');
    writeFileSync(path.join(dir, 'super-20260816T000000.log'), 'one');
    writeFileSync(path.join(dir, 'super-20260816T000001.log'), 'two');
    ...
    utimesSync(
      path.join(dir, 'super-20260816T000000.log'),
      new Date(2026, 7, 16, 0, 0, 0),
      new Date(2026, 7, 16, 0, 0, 0),
    );
    utimesSync(
      path.join(dir, 'super-20260816T000001.log'),
    ...
    expect(remaining).toContain('superApi.log');
    expect(remaining).toContain('other.txt');
    expect(remaining).toContain('super-20260816T000001.log');
    expect(remaining).not.toContain('super-20260816T000000.log');
  });
