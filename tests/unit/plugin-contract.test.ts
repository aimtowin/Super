...
import validManifestFixture from '../fixtures/plugin-manifests/palette-tools.super-plugin.json';
import previewThumbnailManifestFixture from '../fixtures/plugins/preview-thumbnail-probe/super-plugin.json';
...
function validSnapshot() {
  return {
    ...
    files: [
      { path: 'super-plugin.json', byteLength: 1_000, sha256: digest('a'), kind: 'file' },
...
describe('Plugin v1 manifest contract', () => {
  ...
  it('checks Super version, Plugin API and declared native platform before activation', () => {
    ...
    expect(validatePluginManifestCompatibility(manifest, {
      ...
      arch: 'arm64',
      nodeAbi: 140,
    })).toMatchObject({ ok: false, code: 'PLUGIN_SUPER_VERSION_UNSUPPORTED' });
  });
...
describe('Plugin package, installation, trust and resolution contracts', () => {
  ...
  it('keeps trust and version choice device-local and explicit', () => {
    expect(pluginTrustDecisionSchema.parse({
      ...
      sourceFingerprint: 'github:example/super-palette-tools@v1.2.0',
...
describe('Plugin contribution registry and generated SDK', () => {
  ...
  it('generates a standalone Plugin API v1 declaration and transport-safe description', () => {
    const declaration = generatePluginSdkTypeDeclaration('@super/plugin-api');
    ...
    expect(declaration).toContain('interface SuperPluginApi');
    ...
    expect(declaration).toContain('SuperPluginScopedApi');
    ...
    for (const method of [
      'list(input?: SuperPluginPageInput): Promise<unknown>;',
    ...
    expect(declaration).toContain("Pick<SuperPluginApi['jobs'], 'enqueue' | 'reportProgress' | 'cancel' | 'pause' | 'resume' | 'retry'>");
