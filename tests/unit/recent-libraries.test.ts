...
describe('recent libraries store (main)', () => {
  ...
  beforeEach(() => {
    originalE2E = process.env.SUPER_E2E;
    originalRestoreRecent = process.env.SUPER_E2E_RESTORE_RECENT;
    delete process.env.SUPER_E2E;
    delete process.env.SUPER_E2E_RESTORE_RECENT;
    storeDirectory = mkdtempSync(path.join(tmpdir(), 'super-recent-store-'));
    storePath = path.join(storeDirectory, 'recent-library.json');
  });

  afterEach(() => {
    if (originalE2E === undefined) delete process.env.SUPER_E2E;
    else process.env.SUPER_E2E = originalE2E;
    if (originalRestoreRecent === undefined) {
      delete process.env.SUPER_E2E_RESTORE_RECENT;
    } else {
      process.env.SUPER_E2E_RESTORE_RECENT = originalRestoreRecent;
    }
  ...
  it('removes a deleted library from the recent list and clears activePath (Super-9i8)', () => {
    rememberRecentLibrary(storePath, { path: '/libraries/a', name: 'A' });
    rememberRecentLibrary(storePath, { path: '/libraries/b', name: 'B' });
    expect(readActiveLibraryPath(storePath)).toBe('/libraries/b');
  ...
  it('does not persist or read anything when E2E gating disables persistence', () => {
    ...
    process.env.SUPER_E2E_RESTORE_RECENT = '1';
