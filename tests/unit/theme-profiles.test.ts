...
describe('theme profile contract v3', () => {
  it('exposes three themes, each with complete dark and light tone variants', () => {
    expect(THEME_PROFILE_IDS).toEqual(['super', 'vscode', 'soft']);
  ...
  it('accepts semantic color overrides and rejects unsafe or unknown fields', () => {
    ...
    for (const invalid of [
      { version: 1, preset: 'super', overrides: {} },
      { version: 3, preset: 'unknown', overrides: {} },
      { version: 3, preset: 'super', overrides: { '--ui-space-1': '4px' } },
      { version: 3, preset: 'super', overrides: { '--ui-surface-canvas': 'var(--secret)' } },
      { version: 3, preset: 'super', overrides: { '--ui-font-family': 'system-ui' } },
      { version: 3, preset: 'super', extra: true, overrides: {} },
    ]) {
  ...
  it('migrates a v2 preset onto its theme and drops the legacy entry', () => {
    ...
    // super-light and super-dark both collapse onto the super theme.
    const superLight = parseThemeProfile({
      version: 2,
      preset: 'super-light',
      overrides: {},
    });
    expect(superLight.preset).toBe('super');
  ...
  it('resolves the tone variant from the resolved mode, not the profile', () => {
    const dark = resolveThemeProfile(
      { version: THEME_PROFILE_VERSION, preset: 'super', overrides: {} },
      'dark',
    );
    ...
    expect(dark.id).toBe('super');
    expect(light.id).toBe('super');
  });
  ...
  it('clears a previous profile token set before applying the next profile', () => {
    ...
    try {
      ...
      expect(dom.values.get('--ui-action-accent')).toBe(
        THEME_PROFILE_PRESETS.super.tokens.dark['--ui-action-accent'],
      );
