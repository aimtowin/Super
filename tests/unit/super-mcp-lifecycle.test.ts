...
import { callSuperMcpTool } from '../../src/mcp/call-tool';
import { listSuperMcpTools } from '../../src/mcp/tool-catalog';
import type { WorkerCommand } from '../../src/shared/protocol/requests';
import type { WorkerResult } from '../../src/shared/protocol/responses';
import { mcpContext, writeExposure } from './super-mcp-test-fixtures';

/**
 * Super 接管（另一个 agent 的 MCP 生命周期 WIP）：新命令的执行层覆盖。
...
describe('MCP lifecycle commands: folder (Super-dcb1)', () => {
  it('exposes rename/move/delete-empty tools', () => {
    const names = listSuperMcpTools(writeExposure).tools.map((tool) => tool.name);
    expect(names).toContain('super_folder_rename');
    expect(names).toContain('super_folder_move');
    expect(names).toContain('super_folder_delete_empty');
  });
  ...
  it('routes folder.rename to the worker with the new name', async () => {
    ...
    const result = await callTool(worker, 'super_folder_rename', {
      libraryId: 'library-1',
      folderId: 'folder-1',
      newName: 'Renamed',
    });
  ...
  it('routes folder.move with target parent and conflict strategy', async () => {
    ...
    const result = await callTool(worker, 'super_folder_move', {
      libraryId: 'library-1',
      folderIds: ['folder-1'],
  ...
  it('routes folder.delete-empty', async () => {
    ...
    const result = await callTool(worker, 'super_folder_delete_empty', {
      libraryId: 'library-1',
      folderIds: ['folder-1'],
    });
...
describe('MCP lifecycle commands: library (Super-96ys)', () => {
  it('exposes list-recent/close/rename/export/import/delete tools', () => {
    const names = listSuperMcpTools(writeExposure).tools.map((tool) => tool.name);
    for (const name of [
      'super_library_list_recent',
      'super_library_close',
      'super_library_rename',
      'super_library_export',
      'super_library_import_folder',
      'super_library_import_zip',
      'super_library_delete_from_disk',
    ]) {
  ...
  it('routes library.rename with the display name', async () => {
    ...
    const result = await callTool(worker, 'super_library_rename', {
      libraryId: '00000000-0000-4000-8000-0000000000aa',
      displayName: 'New Name',
    });
  ...
  it('routes library.close', async () => {
    ...
    const result = await callTool(worker, 'super_library_close', {
      libraryId: '00000000-0000-4000-8000-0000000000aa',
    });
...
describe('MCP lifecycle commands: tag (Super-j9n2)', () => {
  it('routes tag.rename/delete/merge', async () => {
    ...
    const renameResult = await callTool(renamed, 'super_tag_rename', {
      libraryId: 'library-1',
      tagId: 'tag-1',
      name: 'New Tag',
    });
    ...
    const deleteResult = await callTool(deleted, 'super_tag_delete', {
      libraryId: 'library-1',
      tagId: 'tag-1',
    });
    ...
    const mergeResult = await callTool(merged, 'super_tag_merge', {
      libraryId: 'library-1',
...
describe('MCP lifecycle commands: collection (Super-b6ta)', () => {
  it('routes collection.update/reorder/delete', async () => {
    ...
    const updateResult = await callTool(updated, 'super_collection_update', {
      libraryId: 'library-1',
      collectionId: 'col-1',
      name: 'Renamed',
    });
    ...
    const reorderResult = await callTool(reordered, 'super_collection_reorder', {
      libraryId: 'library-1',
      orderedCollectionIds: ['col-1', 'col-2'],
    });
    ...
    const deleteResult = await callTool(deleted, 'super_collection_delete', {
      libraryId: 'library-1',
      collectionId: 'col-1',
    });
...
describe('MCP lifecycle commands: linked folder (Super-eyus)', () => {
  it('routes linked-folder.create with the explicit source root', async () => {
    ...
    const result = await callTool(worker, 'super_linked_folder_create', {
      libraryId: 'library-1',
...
describe('MCP lifecycle commands: asset helpers (Super-uup6)', () => {
  it('routes asset.copy with target folder', async () => {
    ...
    const result = await callTool(worker, 'super_asset_copy', {
      libraryId: 'library-1',
      assetIds: ['asset-1'],
  ...
  it('reads thumbnail and preview artifact status', async () => {
    ...
    const thumbResult = await callTool(thumb, 'super_asset_thumbnail_get', {
      libraryId: 'library-1',
      assetId: 'asset-1',
    });
    ...
    const previewResult = await callTool(preview, 'super_asset_preview_get', {
      libraryId: 'library-1',
      assetId: 'asset-1',
    });
...
describe('MCP lifecycle commands: smart collection (Super-jxbm)', () => {
  it('routes smart-collection.create with the query definition', async () => {
    ...
    const result = await callTool(worker, 'super_smart_collection_create', {
      libraryId: 'library-1',
      name: 'Recent',
  ...
  it('executes a smart collection rule', async () => {
    ...
    const result = await callTool(worker, 'super_smart_collection_execute', {
      libraryId: 'library-1',
      collectionId: 'sc-1',
    });
