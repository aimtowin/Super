...
import { mcpContext, readCapabilities, readExposure } from './super-mcp-test-fixtures';
...
describe('Super MCP dual-client smoke (Phase C)', () => {
  it('serves tools/list and tools/call to an MCP SDK Client over linked in-memory transport', async () => {
    ...
    const client = new Client({ name: 'super-test-host-a', version: '1.0.0' });
    await Promise.all([
    ...
    const catalogNames = listSuperMcpTools(readExposure).tools.map((tool) => tool.name);
  ...
  it('serves the same Registry tools to a second MCP SDK Client identity', async () => {
    ...
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    const client = new Client({ name: 'super-test-host-b', version: '2.0.0' });
    await Promise.all([
      server.connect(serverTransport),
      client.connect(clientTransport),
    ]);
    ...
    const listed = await client.listTools();
    expect(listed.tools.some((tool) => tool.name === 'super_asset_search')).toBe(true);
    const search = await client.callTool({
      name: 'super_asset_search',
