...
describe('Plugin Standard Host handler', () => {
  it('activates a precompiled entry, brokers a Gateway command, then deactivates', async () => {
    ...
    handler.handle({
      ...
      entryJavaScript: `
        let pluginContext;
        export async function setup(superApi) {
          pluginContext = superApi;
          superApi.subscriptions.add(() => console.log('subscription-disposed'));
          console.log('signal-aborted:' + superApi.signal.aborted);
          const page = await superApi.assets.search({ query: null, limit: 1 });
  ...
  it('delivers domain events through superApi.events.next and attaches cause chains', async () => {
    const posted: PluginRuntimeChildMessage[] = [];
    const handler = createPluginStandardHostHandler({
      postMessage: (message) => {
        posted.push(message);
    ...
    handler.handle({
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          superApi.events.on('library.changed', async (event) => {
            await superApi.storage.set('last-event', {
  ...
  it('invokes onWill hooks and posts hook-decision responses', async () => {
    ...
    handler.handle({
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          superApi.hooks.onWill('asset.trash', async () => ({
            action: 'block',
            code: 'DEMO_BLOCK',
            message: 'refused',
          }));
    ...
    handler.handle({
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          ...
          await superApi.jobs.enqueue({ handlerId: 'tick', payload: { tick: 1 } });
  ...
  it('registers and invokes a derived-field provider in a bounded batch', async () => {
    ...
    handler.handle({
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          superApi.providers.register('derived-field', {
            id: 'ext-upper',
  ...
  it('registers a search provider and streams bounded result chunks', async () => {
    ...
    handler.handle({
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          superApi.providers.registerSearch({
            id: 'fixed-token',
      ...
      entryJavaScript: `
        export async function setup(superApi) {
          superApi.commands.register('probe.write-selection', async (context) => {
            await superApi.storage.set('menu-command', { assetId: context.assetIds[0] });
          });
        }
        export async function dispose() {}
      `,
