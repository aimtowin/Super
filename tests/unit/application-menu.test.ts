...
describe("shouldInstallApplicationMenu (Super-r7gu)", () => {
...
describe("buildApplicationMenuTemplate (Super-46i9)", () => {
  it("omits Electron page-zoom roles that steal Cmd/Ctrl+=,-,0", () => {
    for (const platform of ["darwin", "win32", "linux"] as const) {
      ...
      if (platform === "darwin") {
        ...
        // Super-q0b1: undo/redo are business commands, not text-editing roles.
  ...
  it("adds macOS Edit invert selection via i18n label key (Super-te8p/q0b1)", () => {
    ...
    // Super-166q: Copy is a custom command (not role:copy) so ⌘C can copy files.
...
describe("macOS product menu mirror (Super-q0b1)", () => {
  ...
  it("uses i18n catalog keys for every custom label (no inline locale strings)", () => {
    ...
    // No custom item may carry a hardcoded display label — the Super app
    ...
    expect(hardcodedLabels.map((item) => item.label)).toEqual(["Super"]);
...
describe("shouldHideApplicationMenuBar (Super-znex)", () => {
