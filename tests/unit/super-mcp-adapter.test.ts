...
import { callSuperMcpTool, type SuperMcpPluginToolBridge } from '../../src/mcp/call-tool';
import { createSuperMcpServer } from '../../src/mcp/create-super-mcp-server';
import { listSuperMcpTools, resolveSuperMcpTool, type SuperMcpToolExposure } from '../../src/mcp/tool-catalog';
...
function createTestPermissionBroker(mode?: 'auto' | 'full-access'): McpPermissionBroker {
  const root = mkdtempSync(path.join(tmpdir(), 'super-mcp-challenge-'));
...
function backend(
  exposure: SuperMcpToolExposure = readExposure,
  pluginTools?: SuperMcpPluginToolBridge,
) {
  return {
...
describe('Super MCP tool catalog', () => {
  it('lists requestable Registry tools even when ordinary write permissions are still ask-on-call', () => {
    const listed = listSuperMcpTools(readExposure);
    expect(listed.apiVersion).toBe(1);
    expect(listed.tools.length).toBeGreaterThan(0);
    expect(listed.tools.map((tool) => tool.name)).toContain('super_asset_search');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_library_change_sequence');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_asset_ai_content_get');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_execution_status');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_ui_notify');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_tag_create');
    expect(listed.tools.map((tool) => tool.name)).toContain('super_folder_create');
    expect(listed.tools.find((tool) => tool.name === 'super_tag_create')).toMatchObject({
      riskTier: 'controlled',
  ...
  it('exposes execution- and plan-approved write tools after local write access is granted', () => {
    const listed = listSuperMcpTools(writeExposure);
    const names = listed.tools.map((tool) => tool.name);
    expect(names).toContain('super_tag_create');
    expect(names).toContain('super_folder_create');
    expect(names).toContain('super_collection_create');
    expect(names).toContain('super_ai_enqueue');
    expect(names).toContain('super_library_create');
    expect(names).toContain('super_file_import');
    expect(names).toContain('super_asset_trash');
    expect(names).toContain('super_asset_move');
    expect(names).toContain('super_asset_rename_file');
  ...
  it('keeps MCP tool names Registry-owned and free of eval/shell/sql surfaces', () => {
    ...
    for (const tool of listed.tools) {
      expect(tool.name).toMatch(/^super_[a-z0-9_]+$/u);
      expect(tool.name).not.toMatch(forbidden);
      expect(tool.inputSchema).toBeTypeOf('object');
      expect(tool.annotations.openWorldHint).toBe(false);
    }
    expect(resolveSuperMcpTool('super_eval_code', writeExposure)).toBeUndefined();
  });

  it('exposes explicit filesystem paths and library targets to MCP clients', () => {
    const listed = listSuperMcpTools(writeExposure);
    const libraryCreate = listed.tools.find((tool) => tool.name === 'super_library_create');
    const fileImport = listed.tools.find((tool) => tool.name === 'super_file_import');
...
describe('Super MCP tools/call → Gateway', () => {
  it('routes a public tool through the Gateway with source mcp', async () => {
    ...
    const result = await callSuperMcpTool({
      toolName: 'super_library_inspect',
  ...
  it('returns execution status through the Gateway without Worker dispatch', async () => {
    ...
    const result = await callSuperMcpTool({
      toolName: 'super_execution_status',
      arguments: {},
      context: mcpContext(readExposure),
      exposure: readExposure,
      gateway,
  ...
  it('builds an MCP Server whose tools stay Registry-backed', () => {
    const worker = new RecordingWorker({ ok: true, type: 'library.list', libraries: [] });
    const gateway = createAutomationCommandGateway(worker, resolver());
    const server = createSuperMcpServer({
      gateway,
      backend: backend(readExposure),
    });
    expect(server).toBeTruthy();
    expect(listSuperMcpTools(readExposure).tools.map((tool) => tool.name))
      .toContain('super_asset_search');
  });
  ...
  it('keeps exported plugin tools behind the local write-access gate', async () => {
    ...
    const gateway = createAutomationCommandGateway(worker, resolver());
    const pluginTool = {
      name: 'super_plugin_com_example_probe_declared',
    ...
    const readOnlyServer = createSuperMcpServer({
      gateway,
    ...
    const writeServer = createSuperMcpServer({
      gateway,
...
describe('Super MCP dangerous two-phase challenge (Super-8b5b.2)', () => {
  ...
  it('issues a risk report on the first call and executes only the exact second call', async () => {
    ...
    const first = await callSuperMcpTool({
      toolName: 'super_asset_delete_permanent',
      arguments: input,
      context: mcpContext(readExposure),
      exposure: readExposure,
      gateway,
    ...
    const confirmed = await callSuperMcpTool({
      toolName: 'super_asset_delete_permanent',
      arguments: {
        ...input,
  ...
  it('rejects a stale or forged second call and still executes nothing', async () => {
    ...
    const forged = await callSuperMcpTool({
      toolName: 'super_asset_delete_permanent',
      arguments: {
        ...input,
...
describe('Super MCP permission profiles through the gateway (Super-8b5b.8)', () => {
...
describe('Super MCP jobs, idempotency and reconnect (Super-8b5b.4)', () => {
  it('cancels media jobs by id and reports the cancelled count', async () => {
    ...
    const result = await callSuperMcpTool({
      toolName: 'super_media_jobs_cancel',
  ...
  it('queries media job state from a fresh call after a reconnect without any session context', async () => {
    ...
    for (let i = 0; i < 2; i += 1) {
      const result = await callSuperMcpTool({
        toolName: 'super_media_jobs_list',
  ...
  it('does not execute an import twice for a repeated idempotency key', async () => {
    ...
    const second = await callSuperMcpTool({
      toolName: 'super_file_import',
      arguments: input,
      context: mcpContext(readExposure),
      exposure: readExposure,
