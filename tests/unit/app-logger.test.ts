...
test('persists an error and its cause as structured JSON lines', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-test-'));
  try {
    const logPath = path.join(root, 'super.log');
...
test('reads recent entries and redacts credentials from messages and context', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-read-test-'));
  try {
    const logPath = path.join(root, 'super.log');
    const logger = new AppLogger(logPath);
    logger.info(
...
test('does not persist authorization credentials or an environment object from structured diagnostics', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-sensitive-context-test-'));
  try {
    const logPath = path.join(root, 'super.log');
    const logger = new AppLogger(logPath);
    logger.error(
      ...
      new Error('Authorization: Bearer runtime-secret-token'),
      {
        ...
        environment: { SUPER_API_KEY: 'environment-secret-value' },
      },
    );
...
test('normalizes structured Worker diagnostics so AI causes are readable in the viewer', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-worker-test-'));
  try {
    const logPath = path.join(root, 'super.log');
    const logger = new AppLogger(logPath);
    logger.worker(
...
test('redacts local filesystem paths only for the in-app view', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-path-test-'));
  try {
    const logPath = path.join(root, 'super.log');
    const logger = new AppLogger(logPath);
...
test('filters diagnostics by an automation execution or log correlation ID', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-log-automation-filter-test-'));
  try {
    const logPath = path.join(root, 'super.log');
    const logger = new AppLogger(logPath);
...
test('accepts legacy and session-scoped application log basenames', () => {
  expect(appLogFileNameSchema.safeParse('super.log').success).toBe(true);
  expect(appLogFileNameSchema.safeParse('super-20260825T120000.log').success).toBe(true);
  expect(appLogFileNameSchema.safeParse('/tmp/super.log').success).toBe(false);
