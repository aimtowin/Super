...
describe('plugin install failure detail', () => {
  it('returns failureCode and message when a non-.bin symlink blocks install', async () => {
    const source = temporaryRoot('super-plugin-symlink-');
    const userData = temporaryRoot('super-plugin-symlink-user-');
    writePlugin(source);
  ...
  it('ignores npm node_modules/.bin shims so packaged deps can install', async () => {
    const source = temporaryRoot('super-plugin-npmbin-');
    const userData = temporaryRoot('super-plugin-npmbin-user-');
    writePlugin(source);
