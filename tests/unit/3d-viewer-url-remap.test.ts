...
describe('url-remap (Super-qvc6 / 3D-12)', () => {
  it('builds super://preview and super://source URLs', () => {
    expect(superPreviewUrl('lib-1', 'art-9')).toBe(
      'super://preview/lib-1/art-9',
    );
    expect(superSourceUrl('lib-1', 'asset-1')).toBe(
      'super://source/lib-1/asset-1',
    );
    expect(superSourceUrl('lib-1', 'asset-1', 'rev a/b')).toBe(
      'super://source/lib-1/asset-1?revision=rev%20a%2Fb',
    );
  });
  ...
  it('remaps exact relative paths through the companion map', () => {
    ...
    ).toBe('super://source/lib-1/asset-normal?revision=rev-1');
  });
  ...
  it('never basename-matches absolute URLs', () => {
    expect(
      remapCompanionUrlByBasename({
        fileName: 'super://preview/lib-1/asset-albedo',
        libraryId: 'lib-1',
        companionMap,
      }),
    ).toBeNull();
  ...
  it('rewrites glTF image and buffer uris only', () => {
    ...
    expect(rewritten.images?.[0]?.uri).toBe(
      'super://source/lib-1/asset-albedo?revision=rev-1',
    );
  ...
  it('rewrites MTL texture references, preserving options', () => {
    ...
    expect(rewritten).toContain(
      'map_Kd -s 1 1 1 super://source/lib-1/asset-albedo?revision=rev-1',
    );
    expect(rewritten).toContain(
      'map_Ks super://source/lib-1/asset-normal-subdir?revision=rev-1',
    );
