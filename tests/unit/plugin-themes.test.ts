...
import iframeProbeManifest from '../fixtures/plugins/iframe-workspace-probe/super-plugin.json';
...
describe('plugin theme token packages (PLUGIN-032)', () => {
  ...
  it('extracts and merges manifest theme packages for the active resolved theme', () => {
    ...
    expect(mergePluginIframeThemeTokens({
      ...
      '--super-plugin-ref-accent': '#3b82f6',
      '--super-plugin-token-badge': '#ff9a3c',
    });
  });

  it('builds a schema-valid plugin-ui.theme-changed message with plugin aliases', () => {
    const message = buildPluginUiThemeHostMessage({
      contributionId: 'com.super.iframe-workspace-probe.workspace-probe',
    ...
    if (parsed.type === 'plugin-ui.theme-changed') {
      ...
      expect(parsed.tokens['--super-plugin-ref-accent']).toBe('#2563eb');
      expect(parsed.tokens['--super-plugin-token-badge']).toBe('#c45a00');
    }
  });
