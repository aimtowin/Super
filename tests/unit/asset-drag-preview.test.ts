...
describe('assetDragPreviewModel (REQ-DND-003)', () => {
  it('carries the thumbnail url and shows no badge for a single-asset drag', () => {
    const model = assetDragPreviewModel({
      thumbnailUrl: 'super://preview/lib/artifact',
      fileName: 'hero.png',
      count: 1,
    });
    expect(model.thumbnailUrl).toBe('super://preview/lib/artifact');
  ...
  it('shows the copy "+" badge when Option/Alt copy mode is active (Super-aa3)', () => {
    expect(
      assetDragPreviewModel({
        thumbnailUrl: null,
        fileName: 'hero.png',
