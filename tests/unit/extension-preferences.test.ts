...
describe('extension save preferences', () => {
  ...
  it('uses stable storage keys', () => {
    expect(NOTIFICATIONS_ENABLED_KEY).toBe('superNotificationsEnabled');
    expect(FOCUS_APP_AFTER_SAVE_KEY).toBe('superFocusAppAfterSave');
    expect(REVEAL_IN_LIBRARY_AFTER_SAVE_KEY).toBe(
      'superRevealInLibraryAfterSave',
    );
    expect(DRAG_RADIAL_MENU_ENABLED_KEY).toBe('superDragRadialMenuEnabled');
  });
});
