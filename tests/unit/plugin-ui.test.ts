...
describe('plugin custom UI contract', () => {
  it('requires a relative entry path for workspace views', () => {
    const result = pluginManifestSchema.safeParse({
      ...
      author: 'Super',
      license: 'MIT',
      engines: { super: '>=0.1.0', pluginApi: 1 },
  ...
  it('accepts only typed iframe messages and binds them to the iframe source', () => {
    ...
    expect(isTrustedPluginUiMessage({
      origin: 'super-plugin://com.example.iframe',
      source: 'iframe-window',
      expectedOrigin: 'null',
      expectedPluginOrigin: 'super-plugin://com.example.iframe',
      expectedSource: 'iframe-window',
    })).toBe(true);
    expect(isTrustedPluginUiMessage({
      origin: 'https://superApi.invalid',
      source: 'iframe-window',
      expectedOrigin: 'null',
      expectedSource: 'iframe-window',
    })).toBe(false);
    ...
    if (themedParsed.type === 'plugin-ui.theme-changed') {
      expect(themedParsed.tokens['--super-plugin-token-badge']).toBe('#ff9a3c');
    }
  });
  ...
  it('creates and parses non-app plugin asset URLs without allowing traversal', () => {
    ...
    expect(new URL(url).protocol).toBe('super-plugin:');
    ...
    expect(parsePluginUiAssetRequest(
      'super-plugin://com.example.iframe/instance-a/../secret.js?libraryId=library-a&contributionId=bad',
    )).toBeUndefined();
    expect(parsePluginUiAssetRequest(
      'super-plugin://com.example.iframe/instance-a/%2e%2e/secret.js?libraryId=library-a&contributionId=com.example.iframe.probe-view',
    )).toBeUndefined();
