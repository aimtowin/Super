...
describe('QuickJS/WASM sandbox engine prototype', () => {
  it('transpiles TypeScript and resumes an async host bridge without Node globals', async () => {
    const result = await runQuickJsSandboxPrototype(
      `
        const message: string = await superApi.readText('hello');
        console.log(message);
  ...
  it('exposes only fixed asset automation methods through the asynchronous host bridge', async () => {
    ...
    const result = await runQuickJsSandboxPrototype(
      `
        const page = await superApi.assets.search({ query: 'Ser', limit: 2, offset: 0 });
        return await superApi.assets.setRating(page.items.map((asset) => asset.id), 4);
      `,
      {
  ...
  it('exposes organize automation without leaking library or linked-folder paths to the script guest', async () => {
    ...
    const result = await runQuickJsSandboxPrototype(
      `
        const library = await superApi.library.inspect();
        const linked = await superApi.linkedFolders.list();
        const tags = await superApi.tags.list();
        const created = await superApi.tags.create('天气-云');
        const assigned = await superApi.tags.assign(['asset-a'], [created.id]);
        const folder = await superApi.folders.create('天气');
        return {
          library,
          ...
          switch (commandId) {
            case 'library.inspect':
              return {
                libraryId: 'library-1',
                displayName: 'Temp',
                libraryPath: '/must-not-reach-script/Temp.superlibrary',
              };
  ...
  it('exposes library.changeSequence for cross-process plan fencing without extra input', async () => {
    ...
    const result = await runQuickJsSandboxPrototype(
      `
        const sequence = await superApi.library.changeSequence();
        return sequence;
      `,
      {
        executeAutomationCommand: async (commandId, input) => {
  ...
  it('exposes asset automation without leaking relative or absolute paths to the script guest', async () => {
    ...
    const result = await runQuickJsSandboxPrototype(
      `
        const folders = await superApi.folders.list();
        const assets = await superApi.assets.list();
        const metadata = await superApi.assets.getMetadata(assets.items[0].id);
        const aiContent = await superApi.assets.getAiContent(assets.items[0].id);
        const copied = await superApi.assets.copyFilePaths(assets.items.map((asset) => asset.id));
        const renamed = await superApi.assets.renameFile(assets.items[0].id, 'first-tagged');
        const batchRenamed = await superApi.assets.renameFiles([{ assetId: assets.items[0].id, newBaseName: 'first-concept' }]);
        const read = await superApi.assets.readContent(assets.items[0].id, { maxBytes: 2 });
        const replaced = await superApi.assets.replaceContent(assets.items[0].id, 'AQID', { mimeHint: 'image/png' });
        const trashed = await superApi.assets.moveToTrash(assets.items.map((asset) => asset.id));
        const trash = await superApi.trash.list();
        const restored = await superApi.trash.restoreIfOriginalVacant(trash.items.map((asset) => asset.id));
        const palette = await superApi.palettes.mostFrequent({ days: 2, limit: 3 });
        return {
  ...
  it('exposes headless library creation and file import through the same automation bridge', async () => {
    ...
    const result = await runQuickJsSandboxPrototype(
      `
        const created = await superApi.library.create({
          displayName: 'Headless',
          selectedParentPath: '/private/parent',
        });
        const imported = await superApi.files.import({
          sourceKind: 'files',
  ...
  it('caps script output and pending asynchronous host calls', async () => {
    ...
    await expectSandboxFailure(
      `
        const first = superApi.readText('one');
        const second = superApi.readText('two');
      ...
      runQuickJsSandboxPrototype(
        `
          async function load(value) {
            return await superApi.readText(value);
          }
  ...
  it('times out an unresolved host promise and leaves a fresh execution usable', async () => {
    ...
    await expect(
      runQuickJsSandboxPrototype('return await superApi.readText("wait");', neverResolvingHost, {
        wallTimeoutMs: 20,
      }),
  ...
  it('does not charge time spent awaiting a host call against the CPU budget', async () => {
    ...
    await expect(
      runQuickJsSandboxPrototype('return await superApi.readText("wait");', delayedHost, {
        cpuTimeoutMs: 10,
        wallTimeoutMs: 250,
      }),
  ...
  it('honours cancellation while awaiting an untrusted script host call', async () => {
    ...
    const execution = runQuickJsSandboxPrototype(
      'return await superApi.readText("wait");',
      neverResolvingHost,
