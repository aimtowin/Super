...
function root(): string {
  const value = mkdtempSync(path.join(tmpdir(), 'super-automation-script-file-'));
  roots.push(value);
  return value;
}

describe('AutomationScriptFileService', () => {
  it('opens a user-selected Super script without exposing its path and binds its handle to the sender and exact source', async () => {
    const directory = root();
    const filename = path.join(directory, 'rating.super.ts');
    const source = "return await superApi.assets.search({ query: 'name:Ser' });";
  ...
  it('saves only an allowed extension and returns a Main-issued handle for the exact written source', async () => {
    const directory = root();
    const filename = path.join(directory, 'rename.super.js');
    const source = "return await superApi.assets.renameFiles([{ assetId: 'a', newName: 'b' }]);";
    const service = new AutomationScriptFileService({
  ...
  it('opens a recent script by opaque handle without exposing its path', async () => {
    const directory = root();
    const filename = path.join(directory, 'recent.super.ts');
    const source = "return await superApi.assets.search({ query: 'name:Ser' });";
    writeFileSync(filename, source);
    ...
    recentScripts.record('recent.super.ts', filename);
    const service = new AutomationScriptFileService({
      selectOpenScript: async () => undefined,
      selectSaveScript: async () => undefined,
      recentScripts,
  ...
  it('fails closed for unsupported files, over-limit text, and discarded dialogs', async () => {
    ...
    const tooLarge = path.join(directory, 'large.super.ts');
