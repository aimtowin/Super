...
describe('Plugin package IPC bridge', () => {
  ...
  it('routes preview and thumbnail provider broker requests without exposing paths', async () => {
    const userData = temporaryRoot('super-plugin-ipc-media-user-');
  ...
  it('forwards metadata-provider requests through the Main broker seam', async () => {
    const userData = temporaryRoot('super-plugin-ipc-metadata-user-');
  ...
  it('forwards import/export/ai provider requests through the Main broker seam', async () => {
    const userData = temporaryRoot('super-plugin-ipc-broker-user-');
  ...
  it('rejects malformed Renderer input before selecting a path or touching package storage', async () => {
    const userData = temporaryRoot('super-plugin-ipc-user-');
  ...
  it('checks eligible updates when enabling the global auto-update policy', async () => {
    const userData = temporaryRoot('super-plugin-ipc-global-auto-update-');
  ...
  it('keeps a Main-selected local path out of Renderer responses and supports a cancelled picker', async () => {
    const source = temporaryRoot('super-plugin-ipc-source-');
    const userData = temporaryRoot('super-plugin-ipc-user-');
    writePlugin(source);
  ...
  it('returns plugin command failure diagnostics through the management bridge', async () => {
    const userData = temporaryRoot('super-plugin-ipc-command-error-user-');
    const handler = createPluginPackageRequestHandler({
      manager: createManager(userData),
      resolveLibraryDirectory: async () => userData,
      chooseLocalPackage: async () => undefined,
  ...
  it('returns both exact conflict candidates, then requires library trust before it resolves', async () => {
    const userSource = temporaryRoot('super-plugin-ipc-user-source-');
    const librarySource = temporaryRoot('super-plugin-ipc-library-source-');
    const userData = temporaryRoot('super-plugin-ipc-user-');
    const library = temporaryRoot('super-plugin-ipc-library-');
  ...
  it('notifies Main after trust and safe-mode mutations so the Host can refresh', async () => {
    const source = temporaryRoot('super-plugin-ipc-refresh-source-');
    const userData = temporaryRoot('super-plugin-ipc-refresh-user-');
    const library = temporaryRoot('super-plugin-ipc-refresh-library-');
    writePlugin(source);
  ...
  it('replaces the active package through the typed bridge and reports rollback unavailable', async () => {
    const firstSource = temporaryRoot('super-plugin-ipc-rollback-first-');
    const userData = temporaryRoot('super-plugin-ipc-rollback-user-');
    const library = temporaryRoot('super-plugin-ipc-rollback-library-');
  ...
  it('exposes a quarantined package safely and lets the management bridge clear only its local quarantine', async () => {
    const source = temporaryRoot('super-plugin-ipc-quarantine-source-');
    const userData = temporaryRoot('super-plugin-ipc-quarantine-user-');
    const library = temporaryRoot('super-plugin-ipc-quarantine-library-');
    writePlugin(source);
    const manager = createManager(userData);
  ...
  it('enriches settings.pages with super-plugin:// url and survives Preload parse', async () => {
    const userData = temporaryRoot('super-plugin-ipc-contrib-user-');
    ...
    expect(raw).toMatchObject({
      ...
      contributions: [{
        ...
        url: expect.stringMatching(
          /^super-plugin:\/\/com\.example\.probe\/59847245-d394-4012-ad75-35f837393a8f\/entry\/ui\/settings\.html/u,
        ),
      }],
    });
    ...
    expect(parsed.contributions[0]).toMatchObject({
      kind: 'view',
      target: 'settings.pages',
      url: expect.stringMatching(/^super-plugin:\/\//u),
    });
  });
});
