...
describe('browser extension save client', () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          ...
          'X-Super-Kind': 'image',
          'X-Super-Content-Type': 'image/png',
          'X-Super-Filename': encodeURIComponent('shot.png'),
        }),
      }),
    );
  ...
  it('omits remote URL metadata for local-file uploads', async () => {
    ...
    expect(request.headers).not.toHaveProperty('X-Super-Source-Page-Url');
    expect(request.headers).not.toHaveProperty('X-Super-Media-Url');
  });
  ...
  it('reports Super as unreachable only after every configured port fails', async () => {
    const fetchFn = vi.fn(async () => {
    ...
    expect(fetchFn).toHaveBeenCalledTimes(SUPER_PORTS.length);
  });
  ...
  it('probes Super via /ping and /folders without a token', async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValueOnce(mockFetchResponse({ status: 200 }))
      .mockResolvedValueOnce(mockFetchResponse({ status: 200, text: '{"folders":[]}' }));

    await expect(probeSuperConnection(fetchFn)).resolves.toEqual({ kind: 'connected' });
  ...
  it('reports offline when ping cannot reach Super', async () => {
    const fetchFn = vi.fn(async () => {
      throw new TypeError('connection refused');
    });

    await expect(probeSuperConnection(fetchFn)).resolves.toEqual({ kind: 'offline' });
    expect(fetchFn).toHaveBeenCalledTimes(SUPER_PORTS.length);
  });
