...
function makeActions(calls: RecordedCall[]): SidebarCommandActions {
  ...
  return {
    ...
    addLinkedFolderToCollection: record('addLinkedFolderToCollection'),
...
describe('文件夹分支：可见性矩阵（与历史内联 JSX 条件一致）', () => {
  ...
  it('linked 文件夹加入合集命令保留根 id 与当前相对路径', () => {
    ...
    void registry.get('folder.linked-folder.add-to-collection')?.run(ctx);
    expect(calls).toEqual([
      {
        action: 'addLinkedFolderToCollection',
        args: [LINKED_FOLDER.folderId, 'props/wood', 'wood'],
      },
    ]);
  });
...
describe('文件夹分支：快捷键展示（Super-vf8x）', () => {
...
describe('文件夹分支：平台条件标题', () => {
  ...
  it('folder.move-to 保留注册但已从菜单隐藏（Super-nno6）', () => {
    const { ctx } = makeCtx();
