...
describe('fixed standard Host probe fixture', () => {
  it('installs and activates through guest realm into library storage', async () => {
    const userData = temporaryRoot('super-host-probe-user-');
    const library = temporaryRoot('super-host-probe-library-');
    const source = temporaryRoot('super-host-probe-source-');
    ...
    expect(installed.package.manifest.id).toBe('com.super.standard-host-probe');
    ...
    const result = await runPluginGuestActivate({
      ...
      executeStorageOperation: async (input) => storage.execute({
        ...
        pluginId: 'com.super.standard-host-probe',
    ...
    const document = JSON.parse(readFileSync(
      path.join(library, PLUGIN_LIBRARY_DATA_DIRECTORY, 'com.super.standard-host-probe.json'),
      'utf8',
