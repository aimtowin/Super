...
async function createServerHarness(options: {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'super-mcp-http-'));
  ...
  const clientConfig = parsedConfig.mcpServers.super;
  return {
    server,
...
describe('Embedded MCP Streamable HTTP server', () => {
  ...
  it('starts on loopback, authenticates before parsing, and supports the SDK client', async () => {
    ...
    expect(listed.tools.map((tool) => tool.name)).toContain('super_library_list_open');
    ...
    await client.callTool(
      { name: 'super_library_list_open', arguments: {} },
      undefined,
      { onprogress: progress },
    );
  ...
  it('detaches the protocol session without cancelling the business execution', async () => {
    ...
    // Super review: the SDK transport's close() only aborts its streams —
