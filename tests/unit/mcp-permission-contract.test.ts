...
import { readCapabilities, readExposure } from './super-mcp-test-fixtures';
...
describe('MCP permission contract', () => {
  it('keeps the Registry, tools/list projection and manual on the same model', () => {
    ...
    const tools = listSuperMcpTools(readExposure).tools;
    expect(tools.some((tool) => tool.name === 'super_tag_create')).toBe(true);
    ...
    // Super-8b5b.2: dangerous operations are exposed only through the
