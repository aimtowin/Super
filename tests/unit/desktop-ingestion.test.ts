...
describe('desktop drop path resolution', () => {
  ...
  it('classifies one folder separately and accepts multi-source selections', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-drop-unit-'));
    roots.push(root);
...
describe('clipboard staging', () => {
  it('writes private PNG staging and removes it after Worker preparation', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-clipboard-unit-'));
    roots.push(root);
  ...
  it('reports empty clipboard images and sweeps only Super-owned stale staging', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-clipboard-sweep-'));
    roots.push(root);
    expect(() => stageClipboardImage({ isEmpty: () => true, toPNG: () => Buffer.alloc(0) }, root))
      .toThrow('CLIPBOARD_IMAGE_NOT_FOUND');
    mkdirSync(path.join(root, 'super-clipboard-stale'));
