...
describe('renderer request protocol', () => {
  ...
  it('accepts path-free managed copy and one-shot undo requests (Super-2vn)', () => {
    expect(parseRendererRequest({
...
describe('preview response protocol', () => {
  it('carries only opaque URLs and actionable artifact state', () => {
    const result = parseRendererResult({
      ...
      status: 'failed',
      kind: 'webm_proxy',
      posterUrl: 'super://preview/library-01/poster-01',
  ...
  it('allows an audio proxy response without exposing a source path', () => {
    expect(parseRendererResult({
      ...
      status: 'ready',
      kind: 'audio_proxy',
      url: 'super://proxy/library-01/artifact-01',
      playbackMode: 'proxy',
  ...
  it('carries only numeric EXR part selection and labels to the renderer', () => {
    expect(parseRendererResult({
      ...
      status: 'ready',
      kind: 'thumbnail',
      url: 'super://preview/library-01/artifact-01',
      exrPlanes: [
...
describe('worker request protocol', () => {
    ...
    expect(parseWorkerResponse({
      ...
      result: {
        ...
        entries: [{
          ...
          thumbnailAbsolutePath: '/private/library/.super/artifacts/thumb.webp',
        }],
      },
  ...
  it('validates AI progress and completion events before Main forwards them', () => {
    ...
    // Super-c9r3: the cleared event carries the affected IDs (strict).
  ...
  it('requires both Eagle source and Super destination paths to open Eagle', () => {
    expect(
      parseWorkerRequest({
        requestId: 'req-01',
        command: {
...
describe('public errors', () => {
  ...
  it('classifies a missing SQLite native module as an engine failure, not library damage', () => {
    const sqliteError = Object.assign(
      new Error(
        'The specified module could not be found.\r\n\\\\?\\C:\\Program Files\\Super\\resources\\app.asar.unpacked\\node_modules\\better-sqlite3\\build\\Release\\better_sqlite3.node',
      ),
  ...
  it('keeps library-parent and transfer-timeout reasons renderer-safe (Super-sq4i)', () => {
...
describe('model companion protocol (slice A, Super-fu2i)', () => {
  it('accepts model.resolve-companions as a read-only worker command', () => {
    ...
    // Slice C (Super-qvc6) opened the renderer request surface for the 3D
...
describe('model convert-fbx protocol (slice B, Super-5ygi)', () => {
  it('accepts model.convert-fbx as a worker command (not forgeable by renderer)', () => {
    ...
    // Slice C (Super-qvc6) drives the conversion from the 3D viewer; the
...
describe('sync open-remote-libraries protocol (Super-xffq)', () => {
