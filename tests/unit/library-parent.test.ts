...
describe('resolveWritableLibraryParent (Super-sq4i)', () => {
  it('creates a missing parent folder when asked', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-parent-'));
    roots.push(root);
  ...
  it('accepts a trailing separator from a folder picker', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-parent-trail-'));
    roots.push(root);
    const resolved = resolveWritableLibraryParent({
  ...
  it('rejects a destination inside the source library', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-parent-inside-'));
    roots.push(root);
  ...
  it('accepts a filesystem root as a parent when it is writable (Super-qn6k)', () => {
  ...
  it('rejects a file used as the parent folder', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-parent-file-'));
    roots.push(root);
