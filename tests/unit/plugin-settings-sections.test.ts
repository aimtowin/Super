...
describe('Plugin settings sections IPC', () => {
  ...
  it('round-trips host settings through PluginSettingsStore and mirrors to plugin storage', async () => {
    const source = temporaryRoot('super-plugin-settings-ipc-source-');
    const userData = temporaryRoot('super-plugin-settings-ipc-user-');
    const library = temporaryRoot('super-plugin-settings-ipc-library-');
    writePlugin(source);
    ...
    const settingsPath = path.join(library, '.super', 'plugin-settings', `${libraryPackage.pluginId}.json`);
    expect(readFileSync(settingsPath, 'utf8')).toContain('"palette-size": 16');

    const storagePath = path.join(library, '.super', 'plugin-data', `${libraryPackage.pluginId}.json`);
