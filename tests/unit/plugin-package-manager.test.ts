...
import manifestFixture from '../fixtures/plugin-manifests/palette-tools.super-plugin.json';
...
function writePlugin(
  ...
  writeFileSync(path.join(directory, 'super-plugin.json'), `${JSON.stringify(manifest, null, 2)}\n`);
...
describe('PluginPackageManager installation and integrity', () => {
  it('resolves user global candidates without a library and keeps unrestricted packages opt-in', async () => {
    const restrictedSource = temporaryRoot('super-global-restricted-source-');
    const restrictedUserData = temporaryRoot('super-global-restricted-user-');
    ...
    const trustedSource = temporaryRoot('super-global-trusted-source-');
    const trustedUserData = temporaryRoot('super-global-trusted-user-');
  ...
  it('does not expose a library-scoped global package through the no-library path', async () => {
    const source = temporaryRoot('super-library-only-source-');
    const userData = temporaryRoot('super-library-only-user-');
    const library = temporaryRoot('super-library-only-library-');
  ...
  it('installs a verified directory by staging and atomically adds the package to the selected store', async () => {
    const source = temporaryRoot('super-plugin-source-');
    const userData = temporaryRoot('super-plugin-user-');
    writePlugin(source);
  ...
  it('keeps library code and non-secret lock synchronized while trust stays on each device', async () => {
    const source = temporaryRoot('super-plugin-source-');
    const library = temporaryRoot('super-plugin-library-');
    const userA = temporaryRoot('super-plugin-device-a-');
    const userB = temporaryRoot('super-plugin-device-b-');
    writePlugin(source);
  ...
  it('installs a Windows tar zip whose entries are prefixed with ./', async () => {
    ...
    writePlugin(source);
    const archive = new AdmZip();
    archive.addFile('./super-plugin.json', readFileSync(path.join(source, 'super-plugin.json')));
  ...
  it('uses a GitHub repository client to pick the latest compatible tag without Releases or build commands', async () => {
    ...
    const installed = await manager.installFromGitHub({
      repository: 'https://github.com/example/super-palette-tools',
      scope: 'user',
      client: {
        async listTags() {
          return [
    ...
    expect(installed.package.lock.sourceFingerprint).toBe(
      'github:https://github.com/example/super-palette-tools',
    );
  ...
  it('prefers a platform-matched GitHub Release asset over zipball fallback', async () => {
    const source = temporaryRoot('super-plugin-release-source-');
    const userData = temporaryRoot('super-plugin-release-user-');
      ...
      client: {
        ...
        async listReleases() {
          return [{
            ...
            assets: [{
              name: assetName,
              browserDownloadUrl: `https://github.com/example/super-palette-tools/releases/download/v2.0.0/${assetName}`,
  ...
  it('reports available GitHub updates and honors auto-update preferences', async () => {
    const oldSource = temporaryRoot('super-plugin-update-old-');
    const newSource = temporaryRoot('super-plugin-update-new-');
    const userData = temporaryRoot('super-plugin-update-user-');
    ...
    const manager = createManager(userData);
    const repository = 'https://github.com/example/super-palette-tools';
  ...
  it('treats a newer immutable commit from the same GitHub repository as an ordinary source-stable upgrade', async () => {
    const oldSource = temporaryRoot('super-plugin-github-old-');
    const newSource = temporaryRoot('super-plugin-github-new-');
    const userData = temporaryRoot('super-plugin-github-user-');
    const library = temporaryRoot('super-plugin-github-library-');
...
describe('PluginPackageManager selection, updates and Safe Mode', () => {
  it('requires an explicit per-device choice for a user/library version conflict and selects only one', async () => {
    const sourceOne = temporaryRoot('super-plugin-source-one-');
    const sourceTwo = temporaryRoot('super-plugin-source-two-');
    const library = temporaryRoot('super-plugin-library-');
    const userData = temporaryRoot('super-plugin-user-');
  ...
  it('preserves a same-source, same-mode, no-new-permission version selection but requires a new choice for risk changes', async () => {
    const sourceOne = temporaryRoot('super-plugin-source-one-');
    const sourceTwo = temporaryRoot('super-plugin-source-two-');
    const sourceThree = temporaryRoot('super-plugin-source-three-');
    const library = temporaryRoot('super-plugin-library-');
    const userData = temporaryRoot('super-plugin-user-');
    writePlugin(sourceOne, { version: '1.2.0' });
    writePlugin(sourceTwo, { version: '1.3.0' });
  ...
  it('requires a fresh explicit install to roll back after in-place version replacement', async () => {
    const sourceOne = temporaryRoot('super-plugin-rollback-one-');
    const sourceTwo = temporaryRoot('super-plugin-rollback-two-');
    const library = temporaryRoot('super-plugin-rollback-library-');
    const userData = temporaryRoot('super-plugin-rollback-user-');
    writePlugin(sourceOne, { version: '1.2.0' });
    writePlugin(sourceTwo, { version: '1.3.0' });
    const manager = createManager(userData);
    const first = await manager.installFromDirectory({
  ...
  it('keeps unrestricted packages disabled until the user explicitly enables them', async () => {
    const source = temporaryRoot('super-plugin-unrestricted-default-');
    const userData = temporaryRoot('super-plugin-unrestricted-default-user-');
    const library = temporaryRoot('super-plugin-unrestricted-default-library-');
  ...
  it('inherits user-scoped enablement across libraries for unrestricted global plugins', async () => {
    const source = temporaryRoot('super-plugin-unrestricted-inherit-');
    const userData = temporaryRoot('super-plugin-unrestricted-inherit-user-');
    const libraryA = temporaryRoot('super-plugin-unrestricted-inherit-library-a-');
    const libraryB = temporaryRoot('super-plugin-unrestricted-inherit-library-b-');
    writePlugin(source, { runtime: 'unrestricted', version: '1.0.0' });
    const manager = createManager(userData);
    const installed = await manager.installFromDirectory({
      directory: source,
  ...
  it('uses Safe Mode to suppress unrestricted (trusted) resolution while leaving restricted packages resolvable', async () => {
    const trustedSource = temporaryRoot('super-plugin-trusted-source-');
    const restrictedSource = temporaryRoot('super-plugin-restricted-source-');
    const userData = temporaryRoot('super-plugin-user-');
    const library = temporaryRoot('super-plugin-library-');
    ...
    const restrictedManifestPath = path.join(restrictedSource, 'super-plugin.json');
  ...
  it('quarantines three consecutive supervised crashes only for this library and lets the user explicitly re-enable it', async () => {
    const source = temporaryRoot('super-plugin-quarantine-source-');
    const userData = temporaryRoot('super-plugin-quarantine-user-');
    const library = temporaryRoot('super-plugin-quarantine-library-');
    writePlugin(source);
    const manager = createManager(userData);
    const installed = await manager.installFromDirectory({
      directory: source,
  ...
  it('lists installed package metadata without re-hashing package bytes', async () => {
    const source = temporaryRoot('super-plugin-metadata-source-');
    const userData = temporaryRoot('super-plugin-metadata-user-');
  ...
  it('clears stale resolutions after uninstall so reinstall can enable without a false update prompt', async () => {
    const sourceV1 = temporaryRoot('super-plugin-reinstall-v1-');
    const sourceV2 = temporaryRoot('super-plugin-reinstall-v2-');
    const userData = temporaryRoot('super-plugin-reinstall-user-');
    const library = temporaryRoot('super-plugin-reinstall-library-');
