...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-zip-test-'));
...
// and returns an absolute path (Super-59f). Probe for a writable drive
...
function createForeignDriveRoot(): string | null {
  ...
  for (const letter of 'CDEFGHIJKLMNOPQRSTUVWXYZ') {
    ...
    try {
      const candidate = mkdtempSync(path.join(driveRoot, 'super-xdrive-'));
...
describe('LibraryService ZIP export', () => {
  ...
  it.runIf(process.platform === 'win32')(
    'accepts a ZIP destination on a different drive letter (Super-59f)',
    async (ctx) => {
  ...
  it('exports a library as a valid ZIP with Assets, revisions, trash, and library.db', async () => {
    ...
    expect(entryNames).toContain('.super/library.db');
    expect(entryNames.some((n) => n.startsWith('Assets/') && !n.endsWith('/'))).toBe(true);

    // Verify no previews or operations.
    expect(entryNames.some((n) => n.startsWith('.super/previews/'))).toBe(false);
    expect(entryNames.some((n) => n.startsWith('.super/operations/'))).toBe(false);
  ...
  it('excludes .super/previews/ and .super/operations/ from ZIP export', async () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    mkdirSync(path.join(created.libraryPath, '.super', 'previews'), { recursive: true });
    writeFileSync(path.join(created.libraryPath, '.super', 'previews', 'thumb.jpg'), 'data');

    mkdirSync(path.join(created.libraryPath, '.super', 'operations'), { recursive: true });
    writeFileSync(path.join(created.libraryPath, '.super', 'operations', 'op.json'), '{}');
  ...
  it('includes .super/artifacts in ZIP export (Super-pxd)', async () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    writeFileSync(
      path.join(created.libraryPath, '.super', 'artifacts', `${thumb.artifactId}.wave-tmp.png`),
      'temp',
    );
    ...
    expect(entryNames).toContain(`.super/artifacts/${thumb.artifactId}.webp`);
...
describe('LibraryService ZIP import', () => {
  ...
  it('imports a library from a valid ZIP', async () => {
    ...
    expect(existsSync(path.join(result.libraryPath, '.super', 'library.db'))).toBe(true);
    ...
    const importedDb = new Database(path.join(result.libraryPath, '.super', 'library.db'), { readonly: true });
  ...
  it('rejects ZIP without .super/library.db', async () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    // Remove the .super/library.db entry from the ZIP.
    ...
    const zip = new AdmZip(destZipPath);
    const entries = zip.getEntries();
    const dbEntry = entries.find((e) => e.entryName === '.super/library.db' && !e.isDirectory);
    if (dbEntry) {
      zip.deleteFile('.super/library.db');
    }
  ...
  it('rejects ZIP with path-escape entries', async () => {
    ...
    archive.file(tempFilePath, { name: '.super/library.db' });
  ...
  it.each([
    ...
    zip.addFile('.super/library.db', Buffer.from('db'));
