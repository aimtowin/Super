...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-video-exr-'));
...
it('escapes Windows paths embedded in FFmpeg filtergraphs', () => {
  expect(escapeFfmpegFilterPath(String.raw`C:\Super\fonts\DejaVuSans.ttf`))
    .toBe(String.raw`C\:/Super/fonts/DejaVuSans.ttf`);
  expect(escapeFfmpegFilterPath("/tmp/Super's font.ttf"))
    .toBe("/tmp/Super\\'s font.ttf");
});
...
afterEach(() => {
  ...
  delete process.env['SUPER_FFMPEG_PATH'];
  delete process.env['SUPER_OIIO_PATH'];
...
describe('video (ffprobe + ffmpeg)', () => {
  it('keeps AI contact-sheet preparation independent from poster work (Super-140fe2)', async () => {
    process.env['SUPER_FFMPEG_PATH'] = '/fake/ffmpeg';
    ...
    // Super-140fe2: nothing schedules a contact sheet proactively any more.
  ...
  it('generates a font-independent contact_sheet with fps/scale/tile args', async () => {
    ...
    // Super-140fe2: sheets materialize at analysis time via lazy ensure.
    const ensured = await service.ensureVideoContactSheet(created.libraryId, assetId);
    expect(ensured).toBe(true);
    ...
    // Super-6w40: the AI contact sheet stamps a timestamp (HH:MM:SS.mmm)
  ...
  it('rejects and removes a WebM proxy above the 512 MiB safety limit', async () => {
    ...
    expect(existsSync(path.join(created.libraryPath, '.super', 'artifacts', artifact!.filePath)))
      .toBe(false);
    expect(diagnostics).toContainEqual(expect.objectContaining({
  ...
  it('writes failed artifact when ffmpeg binary is missing (ENOENT)', async () => {
    ...
    process.env['SUPER_FFMPEG_PATH'] = '/fake/ffmpeg-missing';
    const root = temporaryRoot();
    const service = new LibraryService({
  ...
  it('automatically requeues an OIIO component failure after repair', async () => {
    process.env['SUPER_OIIO_PATH'] = '/fake/oiiotool';
    const root = temporaryRoot();
    const failedService = new LibraryService({
      spawnFn: createMockSpawn({
...
describe('EXR/TGA (oiiotool)', () => {
  ...
  it('writes failed artifact when oiiotool binary is missing (ENOENT)', async () => {
    process.env['SUPER_OIIO_PATH'] = '/fake/oiiotool-missing';
    const root = temporaryRoot();
    const diagnostics: LibraryServiceDiagnostic[] = [];
    const service = new LibraryService({
      onDiagnostic: (diagnostic) => diagnostics.push(diagnostic),
...
describe('enqueueThumbnailJobs handles all media types', () => {
  ...
  it('enqueues jobs for audio assets (Super-13v)', () => {
    process.env['SUPER_FFMPEG_PATH'] = '/fake/ffmpeg';
    const root = temporaryRoot();
    const service = new LibraryService({
      spawnFn: createMockSpawn({ ffprobeStdout: CANNED_FFPROBE_JSON }),
  ...
  it('enqueues audio when only the viewer strip exists (Super-051)', async () => {
    process.env['SUPER_FFMPEG_PATH'] = '/fake/ffmpeg';
    const root = temporaryRoot();
    const service = new LibraryService({
      spawnFn: createMockSpawn({ ffprobeStdout: CANNED_FFPROBE_JSON }),
...
describe('audio waveform thumbnail (Super-13v)', () => {
