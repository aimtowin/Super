...
describe('Eagle external-library opening (Super-768x.1)', () => {
  it('converts root files and nested folders into a Super library at the chosen parent', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-open-'));
    roots.push(root);
    const eagleParent = path.join(root, 'eagle-source');
    const destinationParent = path.join(root, 'super-destination');
  ...
  it('returns the Eagle library name for the rename panel and honors a custom display name', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-open-rename-'));
    roots.push(root);
  ...
  it('rejects inspect when the folder is not an Eagle library', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-inspect-invalid-'));
    roots.push(root);
    const service = new LibraryService();
    services.push(service);
    let thrown: unknown;
  ...
  it('rejects a destination inside the Eagle source directory', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-open-inside-'));
    roots.push(root);
    const eagleRoot = path.join(root, 'Reference.library');
    writeEagleFixture(eagleRoot);
  ...
  it('creates a missing destination parent instead of calling it an invalid path', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-open-mkdir-'));
    roots.push(root);
    const eagleRoot = path.join(root, 'Reference.library');
    writeEagleFixture(eagleRoot);
  ...
  it('accepts a trailing separator on the destination parent (Super-sq4i)', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-eagle-open-trail-'));
    roots.push(root);
    const eagleRoot = path.join(root, 'Reference.library');
    const destinationParent = path.join(root, 'super-destination');
    mkdirSync(eagleRoot, { recursive: true });
