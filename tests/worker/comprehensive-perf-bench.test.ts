...
const fixturePath = process.env.SUPER_PERF_BENCH_PATH;
...
const allowFixtureMutation = process.env.SUPER_PERF_BENCH_ALLOW_MUTATION === '1';
...
describe.skipIf(!fixturePath)('comprehensive perf benchmark', () => {
  ...
  it('measures all goal metrics and prints JSON', async () => {
    ...
    if (sampleCollectionId) {
      ...
      results.mutationMetrics = 'skipped (set SUPER_PERF_BENCH_ALLOW_MUTATION=1 on a disposable clone)';
    }
    ...
    const importStaging = fsSync.mkdtempSync(pathMod.join(osSync.tmpdir(), 'super-import-probe-'));
    for (let i = 0; i < 5; i++) {
