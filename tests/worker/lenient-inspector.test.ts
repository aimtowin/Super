// Super-verg.2: inspector-side read paths (tags/collections/metadata/AI
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-lenient-inspector-'));
...
function reopenAfterMutating(
  ...
  mutate(path.join(libraryPath, '.super', 'library.db'));
  service.closeAll();
  service.openLibrary(libraryPath);
}

describe('inspector read paths lenient read (Super-verg.2)', () => {
