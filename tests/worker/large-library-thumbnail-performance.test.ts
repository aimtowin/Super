...
const configuredOffset = Number(process.env.SUPER_LARGE_LIBRARY_PERF_OFFSET ?? 1_100);
...
function removeSelectedThumbnails(assetIds: readonly string[]): number {
  ...
  const database = new TestDatabase(
    path.join(temporaryLibraryPath, ".super", "library.db"),
  );
  ...
  const artifactsDir = path.join(temporaryLibraryPath, ".super", "artifacts");
...
describe.skipIf(!fixturePath)("10k visible-thumbnail generation benchmark", () => {
  beforeAll(() => {
    ...
    temporaryRoot = mkdtempSync(
      path.join(tmpdir(), "super-visible-thumbnail-benchmark-"),
    );
