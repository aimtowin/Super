...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-search-test-'));
...
describe('search filters', () => {
  ...
  it('filters by AI tags as well as human tags (Super-5cvr)', () => {
    const { service, libraryId, assetId } = createLibraryWithAssetAndTags();
  ...
  it('ANDs separate single-value tag clauses (Super-eaxs tag-management AND search)', () => {
    const { service, libraryId, assetId } = createLibraryWithAssetAndTags();
...
describe('pagination', () => {
  ...
  it('returns the full scope ids only when idsOnly is true (Super-ws4k)', () => {
    const { service, libraryId, libraryPath } = createLibraryWithAssetAndTags();
    const secondAssetId = createSecondAsset(service, libraryId, libraryPath, 'Second');
  ...
  it('returns a compact real-asset geometry index when layoutOnly is true (Super-sa65)', () => {
    const { service, libraryId, libraryPath, assetId } = createLibraryWithAssetAndTags();
    const secondAssetId = createSecondAsset(service, libraryId, libraryPath, 'Second');
    const db = new TestDatabase(path.join(libraryPath, '.super', 'library.db'));
    const revisions = db.prepare(
...
describe('smart collections v6', () => {
  it('create allows draft empty; update rejects empty (CU-M5)', () => {
    ...
    // update/save (Super-san / SMART-007).
  ...
  it('executes a smart collection with idsOnly for select-all (Super-ws4k)', () => {
    const { service, libraryId, libraryPath } = createLibraryWithAssetAndTags();
    void createSecondAsset(service, libraryId, libraryPath, 'Second');
    const smart = service.createSmartCollection({
      libraryId,
