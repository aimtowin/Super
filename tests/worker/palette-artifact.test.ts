...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-palette-'));
  roots.push(root);
  return root;
}
...
describe('local extracted palette artifact', () => {
  it('persists deterministic hex ratios and ignores manual palette writes', async () => {
    ...
    // Super-7pg: manual palette writes are ignored; effective stays automatic.
  ...
  it('extracts from a ready video poster without reading the original video', async () => {
    ...
    const artifactsDir = path.join(library.libraryPath, '.super', 'artifacts');
