...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-linked-test-'));
...
describe('Linked folders schema migration', () => {
  it('migrates a v3 library to v4 with a linked_folders table and linked asset columns', () => {
    ...
    const database = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
    try {
...
describe('Collection history snapshots', () => {
  it('keeps a parent collection with descendants valid for rename history', () => {
    const root = temporaryRoot();
    const service = newService();
    const library = service.createLibrary({ displayName: 'Collection history', selectedParentPath: root });
    const parent = service.createCollection({ libraryId: library.libraryId, name: 'Parent' });
    ...
    expect(service.getCollectionHistorySnapshot({
      libraryId: library.libraryId,
      collectionIds: [parent.collectionId],
    }).map((collection) => collection.collectionId).sort()).toEqual([
      child.collectionId,
      parent.collectionId,
    ].sort());
  });
...
describe('Linked folder import', () => {
  it('adds a linked directory to a collection as a non-syncing snapshot', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
    mkdirSync(sourceRoot);
    writeFileSync(path.join(sourceRoot, 'root.png'), 'root');
    mkdirSync(path.join(sourceRoot, 'child'));
    writeFileSync(path.join(sourceRoot, 'child', 'nested.png'), 'nested');

    const service = newService();
    const library = service.createLibrary({ displayName: 'Collection Snapshot', selectedParentPath: root });
    const linked = service.importFolderAsLinked({ libraryId: library.libraryId, sourceRootPath: sourceRoot });
    const collection = service.createCollection({ libraryId: library.libraryId, name: 'Reference' });

    const direct = service.addLinkedFolderAssetsToCollection({
      libraryId: library.libraryId,
    ...
    expect(direct.addedAssetIds).toHaveLength(1);
    expect(service.listCollectionAssets({ libraryId: library.libraryId, collectionId: collection.collectionId, recursive: false })
      .map((asset) => asset.relativeFilePath)).toEqual(['root.png']);

    const recursive = service.addLinkedFolderAssetsToCollection({
      libraryId: library.libraryId,
      collectionId: collection.collectionId,
      linkedFolderId: linked.folderId,
      relativePath: '',
    ...
    expect(recursive.addedAssetIds).toHaveLength(1);
    expect(service.listCollectionAssets({ libraryId: library.libraryId, collectionId: collection.collectionId, recursive: false })
      .map((asset) => asset.relativeFilePath).sort()).toEqual(['child/nested.png', 'root.png']);

    writeFileSync(path.join(sourceRoot, 'later.png'), 'later');
    service.refreshManagedAssets(library.libraryId);
    expect(service.listAssets({ libraryId: library.libraryId, folderId: linked.folderId, recursive: true })
      .map((asset) => asset.relativeFilePath).sort()).toEqual(['child/nested.png', 'later.png', 'root.png']);
    expect(service.listCollectionAssets({ libraryId: library.libraryId, collectionId: collection.collectionId, recursive: false })
      .map((asset) => asset.relativeFilePath).sort()).toEqual(['child/nested.png', 'root.png']);
  });

  it('derives collection source categories only from linked member paths', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
    mkdirSync(sourceRoot);
    mkdirSync(path.join(sourceRoot, 'keep'));
    mkdirSync(path.join(sourceRoot, 'keep', 'deep'));
    mkdirSync(path.join(sourceRoot, 'skip'));
    writeFileSync(path.join(sourceRoot, 'keep', 'one.png'), 'one');
    writeFileSync(path.join(sourceRoot, 'keep', 'deep', 'two.png'), 'two');
    writeFileSync(path.join(sourceRoot, 'skip', 'outside.png'), 'outside');

    const service = newService();
    const library = service.createLibrary({ displayName: 'Collection Categories', selectedParentPath: root });
    const linked = service.importFolderAsLinked({ libraryId: library.libraryId, sourceRootPath: sourceRoot });
    const collection = service.createCollection({ libraryId: library.libraryId, name: 'Selected only' });
    service.addLinkedFolderAssetsToCollection({
      libraryId: library.libraryId,
      collectionId: collection.collectionId,
      linkedFolderId: linked.folderId,
      relativePath: 'keep',
    ...
    const rootCards = service.listCollectionSourceBrowseEntries({
      libraryId: library.libraryId,
      collectionId: collection.collectionId,
      recursive: false,
      parentFolderId: null,
    });
    expect(rootCards.map((entry) => entry.relativePath)).toEqual(['keep']);
    expect(rootCards[0]).toMatchObject({ recursiveAssetCount: 2, childFolderCount: 1 });

    const keepFolderId = rootCards[0]!.folderId;
    const nestedCards = service.listCollectionSourceBrowseEntries({
      libraryId: library.libraryId,
      collectionId: collection.collectionId,
      recursive: false,
      parentFolderId: keepFolderId,
    });
    expect(nestedCards.map((entry) => entry.relativePath)).toEqual(['keep/deep']);

    const scoped = service.searchAssets({
      libraryId: library.libraryId,
      scope: {
        kind: 'collection',
        collectionId: collection.collectionId,
        recursive: false,
        sourceFolderId: keepFolderId,
      },
      limit: 50,
      offset: 0,
    });
    expect(scoped.items.map((asset) => asset.relativeFilePath).sort()).toEqual([
      'keep/deep/two.png',
      'keep/one.png',
    ]);
  });
  ...
  it('Super-c643c2: linked directories support create, rename, nested import and copy', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
  ...
  it('Super-l1oi: header-probed image dimensions are available before thumbnails', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
    mkdirSync(sourceRoot);
    writeFileSync(
  ...
  it('refresh creates an external-change revision when a linked asset is overwritten externally', () => {
    ...
    // External overwrite of the linked source file (not via Super).
  ...
  it('D1 re-verify: refresh still registers new external files when all rules are disabled (re-review misread)', () => {
    ...
    // Add a new file externally (not through Super).
  ...
  it('TEXT-001: refresh discovers a new .txt under a linked subdirectory (Super-4l7)', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
    mkdirSync(sourceRoot);
    mkdirSync(path.join(sourceRoot, 'notes'));
  ...
  it('imports external files into an existing linked folder root (Super-d3h)', () => {
    const root = temporaryRoot();
  ...
  it('Super-linked-ignore: shows an ignored linked root only when requested', () => {
    const root = temporaryRoot();
    const sourceRoot = path.join(root, 'source');
    mkdirSync(sourceRoot);
    writeFileSync(path.join(sourceRoot, 'hidden.png'), 'hidden');

    const service = newService();
    const library = service.createLibrary({
      displayName: 'Ignored linked root',
      selectedParentPath: root,
    });
    const linked = service.importFolderAsLinked({
      libraryId: library.libraryId,
    ...
    expect(service.listLinkedFolders(library.libraryId)).toEqual([]);
    expect(service.listLinkedFolders(library.libraryId, true)).toMatchObject([
      { folderId: linked.folderId, displayName: 'source', relativePath: '' },
    ]);
    expect(service.listAssets({ libraryId: library.libraryId, recursive: true })).toEqual([]);
    expect(service.listAssets({ libraryId: library.libraryId, recursive: true, showIgnored: true }))
      .toMatchObject([{ relativeFilePath: 'hidden.png' }]);
    expect(service.searchAssets({ libraryId: library.libraryId, limit: 50, offset: 0 }).items)
      .toEqual([]);
    expect(service.searchAssets({
      ...
      offset: 0,
      showIgnored: true,
    }).items).toMatchObject([{ relativeFilePath: 'hidden.png' }]);
  });
});
