// Super-5xbg: failed derived artifacts (thumbnail/video_poster/contact_sheet/
...
// Video webm_proxy is intentionally excluded: Super-cljb only creates it
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-derivative-repair-'));
...
function insertFailedAsset(
  ...
  if (input.kind === 'contact_sheet') {
    ...
    if (input.available !== false) {
      writeFileSync(path.join(libraryPath, '.super', 'artifacts', posterRelativePath), 'poster');
    }
    insertArtifact.run(
    ...
    if (input.available !== false) {
      writeFileSync(
        path.join(libraryPath, '.super', 'artifacts', metadataRelativePath),
  ...
  if (input.kind === 'webm_proxy') {
    ...
    if (input.available !== false) {
      writeFileSync(path.join(libraryPath, '.super', 'artifacts', thumbRelativePath), 'png');
    }
    insertArtifact.run(
...
describe('retryable failed derived artifacts (Super-5xbg)', () => {
  ...
  it('keeps failed contact sheets terminal — AI analysis regenerates on demand (Super-140fe2)', () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    // Super-140fe2: contact sheets are generated lazily at AI-analysis time
