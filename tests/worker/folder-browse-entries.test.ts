...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-folder-browse-'));
...
describe('folder browse entries', () => {
  it('lists direct child folders with direct asset counts and cover artifact ids', () => {
    ...
    expect(atRoot[0]).toMatchObject({
      ...
      name: 'Parent',
      directAssetCount: 0,
      // Super-toh: parent badge includes assets under ChildA.
    ...
    // Super-d0nv: the cover candidate asset id rides alongside the artifact
  ...
  it('caps cover candidates at three per folder and maps them to asset ids (Super-d0nv)', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
  ...
  it('returns cover candidates without ready thumbnails and the cover scene enqueues them (Super-d0nv)', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
