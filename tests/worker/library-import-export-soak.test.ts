...
function appendPerfLog(line: string): void {
  ...
  writeFileSync(path.join(tmpdir(), 'super-import-perf.log'), line, { flag: 'a' });
}
...
const ASSET_COUNT = Number(process.env.SUPER_SOAK_ASSET_COUNT ?? 20_000);
...
function seedAssetsAndFiles(libraryPath: string, folderName: string, folderId: string): void {
  ...
  const dbPath = path.join(libraryPath, '.super', 'library.db');
  ...
  // exercises the API path including file move to .super/trash, the
...
beforeAll(() => {
  const root = mkdtempSync(path.join(tmpdir(), 'super-soak-export-import-'));
  ...
  // SQL).  This exercises the full trash path: file move to .super/trash,
  ...
  // Verify .super/trash directory exists after trash API call.
  const trashDir = path.join(library.libraryPath, '.super', 'trash');
  expect(existsSync(trashDir), '.super/trash must exist after API trash').toBe(true);
  const trashContents = readdirSync(trashDir);
  expect(
    trashContents.length,
    '.super/trash must contain at least 1 trashed file',
  ).toBeGreaterThan(0);
...
function verifyRoundTripIntegrity(
  ...
  //    tag/collection links via direct SQLite on the imported library.
  {
    ...
    // 7a0. Physical .super/trash directory verification.
    const trashDir = path.join(importedPath, '.super', 'trash');
    expect(
      existsSync(trashDir),
      `[${label}] .super/trash must exist after round-trip`,
    ).toBe(true);
    const trashFiles = readdirSync(trashDir);
    expect(
      trashFiles.length,
      `[${label}] .super/trash must contain trashed files`,
    ).toBeGreaterThan(0);
    const dbPath = path.join(importedPath, '.super', 'library.db');
    const db = new TestDatabase(dbPath);
