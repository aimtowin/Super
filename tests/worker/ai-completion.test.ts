...
function temporaryRoot(): string {
  const root = path.join(
    os.tmpdir(),
    `super-ai-cmpl-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
describe('clearAiContent', () => {
  ...
  it('clears AI content for a single asset (kind=asset)', () => {
    ...
    // Super-c9r3: the cleared IDs ride out so the renderer can refresh the
  ...
  it('clears only the requested AI field and keeps other AI layers (Super-u7hz)', () => {
    const { service, libraryId, assetId } = setupLibraryWithAiContent();
...
describe('AI job queue management', () => {
  function setupWithJob(
    ...
    const db = new Database(
      path.join(created.libraryPath, '.super', 'library.db'),
    );
  ...
  it('returns exact counts for explicitly tracked job IDs, excluding history', () => {
    ...
    const status = service.getAiJobStatus(libraryId);
    const db = new Database(
      path.join(service.listLibraries()[0]!.libraryPath, '.super', 'library.db'),
    );
  ...
  it('persists redacted error_detail on permanent failure (Super-iokf)', () => {
    const { service, libraryId, jobId } = setupWithJob('queued');
