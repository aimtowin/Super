...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-trash-relink-test-'));
...
describe('downgrade helpers still work with v9', () => {
  it('downgrade to v1 then re-migrate through v9', () => {
    ...
    const dbPath = path.join(created.libraryPath, '.super', 'library.db');
...
describe('trashAssets (soft delete)', () => {
  ...
  it('moves managed asset to .super/trash/ and sets deleted_at', () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    expect(existsSync(path.join(created.libraryPath, '.super', 'trash', assetId, 'photo.jpg'))).toBe(true);
  ...
  it('moves already-missing managed assets to trash without a source file', () => {
    ...
    expect(trashedCount).toBe(1);
    expect(existsSync(path.join(created.libraryPath, '.super', 'trash', assetId, 'gone.jpg'))).toBe(false);
...
describe('trash preview artifacts (BUG-TRASH-001)', () => {
  ...
  it('keeps the thumbnail resolvable and decodable after trashing', async () => {
    ...
    db.close();
    expect(existsSync(path.join(created.libraryPath, '.super', 'artifacts', `${artifactId}.webp`))).toBe(true);
  ...
  it('stops serving artifacts once the asset is permanently deleted', async () => {
    ...
    // file may still sit in .super/artifacts awaiting regeneration sweeps.
    expectServiceError(
...
describe('restoreAssets', () => {
  ...
  it('previewRestoreAssets reports no conflicts when the original path is free (Super-0hnx)', () => {
    const root = temporaryRoot();
    const service = newService();
  ...
  it('previewRestoreAssets reports conflicts when the original path is occupied (Super-0hnx)', () => {
    const root = temporaryRoot();
    const service = newService();
  ...
  it('readTextAsset reads trashed text from the trash store (Super-hv6n)', () => {
    const root = temporaryRoot();
    const service = newService();
  ...
  it('reuses text previews by revision and invalidates them after save (Super-29125f)', () => {
    const root = temporaryRoot();
    const service = newService();
  ...
  it.each([
    ...
    const database = new TestDatabase(path.join(reopened.libraryPath, '.super', 'library.db'));
...
describe('deleteAssetsPermanent', () => {
  it('removes trash file and DB row with cascade', () => {
    ...
    expect(existsSync(path.join(created.libraryPath, '.super', 'trash', assetId))).toBe(false);
    expect(service.listTrash(created.libraryId)).toEqual([]);
  ...
  it('skips already-deleted trash directory (ENOENT ok)', () => {
    ...
    service.trashAssets({ libraryId: created.libraryId, assetIds: [assetId] });
    rmSync(path.join(created.libraryPath, '.super', 'trash', assetId), { recursive: true });
  ...
  it('rejects a mixed or duplicate batch before deleting any trash entry', () => {
    ...
    const trashPath = path.join(created.libraryPath, '.super', 'trash', trashed.assetId);
...
describe('emptyTrash', () => {
  ...
  it('keeps folder tombstones when some assets fail to purge (Super-b3kf)', () => {
    const root = temporaryRoot();
    let busyAssetId = '';
    const service = newService({
      removeTrashPath: (trashPath) => {
...
describe('deleteLinkedAssets', () => {
  ...
  it('trashes every successful source before deleting their records in one transaction', async () => {
    ...
    const recoveryDatabase = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
    ...
    const auditedDatabase = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('does not infer an in-flight source was trashed while its linked root is offline', () => {
    ...
    let auditedDatabase = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
    ...
    auditedDatabase = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  (process.env.SUPER_TEST_REAL_SYSTEM_TRASH === '1' ? it : it.skip)(
    ...
    async () => {
      ...
      const sourcePath = path.join(linkedRoot, `super-trash-${randomUUID()}.txt`);
      mkdirSync(linkedRoot);
...
describe('relinkBatchApply', () => {
  ...
  it('keepMetadata=true preserves human and AI metadata, tags, and collections', () => {
    ...
    const keepDb = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('keepMetadata=false clears human and AI metadata, tags, and collections', () => {
    ...
    const clearDb = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
