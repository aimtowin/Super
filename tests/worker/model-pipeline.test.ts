...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-model-'));
...
describe('model asset pipeline (slice A, Super-fu2i)', () => {
  ...
  it('resolves a model preview to the original source URL contract', async () => {
    ...
    // super://source/<libraryId>/<assetId>?revision=... (same route as
  ...
  // Slice E migration (Super-hnmg): model extensions joined the thumbnail
  ...
  describe('offscreen model thumbnail queue (slice E, Super-hnmg)', () => {
  ...
  describe('model AI four-view sheet (Super-6w40)', () => {
