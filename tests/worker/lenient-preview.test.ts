// Super-verg.2: preview resolution (getPreviewArtifact) must keep working
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-lenient-preview-'));
...
describe('getPreviewArtifact lenient read (Super-verg.2)', () => {
  ...
  it('resolves preview without failing when the artifact table is missing', () => {
    ...
    dropTable(path.join(libraryPath, '.super', 'library.db'), 'revision_artifacts');
    service.closeAll();
  ...
  it('degrades when artifact status columns are missing', () => {
    ...
    dropColumns(path.join(libraryPath, '.super', 'library.db'), 'revision_artifacts', [
      'status',
      'error_code',
    ]);
    service.closeAll();
