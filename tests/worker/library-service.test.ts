...
function startMigrationOpener(
  ...
  const child = spawn(process.execPath, [openerPath], {
    env: {
      ...
      SUPER_MIGRATION_CHILD_ID: childId,
      SUPER_MIGRATION_LIBRARY_PATH: libraryPath,
      SUPER_MIGRATION_RELEASE_PATH: releasePath,
    },
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-library-test-'));
...
describe('LibraryService lifecycle', () => {
  it('creates a self-contained library and exposes it exactly once', () => {
    ...
    expect(statSync(path.join(created.libraryPath, '.super', 'library.db')).isFile()).toBe(
      true,
    );
    expect(statSync(path.join(created.libraryPath, '.super', 'previews')).isDirectory()).toBe(
      true,
    );
  ...
  it('holds SQLite’s real writer mutex for the entire bounded write callback', async () => {
    ...
    const databasePath = path.join(created.libraryPath, '.super', 'library.db');
  ...
  it('translates writer-mutex contention after lease acquisition to LIBRARY_BUSY', async () => {
    ...
    const databaseFilename = path.join(created.libraryPath, '.super', 'library.db');
  ...
  it('serializes the v23-to-v25 coordination migrations before verifying schema history', () => {
    ...
    const verification = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('recreates regenerable directories when reopening a closed library', () => {
    ...
    service.closeLibrary(created.libraryId);
    rmSync(path.join(created.libraryPath, '.super', 'previews'), { recursive: true });
    rmSync(path.join(created.libraryPath, '.super', 'trash'), { recursive: true });
    ...
    expect(statSync(path.join(created.libraryPath, '.super', 'trash')).isDirectory()).toBe(
      true,
    );
    service.closeAll();
  });
  ...
  it('rescues a missing database from the Assets tree', () => {
    ...
    service.closeAll();
    rmSync(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('opens a database created by a newer schema version writable (Super-033e)', () => {
    const root = temporaryRoot();
    const service = newService();
  ...
  it('rolls back a failed v2 migration and preserves the v1 version', () => {
    ...
    // Super-verg.5 (0031 §2.2): a rolled-back migration failure is now a
  ...
  it('does not commit a table-rebuild migration when foreign keys are corrupt', () => {
    ...
    const before = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
    ...
    // Super-verg.5: a rolled-back table-rebuild failure is retryable.
    expectServiceError(() => service.openLibrary(created.libraryPath), 'LIBRARY_MIGRATION_FAILED');

    const after = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('rescues a corrupt database without losing the library root', () => {
    ...
    service.closeAll();
    writeFileSync(path.join(created.libraryPath, '.super', 'library.db'), 'not a sqlite database');
    ...
    expect(readdirSync(path.join(created.libraryPath, '.super', 'corrupt-backup')))
  ...
  it('recreates regenerable directories after a database rescue', () => {
    ...
    service.closeAll();
    const previewsPath = path.join(created.libraryPath, '.super', 'previews');
    rmSync(previewsPath, { recursive: true });
    writeFileSync(path.join(created.libraryPath, '.super', 'library.db'), 'not sqlite');
  ...
  it('deletes an open library root from disk and leaves linked sources (Super-9i8)', () => {
    const root = temporaryRoot();
    const service = newService();
    const created = service.createLibrary({
