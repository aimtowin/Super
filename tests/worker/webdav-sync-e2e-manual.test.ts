/**
 * 手动双设备同步端到端（Super-xffq）。
 *
 * 运行（URL 不写进本文件）：
 *   $env:SUPER_WEBDAV_PROBE_URL='https://<host>/Share/Super/'; `
 *   $env:SUPER_WEBDAV_PROBE_USER='dev'; $env:SUPER_WEBDAV_PROBE_PASS='...'; `
...
const probeUrl = process.env.SUPER_WEBDAV_PROBE_URL;
const roots: string[] = [];
...
function rootConfig(): SyncRootConfig {
  return {
    id: 'manual-e2e',
    baseUrl: probeUrl!,
    username: process.env.SUPER_WEBDAV_PROBE_USER,
    password: process.env.SUPER_WEBDAV_PROBE_PASS,
...
describe.skipIf(!probeUrl)('manual two-device WebDAV sync end-to-end', () => {
  it('round-trips a library between two devices and cleans up', async () => {
    ...
    // ── 设备 A：建库 + 导入两个文件 → 全量上传 ──
    const rootA = mkdtempSync(path.join(tmpdir(), 'super-sync-devA-'));
    roots.push(rootA);
    ...
    const rootB = mkdtempSync(path.join(tmpdir(), 'super-sync-devB-'));
    roots.push(rootB);
    ...
    const driver = new WebDAVDriver({ baseUrl: probeUrl!, username: process.env.SUPER_WEBDAV_PROBE_USER, password: process.env.SUPER_WEBDAV_PROBE_PASS, allowInsecureTls: true });
    await driver.delete(`${directory}/manifest.json`).catch(() => undefined);
    await driver.delete(`${directory}/.super-sync/format-version`).catch(() => undefined);
    ...
    await driver.delete(`${directory}/.super-sync/`).catch(() => undefined);
