...
const ffmpegPath = process.env['SUPER_REAL_FFMPEG_PATH']
  ?? installedOrBuild('ffmpeg/darwin-arm64/ffmpeg');
const ffprobePath = process.env['SUPER_REAL_FFPROBE_PATH']
  ?? installedOrBuild('ffmpeg/darwin-arm64/ffprobe');
const oiiotoolPath = process.env['SUPER_REAL_OIIO_PATH']
...
const requireRealMedia = process.env['SUPER_REQUIRE_REAL_MEDIA'] === '1';
...
function assertRealMediaBundleAvailable(): void {
  ...
  throw new Error(
    'SUPER_REQUIRE_REAL_MEDIA=1 requires the darwin-arm64 LGPL media bundle. '
...
afterAll(() => {
  ...
  delete process.env['SUPER_FFPROBE_PATH'];
...
describe.runIf(hasRealBundle || requireRealMedia)('installed media bundle real-format smoke', () => {
  it('processes the MVP formats through the persistent media queue into playable derivatives', async () => {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'super-real-media-'));
    temporaryRoots.push(root);
    const sourceRoot = path.join(root, 'sources');
    mkdirSync(sourceRoot);
    const sourcePaths = buildFixtureSet(sourceRoot);
    process.env['SUPER_FFMPEG_PATH'] = ffmpegPath;
    process.env['SUPER_FFPROBE_PATH'] = ffprobePath;
    process.env['SUPER_OIIO_PATH'] = oiiotoolPath;
    ...
    // Super-cljb: importing/browsing never eagerly creates video proxies,
