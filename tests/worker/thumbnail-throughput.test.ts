...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-thumb-throughput-'));
 ...
 * exactly what the Super-xoaz batching assertions target.
 */
...
describe('thumbnail queue DB-write batching (Super-xoaz)', () => {
...
describe('thumbnail fill order (Super-xoaz)', () => {
  ...
  it('keeps the caller id order for explicit waves regardless of created_at (Super-x9xu follow-up)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService({ sharpFn: instantSharp() });
