...
function openLibraryDb<T>(libraryPath: string, fn: (db: TestDbHandle) => T): T {
  ...
  const db = new Database(path.join(libraryPath, '.super', 'library.db'));
  try {
    return fn(db);
  } finally {
    db.close();
...
/**
 * Super-140fe2 regression: Eagle/Billfish conversions register videos with
...
describe('contact sheet self-heal for conversion-registered videos', () => {
  it('probes missing duration on demand and generates the contact sheet', async () => {
    ...
    const ffmpeg = process.env['SUPER_FFMPEG_PATH'] ?? 'ffmpeg';
    const root = mkdtempSync(path.join(tmpdir(), 'super-cs-selfheal-'));
    roots.push(root);
    ...
    const metadataAbs = openLibraryDb(created.libraryPath, (db) => {
      ...
      return path.join(created.libraryPath, '.super', 'artifacts', metadataRow!.file_path!);
    });
    ...
    const healed = JSON.parse(readFileSync(
      path.join(created.libraryPath, '.super', 'artifacts', healedRow.file_path!),
      'utf-8',
  ...
  it('writes a terminal failed artifact when ffprobe fails after registration', async () => {
    if (!ffmpegAvailable()) return;
    const ffmpeg = process.env['SUPER_FFMPEG_PATH'] ?? 'ffmpeg';
    const root = mkdtempSync(path.join(tmpdir(), 'super-cs-doomed-'));
    roots.push(root);
    ...
    openLibraryDb(created.libraryPath, (db) => {
      ...
      writeFileSync(
        path.join(created.libraryPath, '.super', 'artifacts', metadataRow!.file_path!),
  ...
  it('enqueues AI analysis for sheet-less videos and lazily self-heals duration (hermetic)', async () => {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'super-cs-hermetic-'));
    roots.push(root);
    ...
    // Super-140fe2 blocker fix: analysis enqueue must NOT gate on a
