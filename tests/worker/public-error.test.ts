...
describe('Library Worker public error boundary', () => {
  ...
  it('exposes a lease conflict as a retryable library-busy result without filesystem details', () => {
    expect(publicErrorForWorkerFailure(
      new LibraryWriteCoordinatorError('Another process owns /private/Library/.super/library.db', 'timed-out'),
    )).toEqual({
      code: 'LIBRARY_BUSY',
  ...
  it('maps sync connection failures to a readable reason (Super-xffq)', () => {
    expect(publicErrorForWorkerFailure(
