...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-export-import-test-'));
...
describe('LibraryService export', () => {
  it('exports a library with Assets, revisions, trash, and library.db', async () => {
    ...
    expect(existsSync(path.join(destPath, '.super', 'library.db'))).toBe(true);
  ...
  it('excludes previews and operations directories', async () => {
    ...
    expect(existsSync(path.join(destPath, '.super', 'previews'))).toBe(false);
    expect(existsSync(path.join(destPath, '.super', 'operations'))).toBe(false);
  ...
  it('includes .super/artifacts so import keeps ready thumbnails (Super-pxd)', async () => {
    const root = temporaryRoot();
    const service = newService();
    ...
    expect(
      existsSync(path.join(destPath, '.super', 'artifacts', `${thumb.artifactId}.webp`)),
    ).toBe(true);
    service.closeAll();
    ...
    expect(
      existsSync(
        path.join(reopened.libraryPath, '.super', 'artifacts', `${thumb.artifactId}.webp`),
      ),
    ).toBe(true);
  ...
  it('invalidates ready artifacts missing on disk so thumbnails requeue (Super-pxd)', async () => {
    const root = temporaryRoot();
    const service = newService();
    const created = service.createLibrary({
    ...
    // Super-tumv: re-open delivers immediately; the missing-artifact sweep
  ...
  it.runIf(process.platform === 'win32')(
    'accepts a folder destination on a different drive letter (Super-59f)',
    async (ctx) => {
...
describe('LibraryService import folder', () => {
  ...
  it('imports a library by copying to a new location', async () => {
    ...
    expect(existsSync(path.join(expectedPath, '.super', 'library.db'))).toBe(true);
  ...
  it('removes its newly-created copied-library directory after a copy failure', async () => {
    ...
    service.closeAll();
    databaseToRemove = path.join(created.libraryPath, '.super', 'library.db');
    const copyParent = path.join(root, 'copy-parent');
