...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-org-test-'));
...
describe('tag assignment', () => {
  ...
  it('removeTags also clears AI-authored tag links (Super-h2i2)', () => {
    const { service, libraryId, libraryPath, assetId } = createLibraryWithAsset();
    ...
    const after = new TestDatabase(path.join(libraryPath, '.super', 'library.db'));
    try {
      const count = (
        after
          .prepare(
...
describe('asset metadata', () => {
  ...
  it('ignores palette writes and leaves stored palette unchanged', () => {
    ...
    // Super-7pg: any palette payload is a no-op for the column / effective view.
...
describe('smart collections', () => {
  it('create allows draft empty/sort-only; update rejects them (CU-M5)', () => {
    ...
    // Sidebar inline-create (Super-san / SMART-007) intentionally allows a
