...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-database-recovery-'));
  roots.push(root);
  return root;
}
...
describe('database damage recovery (Super-dw9a)', () => {
  it('keeps at most two verified rotating backup slots', async () => {
    ...
    const backupDirectory = path.join(library.libraryPath, '.super', 'backups');
  ...
  it('restores a damaged primary from backup 1 and preserves the damaged file', async () => {
    ...
    expect(readdirSync(path.join(library.libraryPath, '.super', 'corrupt-backup'))
