...
const fixtureFfmpegPath = process.env['SUPER_REAL_FIXTURE_FFMPEG_PATH'] ?? ffmpegPath;
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-real-common-av-'));
    ...
    try {
      ...
      for (const asset of assets) {
        ...
        if (asset.mediaType === 'video') {
          ...
          // Super-c8a1a3: 无 proxy 时 hover 保持 source——浏览器可解码的
      ...
      // Super-c8a1a3: 代理生成后 hover 意图返回 proxy（无法解码容器的
