...
const rawFixtureDirectory = process.env['SUPER_REAL_RAW_FIXTURE_DIR'];
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-real-raw-formats-'));
...
function rawFixturePaths(): string[] {
  ...
  expect(missing, 'SUPER_REAL_RAW_FIXTURE_DIR must contain one real file for every MVP RAW extension')
    .toEqual([]);
