// Super-verg.2: the browse main query (listAssets) must keep working on an
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-lenient-listassets-'));
...
function libraryWithOneAsset(root: string, displayName: string): {
  ...
  return {
    ...
    dbPath: path.join(library.libraryPath, '.super', 'library.db'),
  };
}
...
function reopenAfterDroppingColumns(
  ...
  dropColumns(path.join(libraryPath, '.super', 'library.db'), drops);
  service.closeAll();
...
describe('listAssets lenient read (Super-verg.2)', () => {
