...
const outputPath = process.env.SUPER_LARGE_LIBRARY_OUTPUT;
const assetCount = Number(process.env.SUPER_LARGE_LIBRARY_ASSETS ?? LARGE_LIBRARY_ASSET_COUNT);
const seed = Number(process.env.SUPER_LARGE_LIBRARY_SEED ?? 20260816);
const reset = process.env.SUPER_LARGE_LIBRARY_RESET === '1';
const assetProfile = process.env.SUPER_LARGE_LIBRARY_ASSET_PROFILE === 'images-only'
  ? 'images-only'
  : 'mixed';
...
describe.skipIf(!outputPath)('large-library fixture generator', () => {
  it('creates a deterministic reusable large library with the selected asset profile', async () => {
    ...
    expect(JSON.parse(readFileSync(
      `${manifest.libraryPath}/.super/large-library-fixture.json`,
      'utf8',
