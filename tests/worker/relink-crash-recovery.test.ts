...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-relink-crash-'));
...
describe('relinkAsset recovery on reopen', () => {
  it('preserves orphan placement after failpoint and cleans up on reopen', () => {
    ...
    const opsDir = path.join(created.libraryPath, '.super', 'operations');
  ...
  it('preserves the destination in the rename-to-placed-marker crash window', () => {
    ...
    const operationsPath = path.join(fixture.libraryPath, '.super', 'operations');
