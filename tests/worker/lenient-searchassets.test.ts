// Super-verg.2: the search main query (searchAssets) must keep working on
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-lenient-search-'));
...
function reopenAfterMutating(
  ...
  mutateSchema(path.join(libraryPath, '.super', 'library.db'), drops, dropTables);
  service.closeAll();
  service.openLibrary(libraryPath);
}

describe('searchAssets lenient read (Super-verg.2)', () => {
