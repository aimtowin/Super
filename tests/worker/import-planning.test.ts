...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-import-plan-test-'));
...
describe('pending import plans', () => {
  ...
  it('creates a readonly automation import plan and rejects a changed source before staging', () => {
    ...
    expect(existsSync(path.join(library.libraryPath, '.super', 'operations'))).toBe(false);
  ...
  it('enumerates a folder hierarchy without exposing source paths', () => {
    ...
    expect(existsSync(path.join(library.libraryPath, '.super', 'operations', plan.importId, 'stage'))).toBe(true);
    service.closeAll();
    expect(existsSync(path.join(library.libraryPath, '.super', 'operations'))).toBe(false);
  });
  ...
  it('summarizes conflicts using safe display names only', () => {
    ...
    // (IMPORT-007 / Super-12ae). Content-duplicate is only for free names.
  ...
  it('surfaces the colliding asset name + thumbnail in name-conflict examples (Super-793k)', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    const library = service.createLibrary({ displayName: 'Library', selectedParentPath: root });
    ...
    // thumbnail artifact (Super-793k: name-conflict examples must carry the
  ...
  it('detects content duplicates for 3D files under a different name (Super-vqg9)', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    const library = service.createLibrary({ displayName: 'Library', selectedParentPath: root });
  ...
  it('carries the existing asset thumbnail in name-conflict examples when ready (Super-793k)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    const library = service.createLibrary({ displayName: 'Library', selectedParentPath: root });
  ...
  it('detects library-wide suspected duplicates across folders via byteSize and SHA-256', () => {
    ...
    // Super-1syi: cross-folder content match is library-scoped (not path collision).
    expect(plan).toMatchObject({
    ...
    // Super-hy1n: create-copy keeps free destination basename for library scope.
    const completion = service.resolveImport({
      importId: plan.importId,
  ...
  it('keeps a committed import successful when operation cleanup is deferred', () => {
    ...
    const operationPath = path.join(library.libraryPath, '.super', 'operations', plan.importId);
  ...
  it.each<ImportFailurePoint>(['crash-after-backup', 'crash-after-place'])(
    ...
    (failAt) => {
      ...
      expect(existsSync(path.join(opened.libraryPath, '.super', 'operations', plan.importId))).toBe(true);
      crashing.closeAll();
      ...
      expect(existsSync(path.join(recoveredLibrary.libraryPath, '.super', 'operations'))).toBe(false);
...
describe('managed asset refresh', () => {
  it('tracks overwrite, missing, reappearance, and stat-only revisions with a stable asset id', () => {
    ...
    // Super-1tio: a pure mtime touch with identical content (portable
...
describe('explicit path boundaries (Super-8b5b.3)', () => {
  ...
  // Super-8b5b.3: Windows-only evidence — drive roots and UNC roots must be
...
describe('Windows path hardening (Super-8b5b.7 review)', () => {
