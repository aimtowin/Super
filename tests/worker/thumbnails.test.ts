...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-thumbnail-'));
...
describe('schema v9 migration', () => {
  ...
  it('creates .super/artifacts directory', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    const created = service.createLibrary({ displayName: 'Artifacts', selectedParentPath: root });

    expect(existsSync(path.join(created.libraryPath, '.super', 'artifacts'))).toBe(true);
...
describe('generateThumbnail (sharp)', () => {
  ...
  it('generates a JPEG thumbnail for an opaque PNG asset (Super-thumb-perf)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    ...
    const artifactPath = path.join(created.libraryPath, '.super', 'artifacts', `${result.artifactId}.jpg`);
  ...
  it('keeps WebP for an alpha PNG so transparency survives (Super-thumb-perf)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    ...
    const webpPath = path.join(created.libraryPath, '.super', 'artifacts', `${result.artifactId}.webp`);
  ...
  it('generates a JPEG thumbnail for a JPEG asset (Super-thumb-perf)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService();
    ...
    const result = (await service.generateThumbnail({ libraryId: created.libraryId, assetId: assets[0]!.assetId }))!;
    expect(result.artifactId).toBeTruthy();
    expect(existsSync(path.join(created.libraryPath, '.super', 'artifacts', `${result.artifactId}.jpg`))).toBe(true);
...
describe('getThumbnailArtifact', () => {
  ...
  it('returns a ready video poster as the card image artifact for native drag previews', () => {
    ...
    const asset = service.listAssets({ libraryId: created.libraryId, recursive: true })[0]!;
    const db = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
    const posterPath = path.join(created.libraryPath, '.super', 'artifacts', 'drag-poster.jpg');
...
describe('preview availability while derivatives are generated', () => {
  ...
  it('keeps a generating video proxy untouched while mounting the source first', () => {
    ...
    // Super-cljb: even an existing in-flight proxy must not steal playback
...
describe('enqueueThumbnailJobs', () => {
  ...
  it('limits startup work, skips unsupported assets, and prioritizes an explicit visible range', () => {
    ...
    // Super-xoaz: the background fill drains most-recently-imported assets
    ...
    const db2 = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
  ...
  it('defers the startup thumbnail enqueue out of library.open but requeues on demand', () => {
    ...
    // Super-4bdd26: the synchronous open no longer enqueues the startup
    ...
    const db = new TestDatabase(path.join(reopened.libraryPath, '.super', 'library.db'));
...
describe('processThumbnailQueue', () => {
  ...
  it('keeps decoder paths in diagnostics and out of Renderer-visible job details', async () => {
    ...
    expect(failed.errorDetail).toContain('local Super log');
...
describe('getArtifactAbsolutePath', () => {
  it('resolves artifact path', async () => {
    ...
    expect(absPath).toContain('.super');
...
describe('generateThumbnail (animated GIF still page)', () => {
  it('avoids pure-black intro frames for multi-page GIFs', async () => {
    ...
    const ffmpeg = process.env['SUPER_FFMPEG_PATH'] ?? 'ffmpeg';
    try {
    ...
    // Super-thumb-perf: GIF stills encode as webp when the palette reports
    // alpha, jpeg otherwise — assert through the stored file path instead of
    // hardcoding an extension.
    const dbPathForCheck = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
...
describe('animated GIF native playback (Super-43d32f)', () => {
  ...
  it('keeps animated GIFs on the native image path — no proxy job, source playback', async () => {
    ...
    // original source instead of a WebM transcode (Super-43d32f).
  ...
  it('retires queued webm proxy jobs for GIFs left by older builds on reopen', async () => {
    ...
    // Super-4bdd26: the retirement moved into the background reconciliation
    ...
    const verifyDb = new TestDatabase(path.join(created.libraryPath, '.super', 'library.db'));
...
describe('visible-wave priority boost (Super-azf6)', () => {
...
describe('cover-wave priority (Super-d0nv)', () => {
...
describe('visible page window covers the whole browse page (Super-x9xu)', () => {
...
describe('visible-window header probe (Super-visible-window)', () => {
  ...
  it('generates a first-page thumbnail for a PDF asset (Super-8ca259)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService();
  ...
  it('stores an offscreen-captured HTML thumbnail as a JPEG artifact (Super-8ca259)', async () => {
    const root = temporaryRoot();
    const service = new LibraryService({
      documentThumbnailRenderer: async ({ url, signal }) => {
        expect(url).toContain('super://source/');
  ...
  it('enqueues generate_thumbnail jobs for PDF and HTML document assets (Super-8ca259)', () => {
  ...
  it('serves PDF and HTML sources with their real MIME for the viewer (Super-8ca259)', () => {
    const root = temporaryRoot();
    const service = new LibraryService();
