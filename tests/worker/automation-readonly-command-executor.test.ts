...
describe('Automation read-only Worker dispatch', () => {
  ...
  it('uses the existing LibraryService read path without changing library bytes', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-automation-readonly-'));
    roots.push(root);
    ...
    const databasePath = path.join(library.libraryPath, '.super', 'library.db');
  ...
  it('serves automation file-operation previews only through the readonly dispatcher', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-automation-plan-'));
    roots.push(root);
    const service = new LibraryService();
    services.push(service);
    const library = service.createLibrary({
  ...
  it('reads the current AI layer without mutating the library', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'super-automation-ai-readonly-'));
    roots.push(root);
