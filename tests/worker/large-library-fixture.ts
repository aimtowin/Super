...
function manifestPath(libraryPath: string): string {
  return path.join(libraryPath, '.super', 'large-library-fixture.json');
}
...
async function prepareImagePool(
  ...
  const poolDir = path.join(libraryPath, '.super', 'image-pool');
...
function prepareVideoPool(libraryPath: string, planned: PlannedAsset[]): string[] {
  ...
  const poolDir = path.join(libraryPath, '.super', 'video-pool');
  mkdirSync(poolDir, { recursive: true });
...
async function seedDatabase(
  ...
  const database = new Database(path.join(libraryPath, '.super', 'library.db'));
  ...
  const artifactsDir = path.join(libraryPath, '.super', 'artifacts');
  ...
  try {
    for (const item of planned) {
      ...
      insertMetadata.run(
        ...
        rating,
        favorite,
        `https://example.test/super/large/${pad(item.index)}`,
        now,
      );
      insertSearchIndex.run(
        ...
        normalizeSearchText(`https://example.test/super/large/${pad(item.index)}`),
