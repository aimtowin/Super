...
export function createGltfModelBytes(index: number): Buffer {
  return Buffer.from(
    JSON.stringify({
      asset: { version: '2.0', generator: `super-large-library-${index}` },
      scenes: [{ nodes: [0] }],
      nodes: [{ mesh: 0 }],
...
export function createTextBytes(index: number, extension: string): Buffer {
  const body = `Super large-library ${index}\nseed-token line\n`;
  if (extension === 'json') {
...
export function createUnsupportedBytes(index: number): Buffer {
  const header = Buffer.from(`SUPER-UNSUPPORTED-${pad(index)}`, 'utf8');
