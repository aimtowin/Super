...
const fixturePath = process.env.SUPER_LARGE_LIBRARY_PREWARM_PATH;
const jumpFractions = [
...
describe.skipIf(!fixturePath)("prepare persistent thumbnails for Electron loading benchmark", () => {
  ...
  it("prepares the deterministic random-jump pages without renderer cache", async () => {
    ...
    let database = new TestDatabase(
      path.join(fixturePath!, ".super", "library.db"),
    );
    const artifactsDir = path.join(fixturePath!, ".super", "artifacts");
    type ArtifactRow = {
      asset_id: string;
