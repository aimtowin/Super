...
function root(): string {
  const value = mkdtempSync(path.join(tmpdir(), 'super-folder-delete-'));
  roots.push(value);
  return value;
}
...
describe('trashManagedFolder (clarification #7 / Super-ekj)', () => {
  ...
  it('reconciles a folder-trash journal after a Worker restart', () => {
    ...
    expect(existsSync(path.join(library.libraryPath, '.super', 'operations'))).toBe(true);
    crashing.closeAll();
...
describe('deleteAssetsFromDisk (clarification #7 / Super-9zc)', () => {
...
describe('deleteManagedFolderFromDisk (clarification #7 / Super-ekj)', () => {
...
describe('removeLinkedFolder (clarification #7 / Super-ekj)', () => {
...
describe('deleteLinkedFolderSubtree (clarification #7 / Super-ekj)', () => {
...
describe('restoreTrashedManagedFolder (Super-qufh)', () => {
  ...
  it('restores assets matched by path when folder id is null (Super-gz4y)', () => {
    const temp = root();
    const service = newService();
    const library = service.createLibrary({
...
describe('syncTrashedFolderTombstones (Super-kqqy)', () => {
...
describe('same-name trash folders (Super-whvm)', () => {
