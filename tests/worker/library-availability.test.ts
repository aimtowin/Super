// Library availability is Super's hardest product baseline.
...
function temporaryRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), 'super-library-availability-'));
...
function backupFile(libraryPath: string, slot: 1 | 2): string {
  return path.join(libraryPath, '.super', 'backups', `library.db.${slot}`);
}
...
function buildLibraryAtVersion(root: string, targetVersion: number): string {
  ...
  mkdirSync(path.join(libraryPath, '.super'), { recursive: true });
...
describe('library availability baseline', () => {
  ...
  it('rewrites swapped v37/v38 history and opens the library writable', () => {
    ...
    // appended after it (Super-4bdd26) and must not survive the rewind.
  ...
  it('does not record a migration retry latch when history checksums are damaged', () => {
    ...
    expect(existsSync(path.join(libraryPath, '.super', 'migration-failed.json'))).toBe(false);
  });
});
