...
const configuredFfmpegPath = configuredMediaBinary(
  "SUPER_REAL_FFMPEG_PATH",
  "ffmpeg",
) ?? configuredMediaBinary("SUPER_FFMPEG_PATH", "ffmpeg");
...
test("generates a decoded thumbnail and keeps asset viewer context coherent", async () => {
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_OPEN_LIBRARY_PATH: libraryPath,
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "user-data"),
      SUPER_E2E_IMPORT_FILES: [sourcePath, nextSourcePath, svgSourcePath].join(
        path.delimiter,
      ),
    },
  });
  ...
  try {
    ...
    // Plain mouse wheel zooms anchored at the pointer (Super-yo0n). Hover
    ...
    // mid-gesture does not move the zoom center (Super-yo0n, user ruling).
...
test("video preview reports a specific generation failure and persists its diagnostic", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-media-preview-e2e-"),
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: sourcePath,
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "user-data"),
      SUPER_FFMPEG_PATH: missingFfmpegPath,
    },
  });
...
test("repairs a historical video preview after a full process restart", async () => {
  test.skip(
    ...
    "requires SUPER_FFMPEG_PATH (or SUPER_REAL_FFMPEG_PATH) and ffprobe",
  );
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-media-auto-repair-e2e-"),
  );
