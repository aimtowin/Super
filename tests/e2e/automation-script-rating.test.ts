...
test('runs the default Desktop Console script and rates only its matching assets', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-console-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: [matchingSource, otherSource].join(path.delimiter),
      SUPER_E2E_OPEN_AUTOMATION_SCRIPT: savedScript,
