...
test('imports a linked folder, reconciles external changes, and relinks after the root is removed', async () => {
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-linked-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_LINKED_SOURCE: sourceRoot,
      SUPER_E2E_LINKED_NEW_ROOT: newRoot,
    },
  });
...
test('restores a linked library after a full app restart', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-linked-restart-'));
  ...
  const launch = () =>
    electron.launch({
      ...
      env: {
        ...process.env,
        SUPER_E2E: '1',
        SUPER_E2E_RESTORE_RECENT: '1',
        SUPER_E2E_USER_DATA_PATH: profilePath,
        SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
        SUPER_E2E_OPEN_LIBRARY_PATH: libraryPath,
        SUPER_E2E_LINKED_SOURCE: sourceRoot,
      },
  ...
  try {
    ...
    // Restart the app — the library should auto-open because of SUPER_E2E_RESTORE_RECENT.
...
test('applies default ignore rules — .git and node_modules are not registered as linked assets', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-linked-filters-'));
  const sourceRoot = path.join(temporaryRoot, 'source');
