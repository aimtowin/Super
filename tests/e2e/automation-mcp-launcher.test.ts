...
test('Desktop embedded MCP exposes and reconnects over authenticated HTTP', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-mcp-http-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: electronLaunchEnv({
      SUPER_E2E: '1',
      SUPER_E2E_USER_DATA_PATH: temporaryRoot,
    }),
  });
  ...
  const parsedConfig = JSON.parse(configText) as {
    mcpServers: { super: { url: string; headers: { Authorization: string } } };
  };
  const endpoint = parsedConfig.mcpServers.super.url;
  const token = parsedConfig.mcpServers.super.headers.Authorization.replace(/^Bearer /u, '');

  try {
    const client = new Client({ name: 'super-mcp-http-e2e', version: '1.0.0' });
    ...
    expect(tools.tools.map((tool) => tool.name)).toEqual(expect.arrayContaining([
      'super_library_list_open',
      'super_library_create',
      'super_asset_search',
    ]));
    const listed = await client.callTool({
      name: 'super_library_list_open',
      arguments: {},
    });
    ...
    const reconnected = new Client({ name: 'super-mcp-http-e2e-reconnect', version: '1.0.0' });
    ...
    expect(reconnectedTools.tools.map((tool) => tool.name)).toContain('super_library_list_open');
