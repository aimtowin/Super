...
// Super-eaxs / REQ-TAG-010–013: chip-grid tag management workspace —
...
test("tag management chip grid: create, sort, select, menu, merge, delete", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-tag-mgmt-"));
  ...
  try {
    ...
    // Super-eaxs regression: entering management while a discovery input is
