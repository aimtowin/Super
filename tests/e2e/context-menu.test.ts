...
function launchApp(temporaryRoot: string, libraryPath: string, importFiles?: string) {
  ...
  return electron.launch({
    ...
    env: {
      ...
      ...(importFiles ? { SUPER_E2E_IMPORT_FILES: importFiles } : {}),
    },
  });
}
...
test("context menu closes on outside click, Escape, scroll, and window resize", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-close-"));
...
test("context menu clamps at viewport edges", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-clamp-"));
...
test("single-menu enforcement — opening new context menu closes existing one", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-single-"));
...
test("context menu has accessible name and keyboard Escape", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-a11y-"));
...
test("window blur closes the context menu", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-blur-"));
...
test("context menu clamps within viewport at all four corners", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-4corner-"));
...
test("scope change closes the context menu", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-scope-"));
...
test("pointer hover stays subtle in the context menu", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-highlight-"));
...
test("multi-asset menu shows a visible count and mixed-selection skip reasons", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-mixed-"));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_OPEN_LIBRARY_PATH: libraryPath,
      SUPER_E2E_IMPORT_FILES: managedSourcePath,
      SUPER_E2E_LINKED_SOURCE: linkedSourceRoot,
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "user-data"),
    },
  });
...
test("tag picker searches, survives in-menu scroll, has no back button, and closes on Escape", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-cm-tagpicker-"));
