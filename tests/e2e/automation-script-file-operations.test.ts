...
test('runs a planned batch file rename through the Desktop Console without exposing a path to the script', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-file-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...process.env,
      SUPER_E2E: '1',
      SUPER_E2E_AUTOMATION_CONFIRM: '1',
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, 'user-data'),
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_OPEN_LIBRARY_PATH: path.join(temporaryRoot, libraryName),
      SUPER_E2E_IMPORT_FILES: source,
    },
  });
  ...
  try {
    ...
    await dialog.locator('#script-sandbox-preview-source').fill(`
      const page = await superApi.assets.search({ query: 'name:Ser-reference', limit: 200, offset: 0 });
      const result = await superApi.assets.renameFiles(
...
test('moves an asset through the Desktop Console plan and updates its folder location', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-move-e2e-'));
  const sourceRoot = path.join(temporaryRoot, 'sources');
  ...
  try {
    ...
    const dialog = window.getByRole('dialog', { name: '自动化脚本' });
    await dialog.locator('#script-sandbox-preview-source').fill(`
      const folders = await superApi.folders.list({ limit: 20 });
      const target = folders.items.find((folder) => folder.name === 'Automation Target');
      const page = await superApi.assets.search({ query: 'name:automation-move', limit: 20, offset: 0 });
      return await superApi.assets.moveToFolder(page.items.map((asset) => asset.id), target.id);
    `);
    await dialog.getByRole('button', { name: '运行', exact: true }).click();
    await expect(dialog.getByText('返回结果', { exact: true })).toBeVisible();
...
test('undoes a scripted asset move from the Desktop Console', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-undo-e2e-'));
  const sourceRoot = path.join(temporaryRoot, 'sources');
  ...
  try {
    ...
    await dialog.locator('#script-sandbox-preview-source').fill(`
      ...
      const page = await superApi.assets.search({ query: 'name:automation-undo', limit: 20, offset: 0 });
      return await superApi.assets.moveToFolder(page.items.map((asset) => asset.id), target.id);
    `);
    await dialog.getByRole('button', { name: '运行', exact: true }).click();
    await expect(dialog.getByText('返回结果', { exact: true })).toBeVisible();
