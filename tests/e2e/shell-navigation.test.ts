...
test("library switcher, breadcrumbs, and workspace history", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-shell-nav-"));
  ...
  try {
    ...
    await expect(window.getByText("SUPER / LOCAL WORKSPACE")).toHaveCount(0);
    ...
    await expect(
      window.getByRole("button", { name: "打开 Super 资源库" }),
    ).toBeVisible();
    await expect(
