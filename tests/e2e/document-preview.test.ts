...
/** Multi-page PDF (hand-assembled, correct xref offsets) — Super-8ca259. */
...
test("opens a multi-page PDF with one rendered wrap per page, no placeholder leftovers (Super-8ca259)", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-pdf-multipage-e2e-"));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: pdfSourcePath,
    },
  });
...
test("zooms the PDF viewer via toolbar and Ctrl+wheel, pans by drag (Super P2 工单)", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-pdf-zoom-e2e-"));
...
test("opens a PDF asset in the scrollable page viewer (Super-8ca259)", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-pdf-viewer-e2e-"));
...
test("opens an HTML asset in the embedded browser viewer (Super-8ca259)", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-html-viewer-e2e-"));
  const libraryName = "HTML 查看验收";
  const libraryPath = path.join(temporaryRoot, libraryName);
  const htmlSourcePath = path.join(temporaryRoot, "page.html");
  writeFileSync(
    htmlSourcePath,
    '<!doctype html><html><head><style>body{background:#123456;color:#fff;font-family:sans-serif}</style></head><body><h1 id="super-html-marker">Super HTML Preview</h1></body></html>',
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_OPEN_LIBRARY_PATH: libraryPath,
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "user-data"),
      SUPER_E2E_IMPORT_FILES: htmlSourcePath,
    },
  });
  ...
  try {
    ...
    await expect(viewer).toBeVisible({ timeout: 30_000 });
    // The iframe loads super://source with the text/html MIME. Use frameLocator
    // (cross-origin safe) to reach the embedded document's marker element.
    const frame = window.frameLocator(".html-viewer-iframe");
    await expect(frame.locator("#super-html-marker")).toBeVisible({ timeout: 20_000 });
    await expect(frame.locator("#super-html-marker")).toHaveText("Super HTML Preview");
  } finally {
    await application.close();
    rmSync(temporaryRoot, { recursive: true, force: true });
  }
