...
/**
 * Super-wgmy: 日志按会话拆分（super-<时间戳>.log）。返回 logs 目录下
 * 最新的会话日志路径（mtime 优先，同名则按名称降序）；目录为空时返回
 * 固定的 superApi.log 占位（调用方按不存在处理）。
 */
export function resolveSessionLogPath(logsPath: string): string {
  ...
  return entries[0] ?? path.join(logsPath, 'superApi.log');
}

export function resolveElectronExecutablePath(): string {
  const override = process.env.SUPER_E2E_ELECTRON_EXECUTABLE;
...
export async function importFilesThroughBridge(
  ...
  await window.evaluate(async (folderName) => {
    ...
    const imported = await bridge.super.library.importFiles({
      libraryId,
