...
test("persists organization and metadata across restart and surfaces optimistic-lock conflicts", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-organization-persistence-e2e-"),
  );
...
test("switches Inspector assets without connection flashes or mixed metadata", async () => {
  ...
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-inspector-switching-e2e-"),
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E: "1",
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_IMPORT_FILES: [alphaSourcePath, betaSourcePath].join(
        path.delimiter,
      ),
      SUPER_E2E_OPEN_LIBRARY_PATH: libraryPath,
  ...
  try {
    ...
    await window.evaluate(() => {
      ...
      sampleFrames();
      testWindow.__superInspectorSwitchTrace = {
        disconnect: () => {
          observer.disconnect();
...
test("assigns Inspector tag suggestions directly and hides tags after their final use is removed", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-inspector-tag-picker-e2e-"),
  );
  const profilePath = path.join(temporaryRoot, "profile");
