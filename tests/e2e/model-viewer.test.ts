...
/**
 * 3D model viewer journeys (0030 slice C, Super-qvc6).
 *
...
async function launchWithModels(modelFiles: string[]) {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-model-viewer-e2e-"),
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: modelFiles.join(path.delimiter),
    },
  });
  const window = await application.firstWindow();
  window.on("console", (message) => {
    const text = message.text();
    if (/model|fbx|gltf|webgl|error|fail|super/i.test(text)) {
...
test("opens an FBX model through the conversion pipeline", async () => {
  ...
  try {
    ...
    // (MODEL-004 / Super-osr0), not a viewer-bottom hang banner.
