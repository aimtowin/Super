...
/**
 * Import conflict vs content duplicate are two DIFFERENT flows (Super-7gvd):
...
test('name conflicts and content duplicates surface separate dialogs with the right options', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-conflict-flows-e2e-'));
