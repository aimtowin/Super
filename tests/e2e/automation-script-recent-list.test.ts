...
test('reopens a saved script from the recent scripts list', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-recent-e2e-'));
  const sourceRoot = path.join(temporaryRoot, 'sources');
  const matchingSource = path.join(sourceRoot, 'Ser-reference.png');
  const savedScript = path.join(temporaryRoot, 'rating.super.ts');
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: matchingSource,
      SUPER_E2E_SAVE_AUTOMATION_SCRIPT: savedScript,
    },
  });
  ...
  try {
    ...
    await expect(dialog).toContainText('已保存脚本：rating.super.ts');
    await expect(dialog.getByRole('button', { name: 'rating.super.ts' })).toBeVisible();
    expect(readFileSync(savedScript, 'utf8')).toContain('superApi.assets.setRating');
    ...
    await window.getByRole('menuitem', { name: '自动化脚本' }).click();
    await expect(dialog).toBeVisible();
    await dialog.getByRole('button', { name: 'rating.super.ts' }).click();
    await expect(dialog.getByRole('textbox', { name: '脚本' })).toHaveValue(/super\.assets\.setRating/);
    await expect(dialog).toContainText('已保存脚本：rating.super.ts');
