...
test('pastes a Main-owned clipboard image into the current folder and collection', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-desktop-ingestion-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_CLIPBOARD_IMAGE_PATH: clipboardSource,
      SUPER_E2E_CLIPBOARD_NOW: '2026-07-13T12:34:56.000Z',
    },
  });
  ...
  try {
    ...
    await expect.poll(() => readdirSync(tempPath).filter((name) => name.startsWith('super-clipboard-'))).toEqual([]);
  } finally {
...
test('returns specific safe desktop-ingestion errors and records their diagnostic causes', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-desktop-errors-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, 'user-data'),
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_CLIPBOARD_IMAGE_PATH: invalidClipboardSource,
    },
  });
  ...
  try {
    ...
    const invalidDrop = await window.evaluate(async () => {
      ...
      const libraries = await bridge.super.library.listOpen();
      return bridge.super.library.importDropped({
...
async function pasteClipboardImage(
  ...
  return window.evaluate(async ({ folderName: targetFolderName, collectionName: targetCollectionName }) => {
    ...
    const collections = await bridge.super.library.listCollections({ libraryId });
    ...
    return bridge.super.library.pasteClipboardImage({
      libraryId,
...
async function resolveClipboardConflict(window: Page, importId: string): Promise<boolean> {
  return window.evaluate(async (token) => {
    const bridge = globalThis as typeof globalThis & {
      super: { library: { resolveImport(input: {
        importId: string;
