...
test("uses an independent critical window for library disk deletion", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-critical-confirmation-test-"),
  );
  ...
  try {
    ...
    const libraryId = await window.evaluate(async () => {
      ...
      const result = await bridge.super.library.listOpen();
    ...
    const openCriticalWindow = async () => {
      const deletion = window.evaluate(async (id) => {
        ...
        const result = await bridge.super.library.deleteLibraryFromDisk({
          libraryId: id,
        });
        return result;
      }, libraryId);
