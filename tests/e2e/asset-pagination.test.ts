...
test("ordinary browsing continuously appends every asset without page controls", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "superApi-pagination-e2e-"),
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_IMPORT_FILES: sourcePaths.join(path.delimiter),
    },
  });
  ...
  try {
    ...
    const setup = await window.evaluate(async (expectedAssetCount) => {
      ...
      const open = await superApi.library.listOpen();
      const libraryId = open.value?.[0]?.libraryId;
      if (!open.ok || !libraryId) throw new Error("Expected an open library.");
      const listed = await superApi.library.listAssets({
        libraryId,
        recursive: true,
      });
      const assetIds = listed.value?.map((asset) => asset.assetId) ?? [];
      const tag = await superApi.library.createTag({
        libraryId,
        name: "分页标签",
      });
      const collection = await superApi.library.createCollection({
        libraryId,
        name: "分页合集",
      });
      const childCollection = collection.ok && collection.value
        ? await superApi.library.createCollection({
            libraryId,
      ...
      if (
        !(
          await superApi.library.assignTags({
            libraryId,
            assetIds,
      ...
      if (
        !(
          await superApi.library.addCollectionAssets({
            libraryId,
      ...
      if (
        !(
          await superApi.library.createSmartCollection({
            libraryId,
            name: "分页智能合集",
    ...
    const trashed = await window.evaluate(async ({ libraryId, assetIds }) => {
      ...
      return superApi.library.trashAssets({ libraryId, assetIds });
    }, setup);
...
test("bottom-scroll appends assets beyond the first page (Super-ws4k)", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "superApi-pagination-beyond-e2e-"),
  );
  const sourceRoot = path.join(temporaryRoot, "sources");
