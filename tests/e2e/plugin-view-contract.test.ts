...
/**
 * Host-managed plugin view contract E2E (Super-ex46.7):
...
test('plugin view contract: mount, theme-without-reload, reload, unmount, library isolation', async (
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-plugin-view-contract-e2e-'));
  ...
  const mainDiag = await application.evaluate(({ app }) => ({
    ...
    e2e: process.env.SUPER_E2E,
    e2eUserData: process.env.SUPER_E2E_USER_DATA_PATH,
  })).catch(() => null);
  ...
  function probeFrame(): Frame | null {
    for (const window of application.windows()) {
      const frame = window.frames().find(
        (candidate) => candidate.url().includes('super-plugin://') && candidate.url().includes('view-contract-probe'),
      );
  ...
  async function openSidebarView(window: Awaited<ReturnType<typeof application.firstWindow>>) {
    ...
    try {
      await expect.poll(() => probeFrame()?.url() ?? '', { timeout: 30_000 }).toContain('super-plugin://');
    } catch (error) {
  ...
  try {
    ...
    for (const [name, relative] of [
      ...
      ['super-log', path.join('logs', 'superApi.log')],
    ] as const) {
