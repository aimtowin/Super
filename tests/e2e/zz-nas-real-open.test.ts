...
// 真实 NAS 冒烟测试：只在显式设置
// SUPER_NAS_REAL_E2E=1 和 SUPER_NAS_REAL_LIBRARY_PATH 时运行。
// 不保留任何默认的生产库路径，验证脚本应指向隔离的探针副本，
// 避免测试意外读取或写入用户数据。
const REAL_LIBRARY = process.env.SUPER_NAS_REAL_LIBRARY_PATH
  ?.trim();
const NAS_REAL_ENABLED = process.env.SUPER_NAS_REAL_E2E === "1" && Boolean(REAL_LIBRARY);

test.skip(
  !NAS_REAL_ENABLED,
  "real-NAS smoke test: set SUPER_NAS_REAL_E2E=1 and SUPER_NAS_REAL_LIBRARY_PATH to an isolated fixture",
);

test("real NAS library end-to-end open and browse", async () => {
  test.setTimeout(900_000);
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-nas-real-"));
  ...
  const application = await electron.launch({
    ...
    env: electronLaunchEnv({
      SUPER_E2E: "1",
      SUPER_E2E_OPEN_LIBRARY_PATH: REAL_LIBRARY,
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "user-data"),
      SUPER_WORKER_CMD_LOG: "1",
      SUPER_OPEN_STAGE_LOG: "1",
      SUPER_REFRESH_STAGE_LOG: "1",
    }),
  });
  try {
    ...
    // （SUPER_NAS_REAL_EXPECT_SOURCES=1，且探针库的源文件可访问）；
    // 源不可达的探针环境里缩略图注定 SOURCE_NOT_FOUND，不该据此判失败。
    if (process.env.SUPER_NAS_REAL_EXPECT_SOURCES === "1") {
