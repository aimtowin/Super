...
test("does not display the metadata concurrency token as a file version", async () => {
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-metadata-version-semantics-e2e-"),
  );
  ...
  try {
    ...
    // Super-4rr4: locate the card by its stable data-asset-id instead of the
    ...
    const assetId = await window.evaluate(async () => {
      ...
      const listed = await bridge.super.library.listAssets({
        libraryId,
        recursive: true,
      });
