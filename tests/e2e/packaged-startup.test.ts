...
test('the packaged application starts and completes a real Worker import', async () => {
  const executablePath = process.env.SUPER_E2E_PACKAGED_EXECUTABLE;
  if (!executablePath) {
    throw new Error('Set SUPER_E2E_PACKAGED_EXECUTABLE after packaging.');
  }

  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-packaged-test-'));
...
test('the packaged application FTS5 search finds and filters imported assets', async () => {
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-packaged-search-'));
...
test('the packaged Windows close button hides the window and keeps the tray process alive', async () => {
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-packaged-close-'));
  const application = await electron.launch({
    executablePath,
    args: [],
    env: {
