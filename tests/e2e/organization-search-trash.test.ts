...
test('organizes, finds, trashes, and restores an imported asset through the UI', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-organization-e2e-'));
  ...
  try {
    ...
    const readMetadataVersion = () => window.evaluate(async (selectedAssetId) => {
      const api = (globalThis as typeof globalThis & { super: { library: {
    ...
    // auto-search is intentionally silent (Super-huvw), so assert on results.
    await expect(locateAssetCard(window, 'hero.png')).toBeVisible();
...
test('multi-select performs batch organization, trash, restore, and permanent delete', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-batch-organization-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, 'user-data'),
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_IMPORT_FILES: [firstSource, secondSource].join(path.delimiter),
    },
  });
  ...
  try {
    ...
    ).super.e2e.getRequestCount('asset.search.request'));
...
test('critical confirmation windows focus cancel and require red confirmation', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-dialog-focus-e2e-'));
...
test('collection recursion toggle immediately refreshes the visible collection scope', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-collection-recursion-e2e-'));
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, 'user-data'),
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_IMPORT_FILES: [childSourcePath, directSourcePath].join(path.delimiter),
    },
  });
