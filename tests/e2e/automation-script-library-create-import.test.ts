...
test('creates a library, then inspects, imports, and restores it through Desktop Console', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-create-import-e2e-'));
  ...
  try {
    ...
    await dialog.locator('#script-sandbox-preview-source').fill(`
      const inspected = await superApi.library.inspect();
...
test('cancels a planned import before retrying it successfully', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-automation-plan-cancel-e2e-'));
  const sourceRoot = path.join(temporaryRoot, 'sources');
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E: '1',
      SUPER_E2E_AUTOMATION_CONFIRM: '1',
      SUPER_E2E_AUTOMATION_CANCEL_ONCE: '1',
      SUPER_E2E_CREATE_PARENT_PATH: temporaryRoot,
      SUPER_E2E_USER_DATA_PATH: profilePath,
    },
  });
  ...
  try {
    ...
    const dialog = window.getByRole('dialog', { name: '自动化脚本' });
    const sourceCode = `
      return await superApi.files.import({
        sourceKind: 'files',
        sourcePaths: ['${source}'],
      });
    `;
