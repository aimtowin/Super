...
const fixturePath = process.env.SUPER_LARGE_LIBRARY_E2E_PATH;
const defaultJumpFractions = [
  0.11, 0.83, 0.37, 0.69, 0.22, 0.77, 0.46, 0.61, 0.15, 0.54,
];
const jumpFractions = (process.env.SUPER_LARGE_LIBRARY_E2E_JUMPS ?? "")
  .split(",")
...
const observationTimeoutMs = Number(
  process.env.SUPER_LARGE_LIBRARY_E2E_OBSERVATION_MS ?? 5_000,
);
...
const minAssets = Number(process.env.SUPER_LARGE_LIBRARY_E2E_MIN_ASSETS ?? 10_000);
...
const cloneRoot = process.env.SUPER_LARGE_LIBRARY_E2E_CLONE_ROOT ?? tmpdir();
...
const reuseLibraryPath = process.env.SUPER_LARGE_LIBRARY_E2E_REUSE_LIBRARY ?? "";
...
const resultPath = process.env.SUPER_LARGE_LIBRARY_E2E_RESULT_PATH ?? "";
...
const profileDir = process.env.SUPER_LARGE_LIBRARY_E2E_PROFILE_DIR ?? "";
...
const traceDir = process.env.SUPER_LARGE_LIBRARY_E2E_TRACE_DIR ?? "";
...
function countLiveAssetsViaElectron(libraryPath: string): number {
  const script = `
    ...
    const db = new Database(${JSON.stringify(path.join(fixturePath ?? "", ".super", "library.db"))}, { readonly: true, fileMustExist: true });
...
test.skip(!fixturePath, "Set SUPER_LARGE_LIBRARY_E2E_PATH to a generated 10k+ fixture.");
...
test("fourth-stop random scrollbar jumps decode the visible viewport within 500ms", async () => {
  ...
  const manifestPath = path.join(fixturePath, ".super", "large-library-fixture.json");
  ...
  const temporaryRoot = mkdtempSync(path.join(cloneRoot, "super-large-scroll-benchmark-"));
  ...
  const userDataPath = process.env.SUPER_LARGE_LIBRARY_E2E_USER_DATA_PATH
  ...
  const application = await electron.launch({
    ...
    env: electronLaunchEnv({
      ...
      SUPER_E2E_USER_DATA_PATH: userDataPath,
    }),
  });
  ...
  try {
    ...
    const minScrollHeight = Number(process.env.SUPER_LARGE_LIBRARY_E2E_MIN_SCROLL_HEIGHT ?? 100_000);
    await expect.poll(
    ...
    await window.evaluate(() => {
      const root = globalThis as typeof globalThis & {
        __superBrowsePages?: unknown[];
        __superLongTasks?: number[];
      };
      root.__superBrowsePages = [];
      root.__superLongTasks = [];
      if ("PerformanceObserver" in globalThis) {
        const observer = new PerformanceObserver((list) => {
          for (const entry of list.getEntries()) {
            root.__superLongTasks!.push(entry.duration);
          }
        });
        observer.observe({ entryTypes: ["longtask"] });
      }
      // Stage breakdown needs every super://preview fetch of the current
      ...
      const capture = ((event: CustomEvent) => {
        root.__superBrowsePages!.push({
          at: performance.now(),
          detail: event.detail,
        });
      }) as EventListener;
      globalThis.addEventListener("super:e2e-browse-page", capture);
      globalThis.addEventListener("super:e2e-browse-result", capture);
      globalThis.addEventListener("super:e2e-browse-request", capture);
    });
      ...
      const sample = await window.evaluate(
        async ({ fraction: targetFraction, timeoutMs }) => {
          ...
          const diagnostics = (globalThis as unknown as {
            super: { e2e: { getRequestCount: (type: "asset.search.request") => number } };
          }).super.e2e;
          ...
          // Keep only this jump's super://preview resource entries so the
          ...
          return await new Promise<{
            ...
            const inspect = () => {
              ...
              const pageEvents = eventRoot.__superBrowsePages ?? [];
              ...
              const fetchStages = performance.getEntriesByType("resource")
                .filter((entry): entry is PerformanceResourceTiming =>
                  entry.name.startsWith("super://preview/"))
              ...
              // Super-140fe2 diagnostics: libraries that legitimately contain
              ...
              // Super-4bdd26: done must key on media cards only. The old
              ...
              if (done || elapsedMs >= timeoutMs) {
                ...
                resolve({
                  ...
                  fetchStages,
                  doneTimeline,
                  longTaskCount: eventRoot.__superLongTasks?.length ?? 0,
                  longTaskMaxMs: Math.max(0, ...(eventRoot.__superLongTasks ?? [])),
                  imageStates,
                  timedOut: !done,
                });
                return;
  ...
  // Super-140fe2: no-unsafe-finally — cleanup runs outside try/finally so
