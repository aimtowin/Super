...
const PLUGIN_ID = 'com.super.unrestricted-settings-probe';
  ...
  return window.evaluate(async (id) => {
    ...
    return {
      ...
      pages: pages.contributions.map((item) => ({
        ...
        pluginId: item.pluginId,
        target: item.target,
        hasUrl: typeof item.url === 'string' && item.url.startsWith('super-plugin:'),
      })),
    };
  }, libraryId);
}
...
test('lists menus.asset and settings.pages after enable, and after recent-library restart restore', async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-unrestricted-settings-e2e-'));
  ...
  const launch = async (): Promise<ElectronApplication> => electron.launch({
    ...
    env: electronLaunchEnv({
      ...
      SUPER_E2E_IMPORT_FILES: [sourcePath, secondSourcePath].join(path.delimiter),
