...
const configuredRawFixture = process.env.SUPER_REAL_RAW_TEST_FILE;
...
test.skip(
  !canRun,
  "Set SUPER_REAL_RAW_TEST_FILE to a real camera RAW file to run this evidence test.",
);

test("imports a real RAW file, renders its metadata, and opens its preview", async () => {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), "super-raw-e2e-"));
