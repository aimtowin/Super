...
// Each process-lifecycle test gets its own fresh SUPER_E2E_USER_DATA_PATH profile so
...
function createIsolatedProfile(): { temporaryRoot: string; profilePath: string } {
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-process-lifecycle-'));
...
test('a second instance restores the existing window', async () => {
  ...
  const env = { ...environment(), SUPER_E2E: '1', SUPER_E2E_USER_DATA_PATH: profilePath };
