...
function assertRealMediaBundleAvailable(): void {
  ...
  throw new Error(
    "SUPER_REQUIRE_REAL_MEDIA=1 requires the darwin-arm64 LGPL FFmpeg bundle. " +
...
test("plays a direct MP4 and a generated WebM fallback through the asset viewer", async () => {
  ...
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-video-playback-e2e-"),
  );
  ...
  try {
    ...
    application = await electron.launch({
      ...
      env: {
        ...
        SUPER_E2E_IMPORT_FILES: [directPath, proxyPath].join(path.delimiter),
    ...
    await expect
      ...
      .toMatch(/^super:\/\/source\//);
    await expectPlayableAndSeekable(directVideo);
    // Video zoom/pan/fit mirrors the image viewer (Super-190): the mouse
