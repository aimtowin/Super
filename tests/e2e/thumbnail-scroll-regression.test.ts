...
const ffmpegCommand = process.env.SUPER_REAL_FFMPEG_PATH ??
  process.env.SUPER_FFMPEG_PATH ??
  (existsSync(installedFfmpegPath) ? installedFfmpegPath : buildFfmpegPath);
const ffprobeCommand = process.env.SUPER_REAL_FFPROBE_PATH ??
...
test("keeps visible masonry cards complete during rapid scroll at the tuned thumbnail size", async () => {
  ...
  const temporaryRoot = mkdtempSync(
    path.join(tmpdir(), "super-thumbnail-scroll-regression-"),
  );
  ...
  const application = await electron.launch({
    ...
    env: {
      ...
      SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "profile"),
    },
  });
...
test.describe("video poster scrolling", () => {
  ...
  test("keeps visible video posters decoded after rapid scrolling", async () => {
    const assetCount = 20;
    const temporaryRoot = mkdtempSync(
      path.join(tmpdir(), "super-video-thumbnail-scroll-regression-"),
    );
    const sourceRoot = path.join(temporaryRoot, "sources");
    mkdirSync(sourceRoot);
    const sourcePaths: string[] = [];
    ...
    const application = await electron.launch({
      ...
      env: {
        ...
        SUPER_E2E_IMPORT_FILES: sourcePaths.join(path.delimiter),
        SUPER_E2E_USER_DATA_PATH: path.join(temporaryRoot, "profile"),
        SUPER_FFMPEG_PATH: ffmpegCommand,
        SUPER_FFPROBE_PATH: ffprobeCommand,
      },
    });
