**Super 0.1.3** — NAS 资源库、检查更新与一键更新、PDF 缩放与 Windows 文件复制 · NAS libraries, check-for-updates with one-click install, PDF zoom, and Windows file copy
...
*Libraries on network shares (NAS/SMB) are now supported (experimental). Super uses rollback journaling there; file locking and disconnect recovery depend on the NAS. Keep a single writer at a time and keep your own backups.*
...
**诊断日志按会话拆分**
- 每次启动写入独立日志文件，并自动清理过旧会话日志，不再无限追加到单一 `super.log`。

*Each launch writes its own session log; older session logs are pruned instead of appending forever to a single `super.log`.*
