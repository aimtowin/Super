/**
 * Application menu template without Electron page-zoom roles (Super-46i9).
 *
 * Electron's default View menu registers zoomIn / zoomOut / resetZoom with
 * Cmd/Ctrl+=,-,0. Those accelerators steal the chords from Super's own
 ...
 *
 * Super-r7gu: Windows has no native menu bar — capabilities live in-app
 * (library menu, context menus, commands). macOS keeps the system menu.
 *
 * Super-q0b1: the macOS menu mirrors the Windows in-app MainMenu commands
...
export type ApplicationMenuItemTemplate = {
   ...
   * never inlines locale strings (Super-q0b1).
   */
  readonly labelKey?: string;
  readonly submenu?: readonly ApplicationMenuItemTemplate[];
  /** Custom Super command (wired in Main when installing the menu). */
...
export type ApplicationMenuTemplateOptions = {
  ...
  /** App UI locale for custom menu labels (Super-te8p). */
...
/**
 * Windows: hide the native top menu bar (Super-r7gu / Super-j5x).
...
export function buildApplicationMenuTemplate(
  ...
  const viewSubmenu: ApplicationMenuItemTemplate[] = [
    ...
    ...(options.showDevTools
      ...
      : []),
    { type: "separator" },
    // Super-46i9: do NOT include zoomIn / zoomOut / resetZoom.
  ...
  const macAppMenu: ApplicationMenuItemTemplate = {
    label: "Super",
    submenu: [
      { role: "about", labelKey: "shell.mainMenuAboutSuper" },
      { type: "separator" },
  ...
  const editSubmenu: ApplicationMenuItemTemplate[] = isMac
    ? [
        // Super-q0b1: business undo/redo (not the always-available text
        ...
        // Super-166q: do not use role:copy — it steals ⌘C from asset file copy.
  ...
  // Super-q0b1: macOS keeps a real application menu, so the product
  ...
  const helpSubmenu: ApplicationMenuItemTemplate[] = isMac
    ? [
        commandItem("about.super", "shell.mainMenuAboutSuper"),
        { type: "separator" },
