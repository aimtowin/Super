...
export interface SuperShellApi {
  /** 打开外部 HTTP(S) 链接；失败时返回公开错误码（不含 URL）。 */
  openExternalUrl(url: string): Promise<OpenExternalUrlResult>;
  /**
   * Reveal the main-process log file in the OS file manager (Super-iokf).
  ...
  /**
   * Sync the effective UI locale to Main for native file dialogs (Super-bwb).
  ...
  /** macOS Edit 菜单反选（Super-te8p）；与 ⌘I / Ctrl+I 等价。 */
  ...
  /**
   * Windows hidden F2 / Delete accelerators (Super-g8u9). Disable while an
