...
/**
 * Host-managed plugin view contract (Super-ex46.7).
 *
...
const pluginUiThemeTokenNameSchema = z.string().regex(
  /^--(?:ui-[a-z0-9-]+|super-plugin-(?:ref|token)-[a-z][a-z0-9.-]*)$/u,
);
 ...
 * `null` origin. Chromium custom-scheme documents (super-plugin://) may
...
export function isTrustedPluginUiMessage(input: {
  ...
  /** Optional `super-plugin://<pluginId>` origin accepted from plugin UI frames. */
