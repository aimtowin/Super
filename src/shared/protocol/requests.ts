...
// Author/creator (Super-7x0) mirrors sourcePageUrl's clear-with-empty-string
...
export const rendererRequestSchema = z.discriminatedUnion('type', [
  ...
  // Irreversible: close then delete the library root on disk (Super-9i8).
  z.strictObject({
  ...
  // Soft-forget: drop from recent list only; disk untouched (Super-ucx).
  z.strictObject({
  ...
  // Clarification #5 / Super-vgp: OS file clipboard (Finder/Explorer).
  z.strictObject({
  ...
  // Clarification #7 / Super-ekj: managed folder trash / permanent disk delete.
  z.strictObject({
  ...
  z.strictObject({
    type: z.literal('collection.linked-folder.add.request'),
    libraryId: identifierSchema,
    collectionId: identifierSchema,
    linkedFolderId: identifierSchema,
    relativePath: linkedSubtreeRelativePathSchema,
  ...
  z.strictObject({
    type: z.literal('collection.source-browse-entries.request'),
    libraryId: identifierSchema,
    collectionId: identifierSchema,
    recursive: z.boolean(),
  ...
  z.strictObject({
    ...
    /** Super-ws4k: return only `assetIds` for the whole scope (select-all); limit/offset ignored. */
  ...
  z.strictObject({
    ...
    serverId: nonBlankString,
    directoryName: z.string().optional(),
    /** 自动同步开关（Super-bfsb 后续：打开远程库后默认开启）。 */
  ...
  // Slice C (Super-qvc6): renderer request surface for the 3D viewer. The
  ...
  // Clarification #5 / Super-bkef: OS file clipboard for asset files.
  z.strictObject({
  ...
  z.strictObject({
    ...
    confirm: z.boolean(),
    /** When set, only these AI layers are cleared (Super-u7hz). Omit = all. */
    fields: z
...
export const workerCommandSchema = z.discriminatedUnion('type', [
  ...
  z.strictObject({
    type: z.literal('collection.linked-folder.add'),
    libraryId: identifierSchema,
    collectionId: identifierSchema,
    linkedFolderId: identifierSchema,
    relativePath: linkedSubtreeRelativePathSchema,
  ...
  // Slice A (Super-fu2i): companion-texture index for model assets. The
