...
/** Super-xffq: WebDAV 能力探测结果（probe/preview/run 的 report 内均带）。 */
...
export const internalLibrarySummarySchema = z.strictObject({
  ...
  // Super-033e: the library was written by a newer Super build than the
...
/**
 * 同步传输进度（Super-xffq 增量）：done/total 为文件动作数，
...
export const aiContentClearedEventSchema = z.strictObject({
  ...
  // Super-c9r3: IDs whose AI layer was cleared, so the renderer can refresh
...
const assetOperationSuccessSchemas = [
  ...
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.removal-queued'),
    job: z.strictObject({
      jobId: nonBlankString,
      folderId: nonBlankString,
      folderName: safeDisplayName,
      totalAssets: z.number().int().nonnegative(),
      removedAssets: z.number().int().nonnegative(),
      status: z.enum(['queued', 'running', 'paused', 'failed', 'succeeded']),
    }),
  }),
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.removal-jobs'),
    jobs: z.array(z.strictObject({
      ...
      errorDetail: nonBlankString.nullable(),
      createdAt: nonBlankString,
      updatedAt: nonBlankString,
    })),
  }),
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.removal-jobs-paused'),
    pausedCount: z.number().int().nonnegative(),
  }),
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.removal-jobs-resumed'),
    resumedCount: z.number().int().nonnegative(),
  }),
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.index-jobs'),
    jobs: z.array(z.strictObject({
      jobId: nonBlankString,
      folderId: nonBlankString,
      folderName: safeDisplayName,
      indexedAssets: z.number().int().nonnegative(),
      scannedDirectories: z.number().int().nonnegative(),
      pendingDirectories: z.number().int().nonnegative(),
      status: z.enum(['queued', 'running', 'paused', 'failed', 'succeeded']),
      errorCode: nonBlankString.nullable(),
      errorDetail: nonBlankString.nullable(),
      createdAt: nonBlankString,
  ...
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.index-jobs-paused'),
    pausedCount: z.number().int().nonnegative(),
  }),
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('linked-folder.index-jobs-resumed'),
    resumedCount: z.number().int().nonnegative(),
  }),
  z.strictObject({
    ok: z.literal(true),
  ...
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('collection.linked-folder.added'),
    collectionId: nonBlankString,
  ...
  z.strictObject({
    ok: z.literal(true),
    type: z.literal('collection.source-browse-entries'),
    entries: z.array(folderBrowseEntrySchema),
  }),
  z.strictObject({
    ok: z.literal(true),
  ...
  z.strictObject({
    ...
    total: z.number().int().nonnegative(),
    offset: z.number().int().nonnegative(),
    /** Super-ws4k: present when the request used idsOnly (select-all). */
  ...
  z.strictObject({
    ...
    // (no Worker raster generator; Super-fu2i).
...
const workerSuccessResultSchema = z.discriminatedUnion('type', [
  ...
  z.strictObject({
    ...
    // Path of the GLB artifact relative to `.super/artifacts`.
  ...
  // Slice A (Super-fu2i): companion-texture index for a model asset. Only
...
const rendererSuccessResultSchema = z.discriminatedUnion('type', [
  ...
  // Slice C (Super-qvc6): renderer-side results for the 3D viewer requests.
