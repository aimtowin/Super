...
export const PUBLIC_ERROR_MESSAGES = {
  ...
  INTERNAL_ERROR:
    'Super hit an unexpected error and could not finish. Open Diagnostics in the app to inspect this session’s log, then retry. If you were opening a library, copy the whole library folder onto a local disk and open it from there.',
  ...
  IMPORT_COLLECTION_ASSIGN_FAILED: 'The assets were imported, but Super could not add them to the selected collection.',
  ...
  LIBRARY_NOT_FOUND:
    'Super cannot find that library folder. It may have been moved, renamed, or the disk may be disconnected. Reconnect the drive or choose the library again from Open Library.',
  NOT_A_LIBRARY:
    'The selected folder is not a Super library (it has no .super database).',
  LIBRARY_CORRUPT:
    'Super could not read this library’s database (integrity check or migration history failed). Reopen the library so Super can restore from a backup. Copy the whole library folder to a local disk first if you want a safety copy.',
  LIBRARY_ENGINE_UNAVAILABLE:
    'Super could not load its SQLite database engine. This is an application install problem, not damage to your library files. Reinstall Super or rebuild native modules; do not restore backups or delete the library folder.',
  LIBRARY_VERSION_TOO_NEW:
    'This library was created by a newer version of Super, so this app cannot open it. Install the latest Super, then open the library again.',
  LIBRARY_READ_ONLY:
    'Super cannot write to this library because the files are locked or marked read-only. Clear the read-only flag, close other programs using the folder, and retry.',
  ...
  LIBRARY_MIGRATION_STUCK:
    'This library could not be migrated after repeated attempts. Super opened it at the last working schema so you can keep using it. Upgrade Super when a fix is available; do not delete the library folder.',
  LIBRARY_STRUCTURE_MISMATCH:
    'This library’s database structure does not match this version of Super. Upgrade to the latest Super and reopen the library.',
  LIBRARY_NOT_WRITABLE:
    'Super could not write files in the selected folder. The folder may lack write permission, the disk may be full or read-only, or another program may have the files locked. Check the folder and NAS connection, then retry.',
  LIBRARY_NETWORK_SHARE:
    'Super could not open or write the library database on this network share (NAS/SMB). NAS libraries use rollback journaling and depend on the share’s file-locking and reconnect behavior. Check the NAS connection and permissions; if it keeps failing, copy the library to a local disk or use WebDAV sync.',
  LIBRARY_IO_ERROR:
    'Super could not complete the library operation because the disk or filesystem reported an I/O error. Check the drive connection, free space, and permissions; if it keeps failing, copy the library to a local disk and inspect Diagnostics.',
  LIBRARY_BUSY: 'This library is being updated by another Super window or a brief database lock. Wait a few seconds and retry; do not open the same library from two computers at once.',
  LIBRARY_CLEANUP_FAILED:
    'Creating the library failed, and leftover temporary files could not be removed automatically. Delete any `.super-create-*.partial` folder next to the chosen location, then retry with a writable folder.',
  ...
  ASSET_SOURCE_TRASH_FAILED: 'Super could not move the asset source to the system trash.',
  ...
  SYNC_CONNECTION_FAILED:
    'Super could not reach the sync server. Check the address, username, password, and that the computer is online. A more specific reason is listed when Super can classify the failure.',
