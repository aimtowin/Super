...
export interface SuperLibraryApi {
  ...
  /** Convert and open an Eagle library as a new Super library. */
  ...
  /** Convert and open a Billfish library as a new Super library. */
  ...
  /** Remove a path from the recent list without deleting disk (Super-ucx). */
  ...
  /** Close then permanently delete the library root on disk (Super-9i8). */
  ...
  >;
  listLinkedFolders(input: {
    libraryId: string;
    /** Includes roots hidden by explicit ignore rules for recovery and removal. */
    showIgnored?: boolean;
  ...
  /** Adds a one-time snapshot of visible, available assets from a linked directory. */
  addLinkedFolderAssetsToCollection(input: { libraryId: string; collectionId: string; linkedFolderId: string; relativePath: string; recursive: boolean }): Promise<LibraryApiResult<{ collectionId: string; addedCount: number; historyEntryId?: string }>>;
  ...
  /** Read-only linked source-directory cards derived from current collection members. */
  listCollectionSourceBrowseEntries(input: { libraryId: string; collectionId: string; recursive: boolean; parentFolderId: string | null; showIgnored?: boolean }): Promise<LibraryApiResult<FolderBrowseEntry[]>>;
  ...
  // Search
  searchAssets(input: { libraryId: string; query?: SearchQuery | null; filters?: FilterClause[]; scope?: SearchScope; sort?: { field: 'name' | 'modified_at' | 'created_at' | 'byte_size' | 'long_edge' | 'duration' | 'rating' | 'color' | 'author'; order: 'asc' | 'desc' }; scopeMode?: boolean; idsOnly?: boolean; layoutOnly?: boolean; deferTotal?: boolean; limit?: number; offset?: number; showIgnored?: boolean }): Promise<LibraryApiResult<{ items: AssetSummary[]; total: number; totalIsExact?: boolean; offset: number; snippets?: { assetId: string; text: string }[]; assetIds?: string[]; layout?: BrowseLayoutEntry[] }>>;
  ...
  /** Super-xffq: 保存/新增同步服务器（密码经 Main safeStorage 加密）。 */
  ...
  /** Super-xffq: 保存库绑定（服务器 + 可选同步文件夹名，默认库名；enabled=自动同步开关）。 */
  ...
  /** Super-xffq: 从服务器拉取远端库并创建本地库（Main 选择保存位置）。 */
