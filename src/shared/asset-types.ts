 ...
 *
 * Super persists relative paths with POSIX separators on every platform.
...
export const folderBrowseEntrySchema = z.strictObject({
  ...
  directAssetCount: z.number().int().nonnegative(),
  /** All descendant assets (REQ-FOLDER-003 / Super-toh). */
  ...
  /**
   * Up to 3 cover candidate asset ids (Super-d0nv). The Worker schedules
...
export const linkedFolderSummarySchema = z.strictObject({
  ...
  status: z.enum(['available', 'offline']),
  assetCount: z.number().int().nonnegative(),
  /** Absolute linked root for hover affordance (Super-rc9). */
...
export const assetSummarySchema = z.strictObject({
  ...
  /** Tombstone that owned this asset when its folder was trashed (Super-whvm). */
...
export const browseLayoutEntrySchema = z.strictObject({
  ...
  /** Caption fields so layout slots are not blank while AssetSummary pages stream in (Super-l2at). */
...
export const assetMetadataResultSchema = z.strictObject({
  ...
  // Author/creator (Super-7x0): user-editable, auto-extracted from
...
export const searchScopeSchema = z.discriminatedUnion('kind', [
  ...
  z.strictObject({
    ...
    collectionId: nonBlankString,
    recursive: z.boolean(),
    /** Optional virtual linked directory; still constrained to collection members. */
    sourceFolderId: z.string().min(1).max(4096).optional(),
  }),
  z.strictObject({
 ...
 * understood by Super's ordinary search engine. It cannot carry SQL,
