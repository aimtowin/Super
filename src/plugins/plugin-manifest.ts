...
export const PLUGIN_MANIFEST_FILE_NAME = 'super-plugin.json';
...
export const semverRangeSchema = z.string().min(1).max(256).superRefine((value, context) => {
  if (!isValidSemverRange(value)) {
    context.addIssue({
      code: 'custom',
      message: 'Super engine ranges use explicit SemVer comparators (for example >=0.2.0 <1.0.0).',
    });
  }
});
...
export type PluginCompatibilityResult =
  ...
  | {
    ok: false;
    code: 'PLUGIN_SUPER_VERSION_UNSUPPORTED' | 'PLUGIN_API_VERSION_UNSUPPORTED' | 'PLUGIN_PLATFORM_UNSUPPORTED';
    message: string;
  };
...
export function validatePluginManifestCompatibility(
  ...
  if (!satisfiesSemverRange(target.superVersion, manifest.engines.super)) {
    return {
      ok: false,
      code: 'PLUGIN_SUPER_VERSION_UNSUPPORTED',
      message: 'This plugin version does not support the current Super version.',
    };
  }
