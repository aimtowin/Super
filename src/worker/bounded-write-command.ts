...
export function executeBoundedWriteWorkerCommand(
  ...
  switch (command.type) {
    ...
    case 'collection.linked-folder.add': {
      const result = libraryService.addLinkedFolderAssetsToCollection(command);
      const history = libraryService.recordCollectionMembershipHistory({
        libraryId: command.libraryId,
        collectionId: result.collectionId,
        assetIds: result.addedAssetIds,
        direction: 'add',
        source: historyContext?.source,
        sourceReference: historyContext?.sourceReference,
      });
      return {
        ok: true,
        type: 'collection.linked-folder.added',
        collectionId: result.collectionId,
        addedCount: result.addedAssetIds.length,
        ...(history ? { historyEntryId: history.historyEntryId } : {}),
      };
    }
