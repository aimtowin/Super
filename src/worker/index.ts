...
// Super-8ca259: Electron's UtilityProcess sets process.type = 'utility'.
...
const libraryService = new LibraryService({
  ...
  // Super-8ca259: HTML document thumbnails capture offscreen in Main.
...
/** Super-xffq：按设备身份构建同步引擎（deviceId 由 Main 持久化生成）。 */
...
// ── Slice E: offscreen model-thumbnail render client (Super-hnmg) ────
...
/**
 * Super-8ca259: ask Main to capture an HTML document thumbnail in a fresh
...
function modelSourceUrl(libraryId: string, assetId: string, revisionId: string): string {
  return `super://source/${libraryId}/${assetId}?revision=${encodeURIComponent(revisionId)}`;
}
function modelPreviewUrl(libraryId: string, artifactId: string): string {
  return `super://preview/${libraryId}/${artifactId}`;
}
...
function scheduleThumbnailQueue(
  ...
  options: {
    ...
    retryFailed?: boolean;
    /** Super-x9xu light scenes skip stale-artifact invalidation sweeps. */
    skipStaleRepair?: boolean;
    /** Super-4bdd26: cap in-flight decodes for this scene (startup backfill). */
  ...
  const runBatch = async (): Promise<void> => {
    ...
    try {
      ...
      const processed = await libraryService.processThumbnailQueue(libraryId, {
        ...
        // Slice E (Super-hnmg): model jobs render offscreen in Main; the
...
function noteInteractiveMediaRequest(libraryId: string): void {
  // Super-4bdd26 收编 codex/large-library-performance@15f3325c：冷可见波包含
...
/**
 * Super-x9xu redesign: scenes that fire on every browse/search response are
...
function scheduleThumbnailScene(
  ...
  const configs: Record<ThumbnailScheduleScene, { limit?: number; priority: number; maxIds?: number; processMaxJobs?: number }> = {
    ...
    // Super-azf6: the CURRENT VIEW must outrank the import flood — browsing
    ...
    // Super-x9xu / Super-87pd: the visible wave covers the current
    ...
    // Super-d0nv: folder-card covers are direct assets of child folders,
...
function thumbnailFailureReason(errorCode: string): string {
  switch (errorCode) {
    ...
    case 'FFMPEG_REQUIRED': return '无法生成视频缩略图（媒体组件不可用）。请重新安装或修复 Super 后重试。';
    case 'OIIO_REQUIRED': return '缺少 OpenImageIO，无法解码此图片。请安装图像组件后重试。';
    case 'SHARP_UNAVAILABLE': return '图片解码组件不可用。请重新安装或更新 Super 后重试。';
...
/**
 * Super-fatf：构造 WebDAVDriver 前规范化服务器地址（缺协议自动补
...
async function handleRequestWithoutWriteLease(request: WorkerRequest): Promise<WorkerResult> {
  switch (request.command.type) {
    ...
    case 'sync.poll-remote': {
      // 自动同步轮询（Super-bfsb 后续）：轻量检测远端 manifest 变化，
      // 不做本地全量 hash，供 Main 定时调度器决定是否触发完整同步。
      // Super-140fe2/308675: pollRemoteChange 先打一轮 WebDAV 网络请求、
    ...
    case 'library.open': {
      ...
      if (!library.readOnly) {
        deferStartupThumbnailScene(library.libraryId);
        // Super-tumv (LIB-018): deliver the opened response first, then run the
    ...
    case 'folder.browse-entries': {
      ...
      // Super-d0nv: folder covers are direct assets of child folders —
    ...
    case 'asset.list':
      {
        ...
        // Super-4bdd26 收编 codex/large-library-performance@d5f58088：渲染端
    ...
    case 'linked-folder.list':
      return {
        ...
        folders: libraryService.listLinkedFolders(
          request.command.libraryId,
          request.command.showIgnored === true,
        ),
      };
    ...
    case 'collection.linked-folder.add':
      throw new Error('Bounded collection.linked-folder.add write was not dispatched through its transaction fence.');
    ...
    case 'collection.assets.list': {
      ...
      // Super-4bdd26 收编 codex/large-library-performance@d5f58088：同
      // asset.list——视口上报（asset.thumbnail.visible-window）才是可见波的唯一触发源。
      return { ok: true, type: 'collection.assets.list', assets };
    }
    case 'collection.source-browse-entries': {
      const entries = libraryService.listCollectionSourceBrowseEntries(request.command);
      const coverAssetIds = entries.flatMap((entry) => entry.coverAssetIds);
      if (coverAssetIds.length > 0) {
        scheduleThumbnailScene(
          request.command.libraryId,
      ...
      return { ok: true, type: 'collection.source-browse-entries', entries };
    }
    ...
    case 'asset.analyze': {
      ...
      if (isVideo) {
        // Super-140fe2: contact sheets are generated lazily at analysis time
        ...
        // Super-6w40: 3D models get an AI four-view sheet — render the
    ...
    case 'media.get-preview-artifact': {
      ...
      // Super-tz35: the viewer wave stays at priority 350 and skips repair
    ...
    case 'media.get-asset-drag-infos': {
      ...
      // Super-v4jf: batched resolution — the legacy per-asset loop cost 3-4
    ...
    case 'asset.thumbnail.visible-window': {
      // Super-visible-window: the renderer reports what the user is actually
      ...
      // Super-4bc4ac: ignored assets are not indexed or operated on —
      ...
      // Super-4bdd26 收编：先中断视口外的解码并调度可见波（让新视口尽快
      // 抢占），再做尺寸探测。
    ...
    case 'ai.process-queue': {
      ...
      // take effect for the next queue batch without restarting Super, while
...
parentPort.on('message', async (event) => {
  ...
  try {
    // Super-8ca259: Main's offscreen document capture result.
  ...
  // Super-onch/9e1d8d: SUPER_WORKER_CMD_LOG=1 emits one JSON line per
  ...
  // Super-2cc492: settle the open-reconciliation startup gate only after the
