 ...
 * tool `super_classify_asset` with the structured `input_schema`
...
export class AnthropicVendorAdapter implements VendorAdapter {
  ...
  #buildSystemPrompt(request: AiAnalysisRequest): string {
    return (
      ...
      '\n通过调用 `super_classify_asset` 工具返回结果。\n'
    );
  }
  ...
  #extractResult(json: unknown): AiAnalysisResult {
    ...
    if (block.type !== 'tool_use') {
      ...
      // Super-iokf: some Anthropic-compatible proxies return plain text JSON
