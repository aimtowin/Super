 ...
 *
 * Resolution order:
 *   1. Environment variable (SUPER_FFMPEG_PATH / SUPER_OIIO_PATH)
 ...
 *
 * Main also injects absolute SUPER_*_PATH when forking the Library Worker
 ...
 *
 * Priority: SUPER_FFMPEG_PATH env > bundled/dev resources > 'ffmpeg' (PATH).
 */
export function resolveFfmpegPath(): string {
  const envPath = process.env['SUPER_FFMPEG_PATH'];
 ...
 *
 * If SUPER_FFMPEG_PATH is set, ffprobe is resolved in the same directory.
 * Otherwise: bundled/dev resources > 'ffprobe' (PATH).
 */
export function resolveFfprobePath(): string {
  const envFfmpeg = process.env['SUPER_FFMPEG_PATH'];
  if (envFfmpeg) {
 ...
 *
 * Priority: SUPER_OIIO_PATH env > bundled/dev resources > 'oiiotool' (PATH).
 */
export function resolveOiiotoolPath(): string {
  const envPath = process.env['SUPER_OIIO_PATH'];
  if (envPath) return envPath;
