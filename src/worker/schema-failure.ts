 ...
 * a `.super/migration-failed.json` record per library: the open path reads
...
export interface MigrationFailureRecord {
   ...
   * Super upgrade the migration is retried once, so a newer build can
...
export function migrationFailurePath(libraryPath: string): string {
  return path.join(libraryPath, '.super', 'migration-failed.json');
}
