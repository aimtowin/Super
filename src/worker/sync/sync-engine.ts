/**
 * 同步编排服务（Super-xffq）——SyncEngine 的高层入口。
 *
...
export interface SyncEngineOptions {
  ...
  now?(): string;
  /**
   * 传输进度回调（Super-xffq 增量）：done/total 为已处理动作数，
...
export class SyncEngine {
  ...
  buildDriver(root: SyncRootConfig): RemoteStorageDriver {
    // Super-fatf: 纵深防御 —— 命令入口已规范化，但 engine 可能被直接
  ...
  async syncOnce(libraryId: string, root: SyncRootConfig): Promise<SyncOutcome> {    const driver = this.buildDriver(root);
    ...
    const metaDir = `${directoryName === '' ? '' : `${directoryName}/`}.super-sync`;
