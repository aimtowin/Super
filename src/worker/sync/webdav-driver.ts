/**
 * WebDAV 存储驱动（Super-xffq）——RemoteStorageDriver 的第一个实现。
 *
...
export interface WebDAVDriverConfig {
   ...
   * （Super-4s8b 用户决定），只依赖 TCP 层行为与用户取消。
   */
  timeoutMs?: number;
}
...
const USER_AGENT = 'Super/0.1 sync';
    ...
    try {
      ...
      if (mkcolOk) {
        const written = await this.request('PUT', probeFile, {
          body: Buffer.from(`super-probe-${Date.now()}`),
          headers: { 'Content-Type': 'application/octet-stream' },
        });
        if (written.status >= 200 && written.status < 300) {
          ...
          const moved = await this.request('MOVE', probeFile, {
            destination: joinWebDAVUrl(this.config.baseUrl, '.super-sync/.probe-2'),
            headers: { Overwrite: 'T' },
          });
          capabilities.supportsMove = moved.status >= 200 && moved.status < 300;
          if (capabilities.supportsMove) await this.request('DELETE', '.super-sync/.probe-2');
