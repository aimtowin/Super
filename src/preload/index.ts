...
import type { AiJobStatus, LibraryApiResult, LinkedAssetDeleteResult, MediaJobStatus, PluginJobStatus, PreviewResolution, RelinkAssetResult, SuperLibraryApi, SyncCapabilities, SyncReport } from '../shared/library-api';
...
const e2eEnabled = process.env.SUPER_E2E === '1';
if (e2eEnabled) {
  const e2eLocale = process.env.SUPER_E2E_LOCALE === 'en' ? 'en' : 'zh-CN';
  (globalThis as { __SUPER_E2E_LOCALE__?: string }).__SUPER_E2E_LOCALE__ =
    e2eLocale;
  // Default dark so system-theme preference stays visually stable in CI.
  const e2eTheme = process.env.SUPER_E2E_THEME === 'light' ? 'light' : 'dark';
  (globalThis as { __SUPER_E2E_THEME__?: string }).__SUPER_E2E_THEME__ =
    e2eTheme;
}
...
const library: SuperLibraryApi = Object.freeze({
  async create({
    displayName,
  }: {
    displayName: string;
  ...
  async listLinkedFolders({ libraryId, showIgnored }: {
    libraryId: string;
    showIgnored?: boolean;
  }): Promise<LibraryApiResult<LinkedFolderSummary[]>> {
    const result = await request({ type: 'linked-folder.list.request', libraryId, showIgnored });
    if (!result.ok) return failure(result);
  ...
  async addLinkedFolderAssetsToCollection({ libraryId, collectionId, linkedFolderId, relativePath, recursive }: { libraryId: string; collectionId: string; linkedFolderId: string; relativePath: string; recursive: boolean }): Promise<LibraryApiResult<{ collectionId: string; addedCount: number; historyEntryId?: string }>> {
    const result = await request({ type: 'collection.linked-folder.add.request', libraryId, collectionId, linkedFolderId, relativePath, recursive });
    if (!result.ok) return failure(result);
    if (result.type !== 'collection.linked-folder.added') throw new Error('Unexpected add-linked-folder-to-collection response.');
    return { ok: true, value: { collectionId: result.collectionId, addedCount: result.addedCount, ...(result.historyEntryId ? { historyEntryId: result.historyEntryId } : {}) } };
  },
  ...
  async listCollectionSourceBrowseEntries({ libraryId, collectionId, recursive, parentFolderId, showIgnored }: { libraryId: string; collectionId: string; recursive: boolean; parentFolderId: string | null; showIgnored?: boolean }): Promise<LibraryApiResult<FolderBrowseEntry[]>> {
    const result = await request({ type: 'collection.source-browse-entries.request', libraryId, collectionId, recursive, parentFolderId, showIgnored });
    if (!result.ok) return failure(result);
    if (result.type !== 'collection.source-browse-entries') throw new Error('Unexpected list-collection-source-browse-entries response.');
    return { ok: true, value: result.entries };
  },
  ...
  // Super-visible-window: fire-and-forget report of the viewport assets —
  ...
  // Super-xffq: 同步服务器（全局）与库绑定（Main 本地，密码 safeStorage）。
  ...
  // Super-xffq: 同步探测/预览/执行（Main 按 serverId 解析 URL 与凭据）。
  ...
  // Slice C (Super-qvc6): 3D viewer renderer request surface. Both requests
  ...
  startAssetDrag({
    ...
    // Do not make this synchronous: dropping back onto Super would otherwise
...
const shell: SuperShellApi = Object.freeze({
...
const appUpdate: SuperAppUpdateApi = Object.freeze({
...
const automation: SuperAutomationScriptApi = Object.freeze({
  async open() {
...
const mcp: SuperMcpSettingsApi = Object.freeze({
...
const plugins: SuperPluginManagerApi = Object.freeze({
  ...
  async listPluginContributions(input: {
    ...
    return 'contributions' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['listPluginContributions']>>
  ...
  async runPluginCommand(input: Extract<PluginManagerRequest, { type: 'plugin-manager.run-command' }>) {
    ...
    return 'executed' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['runPluginCommand']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async searchProviders(input: Extract<PluginManagerRequest, { type: 'plugin-manager.search-providers' }>) {
    ...
    return 'search' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['searchProviders']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async previewProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.preview-provider' }>) {
    ...
    return 'media' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['previewProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async thumbnailProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.thumbnail-provider' }>) {
    const response = await this.request(input);
    return 'media' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['thumbnailProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async metadataProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.metadata-provider' }>) {
    ...
    return 'metadata' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['metadataProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async importProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.import-provider' }>) {
    ...
    return 'import' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['importProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async exportProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.export-provider' }>) {
    ...
    return 'export' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['exportProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
  async aiProvider(input: Extract<PluginManagerRequest, { type: 'plugin-manager.ai-provider' }>) {
    ...
    return 'ai' in response || response.ok === false
      ? response as Awaited<ReturnType<SuperPluginManagerApi['aiProvider']>>
      : { ok: false as const, code: 'operation-failed' as const };
  },
