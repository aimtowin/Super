...
import {
  ...
  resolveSuperMcpTool,
  type SuperMcpToolExposure,
} from './tool-catalog';

export type SuperMcpCallToolSuccess = {
  ok: true;
  toolName: string;
...
export type SuperMcpCallToolFailure = {
  ok: false;
  code:
    | 'MCP_TOOL_NOT_FOUND'
    ...
    | 'MCP_GATEWAY_FAILURE';
  message: string;
  /** Super review: structured error contract (§10) — retryable + version. */
  retryable?: boolean;
...
export type SuperMcpCallToolResult = SuperMcpCallToolSuccess | SuperMcpCallToolFailure;

export type SuperMcpPluginToolBridge = {
...
export type SuperMcpCallToolInput = {
  toolName: string;
  arguments: unknown;
  context: AutomationExecutionContext | undefined;
  exposure: SuperMcpToolExposure;
...
function outputLimitFor(input: SuperMcpCallToolInput): number {
...
export async function callSuperMcpTool(
  input: SuperMcpCallToolInput,
): Promise<SuperMcpCallToolResult> {
  const context = contextWithRequestSignal(input.context, input.signal);
  const executionId = context?.executionId;
  const tool = resolveSuperMcpTool(input.toolName, input.exposure);
  if (!tool) {
    ...
    if (!knownRegistryTool && !knownPluginTool) {
      return { ok: false, code: 'MCP_TOOL_NOT_FOUND', message: `Unknown Super MCP tool: ${input.toolName}` };
    }
    if (knownRegistryTool && !knownPluginTool) {
  ...
  if (libraryContext === 'active') {
    // Stateless contract (Super-8b5b): every library-scoped call carries its
    ...
    if (typeof rawRecord.libraryId !== 'string' || rawRecord.libraryId.trim().length === 0) {
      return {
        ok: false,
        code: 'MCP_LIBRARY_TARGET_REQUIRED',
        message: `MCP tool ${tool.name} requires an explicit libraryId. Use super_library_list_open or super_library_create first.`,
      };
    }
  ...
  if (!gatewayResult.ok) {
    // Super-8b5b.2: a dangerous call answered with a two-phase challenge is
    ...
    return {
      ok: false,
      code: gatewayError.code as SuperMcpCallToolFailure['code'],
      message: gatewayError.message ?? gatewayError.code,
      // Super review: structured error contract (§10) — transient codes are
