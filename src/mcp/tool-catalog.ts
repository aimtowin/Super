...
export type SuperMcpToolExposure = {
...
export function mcpExposureAllowsWrite(exposure: SuperMcpToolExposure): boolean {
...
export type SuperMcpToolDefinition = {
  name: string;
...
/**
 * Super-8b5b.2: dangerous tools accept the agent's second-phase confirmation
...
export function listSuperMcpTools(
  exposure: SuperMcpToolExposure,
): { apiVersion: typeof AUTOMATION_API_VERSION; tools: SuperMcpToolDefinition[] } {
  const tools: SuperMcpToolDefinition[] = [];
...
export function resolveSuperMcpTool(
  toolName: string,
  exposure: SuperMcpToolExposure,
): SuperMcpToolDefinition | undefined {
  return listSuperMcpTools(exposure).tools.find((tool) => tool.name === toolName);
}

export function automationLibraryContextForMcpTool(
  tool: SuperMcpToolDefinition,
): 'none' | 'active' | 'transition' {
