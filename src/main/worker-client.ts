...
// Super-2cc492：开库宽限窗口——SMB 冷缓存首读单条查询实测 20-35s，窗口内
...
export class LibraryWorkerClient {
  ...
  async #startAttempt(attempt: number): Promise<void> {
    const child = utilityProcess.fork(this.#modulePath, [], {
      serviceName: 'Super Library Worker',
      stdio: 'pipe',
  ...
  /** Super-8ca259: worker asks Main to capture an HTML document thumbnail. */
  ...
  /**
   * Handle a worker `document-thumbnail.render-request` (Super-8ca259):
  ...
  readonly #onMessage = (message: unknown) => {
    ...
    // Super-8ca259: HTML document thumbnail capture, same request/response
