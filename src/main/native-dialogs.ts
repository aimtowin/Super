/**
 * Main-process native open/save dialog helpers (Super-bwb).
 *
...
export async function selectImportSources(
  ...
  if (host.isE2e()) {
    const value =
      sourceKind === "files"
        ? e2eEnv.SUPER_E2E_IMPORT_FILES
        : e2eEnv.SUPER_E2E_IMPORT_FOLDER;
...
export async function selectLibraryDirectory(
  ...
  if (host.isE2e()) {
    return dialogId === "createLibrary"
      ? e2eEnv.SUPER_E2E_CREATE_PARENT_PATH
      : e2eEnv.SUPER_E2E_OPEN_LIBRARY_PATH;
  }
