...
import { createSuperMcpServer, type SuperMcpSessionEvent } from '../mcp/create-super-mcp-server';
import type { SuperMcpPluginToolBridge } from '../mcp/call-tool';
import {
  MCP_ENDPOINT_PATH,
  MCP_MAX_PORT,
  MCP_MIN_PORT,
...
export class EmbeddedMcpServer {
  ...
  readonly #getPluginTools: () => SuperMcpPluginToolBridge | undefined;
  ...
  public async stop(): Promise<McpSettingsSnapshot> {
    ...
    // Super-8b5b.2: a stopped server must not leave confirmable challenges.
  ...
  public async revokeCredential(credentialId: string): Promise<McpSettingsSnapshot> {
    ...
    // Super-8b5b.2: revoked credentials must not be able to confirm
            ...
            : code === 'MCP_CLIENT_UNAUTHORIZED' || code === 'MCP_CLIENT_REVOKED'
              ...
              : code === 'MCP_SESSION_NOT_FOUND'
                ...
                : code === 'MCP_SESSION_CLOSED'
                  ? 410
                  : 500;
        // Super review: never leak raw internal error messages (they may
  ...
  async #handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (!allowedHost(request.headers.host, port) || !allowedOrigin(request.headers.origin, port)) {
      writeJson(response, 403, {
        ...
        error: 'MCP requests must originate from the Super loopback endpoint.',
      });
      return;
    }
    ...
    if (session !== undefined) {
      ...
      // Super review: the 'close' listener must stay attached until the
  ...
  async #closeSession(sessionId: string): Promise<void> {
    ...
    // Super review: endSession matches on the record's sessionId, not the
  ...
  #notifySessions(event: SuperMcpSessionEvent): void {
