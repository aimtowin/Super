...
import { superProtocolSchemes } from "./super-protocol-privileges";
import {
...
const hasExplicitUserDataDirectory = process.argv.some((argument) =>
  argument.startsWith("--user-data-dir"),
);

// Super is a display/distribution rename. Keep using the prior user-data
// location for normal upgrades so existing libraries, preferences, and local
// credentials remain available instead of appearing to vanish after rename.
if (process.env.SUPER_E2E !== "1" && !hasExplicitUserDataDirectory) {
  app.setPath("userData", path.join(app.getPath("appData"), "Super"));
}

if (process.env.SUPER_E2E === "1") {
  const explicitUserDataPath = process.env.SUPER_E2E_USER_DATA_PATH;
  app.setPath(
    "userData",
    explicitUserDataPath && path.isAbsolute(explicitUserDataPath)
      ? explicitUserDataPath
      : path.join(tmpdir(), "super-e2e-user-data", String(process.pid)),
  );
}

// Dev multi-instance (Super-i6xg): isolate userData so SingletonLock / prefs
...
const allowMultiInstance = process.env.SUPER_ALLOW_MULTI_INSTANCE === "1";
if (allowMultiInstance && process.env.SUPER_E2E !== "1") {
  app.setPath(
    "userData",
...
// over super:// Range responses (Super-jh2).
protocol.registerSchemesAsPrivileged(superProtocolSchemes());
...
/** 自动同步调度器（Super-bfsb 后续），随 Worker 生命周期启停。 */
...
/**
 * Super-1e3d4f: Chromium never persists custom-protocol responses on disk,
...
function initializePreviewCache(): void {
  if (process.env.SUPER_PREVIEW_CACHE_DISABLED === "1") return;
  ...
  if (process.env.SUPER_E2E === "1" && process.env.SUPER_PREVIEW_CACHE_FORCE !== "1") {
    return;
  }
  const budgetBytes = Number(process.env.SUPER_PREVIEW_CACHE_BUDGET_BYTES ?? "");
  const cacheLogEnabled = process.env.SUPER_PREVIEW_CACHE_LOG === "1";
...
/** Slice E: shared offscreen window that renders model thumbnails (Super-hnmg). */
...
// A tray-hidden window has no user-visible benefit from thumbnail backfill,
// open reconciliation, or automatic AI processing. Keep a short foreground
// grace period so the browse request after restore always wins the Worker.
const BACKGROUND_WORK_RESUME_GRACE_MS = 1_500;
let backgroundWorkResumeTimer: ReturnType<typeof setTimeout> | undefined;

function pauseBackgroundWorkForHiddenWindow(): void {
  if (backgroundWorkResumeTimer !== undefined) {
    clearTimeout(backgroundWorkResumeTimer);
    backgroundWorkResumeTimer = undefined;
  }
  aiQueueScheduler.setPaused(true);
  workerClient?.setBackgroundMode('paused');
}

function resumeBackgroundWorkAfterForeground(window: BrowserWindow): void {
  if (window.isDestroyed() || window.isMinimized() || !window.isVisible()) return;
  if (backgroundWorkResumeTimer !== undefined) clearTimeout(backgroundWorkResumeTimer);
  backgroundWorkResumeTimer = setTimeout(() => {
    backgroundWorkResumeTimer = undefined;
    if (window.isDestroyed() || window.isMinimized() || !window.isVisible()) return;
    workerClient?.setBackgroundMode('active');
    aiQueueScheduler.setPaused(false);
  }, BACKGROUND_WORK_RESUME_GRACE_MS);
}
...
>();
// Last Super window that published browse context or received OS focus.
...
interface SyncBindingRecord {
  ...
  /** 旧格式字段：subPath 曾是同步文件夹名（Super-xffq 早期）；读取时兼容。 */
  subPath?: string;
  /** 上次成功同步时间（ISO 字符串）。 */
  lastSyncedAt?: string;
...
function focusSuperWindow(windowId?: number): boolean {
  const target =
    windowId === undefined
      ? mainWindow
...
/**
 * Isolated-session placement for `SUPER_E2E_ISOLATED=1`: real E2E must keep
...
function resolveE2eIsolatedPlacement(
  ...
  if (process.env.SUPER_E2E_ISOLATED !== "1") return undefined;
  ...
  logger?.info(
    "e2e.isolated-window",
    "SUPER_E2E_ISOLATED=1 was set but no secondary display was detected; " +
      "falling back to the primary display. The E2E window will steal " +
      "foreground focus on this machine (documented limitation of Super-a1b).",
...
async function createMainWindow(): Promise<void> {
  ...
  // Super-tluf: the macOS dock otherwise shows the default Electron icon
  ...
  const window = new BrowserWindow({
    ...
    ...(process.platform === "darwin"
      ...
      : shouldUseFramelessTitleBar(process.platform)
        ? {
            // Super-znex: hide system title bar; renderer draws caption buttons.
  ...
  window.on("minimize", () => pauseBackgroundWorkForHiddenWindow());
  window.on("hide", () => pauseBackgroundWorkForHiddenWindow());
  window.on("restore", () => resumeBackgroundWorkAfterForeground(window));
  window.on("show", () => resumeBackgroundWorkAfterForeground(window));
  ...
  window.webContents.session.on("will-download", (event, item) => {
    if (item.getURL().startsWith("super-plugin:")) event.preventDefault();
  });
  ...
  window.on("focus", () => {
    publishWindowFocus();
    resumeBackgroundWorkAfterForeground(window);
  });
  window.on("blur", () => {
  ...
  if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
    window.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedURL) => {
      ...
      dialog.showErrorBox("Super renderer failed to load", detail);
    });
    try {
  ...
  if (!saveContext) {
    logger?.info(
      ...
      focusedWindow
        ...
        : "No focused Super window and no fallback browse context; dropping save intent.",
      {
        focusedWindowId: focusedWindow?.id ?? null,
        lastTargetWindowId: lastExtensionTargetWindowId ?? null,
        mainWindowId:
...
async function handleSaveUpload(
  ...
  if (!saveContext) {
    logger?.info(
      ...
      focusedWindow
        ...
        : "No focused Super window and no fallback browse context; dropping upload.",
      {
        focusedWindowId: focusedWindow?.id ?? null,
        lastTargetWindowId: lastExtensionTargetWindowId ?? null,
        mainWindowId:
  ...
  try {
    ...
    if (upload.focusAppAfterSave) {
      focusSuperWindow(saveRouting.targetWindowId ?? undefined);
    }
  ...
  if (result.type === "library.opened") {
    return parseRendererResult({
      ...
      library: {
        ...
        // Super-033e: read-only degrade for newer-schema libraries.
        ...
        // Super-verg.5: read-only because the migration is stuck.
...
/**
 * Super-v4jf/Super-29125f: how many sorted-list-head assets to prime
...
function createNativeDialogHost(): NativeDialogHost {
  return {
    ...
    isE2e: () => !app.isPackaged && process.env.SUPER_E2E === "1",
  };
}
...
async function selectAutomationScriptToOpen(): Promise<string | undefined> {
  return selectOpenFile(
    ...
    process.env.SUPER_E2E_OPEN_AUTOMATION_SCRIPT,
    [{ name: 'Super scripts', extensions: ['superApi.js', 'superApi.ts'] }],
  );
}
...
async function selectAutomationScriptToSave(): Promise<string | undefined> {
  return selectSavePath(
    ...
    process.env.SUPER_E2E_SAVE_AUTOMATION_SCRIPT,
    {
      defaultPath: 'Untitled.superApi.ts',
      filters: [{ name: 'Super scripts', extensions: ['superApi.js', 'superApi.ts'] }],
    },
  );
}
...
async function selectPluginPackage(): Promise<string | undefined> {
  ...
  if (!app.isPackaged && process.env.SUPER_E2E === '1') {
    const e2ePackage = process.env.SUPER_E2E_PLUGIN_PACKAGE;
    return e2ePackage && path.isAbsolute(e2ePackage) ? e2ePackage : undefined;
  }
  const result = mainWindow
    ? await dialog.showOpenDialog(mainWindow, {
      title: 'Install a Super plugin',
      buttonLabel: 'Choose plugin',
      properties: ['openFile', 'openDirectory'],
      filters: [{ name: 'Super plugin package', extensions: ['zip'] }],
    })
...
/** Super-xffq：每台设备的稳定同步身份（userData 持久化，非敏感）。 */
  ...
  switch (request.type) {
    ...
    case "library.inspect-eagle.request": {
      ...
      const selectedSourcePath = await selectOpenLibrarySource(
        createNativeDialogHost(),
        "openEagleLibrary",
        process.env.SUPER_E2E_OPEN_EAGLE_LIBRARY,
    ...
    case "library.inspect-billfish.request": {
      ...
      const selectedSourcePath = await selectOpenFile(
        createNativeDialogHost(),
        "openBillfishLibrary",
        process.env.SUPER_E2E_OPEN_BILLFISH_LIBRARY,
    ...
    case "library.open-eagle.request": {
      ...
      const selectedParentPath = await selectOpenDirectory(
        ...
        process.env.SUPER_E2E_OPEN_EAGLE_PARENT,
    ...
    case "library.open-billfish.request": {
      ...
      const selectedParentPath = await selectOpenDirectory(
        createNativeDialogHost(),
        "openEagleLibraryDestination",
        process.env.SUPER_E2E_OPEN_BILLFISH_PARENT,
        { createDirectory: true },
      );
      return selectedParentPath
        ? {
    ...
    case "library.forget-recent.request":
      ...
      // forget-recent only mutates the Main store (Super-ucx).
      return undefined;
      ...
      return sourcePaths
        ? {
            ...
            expandImageSequences:
              !app.isPackaged && process.env.SUPER_E2E === "1",
    ...
    case "asset.import-eagle.request": {
      const selectedSourcePath = await selectOpenLibrarySource(
        createNativeDialogHost(),
        "importEagleLibrary",
        process.env.SUPER_E2E_IMPORT_EAGLE_LIBRARY,
        ["zip", "eaglepack", "rar", "7z", "tar", "gz", "tgz", "bz2", "tbz", "tbz2", "xz", "txz"],
      );
      if (!selectedSourcePath) return undefined;
      const materialized = await materializeExternalLibrarySource({
    ...
    case "asset.import-billfish.request": {
      const selectedSourcePath = await selectOpenFile(
        ...
        process.env.SUPER_E2E_IMPORT_BILLFISH_LIBRARY,
        [{ name: "Billfish Pack", extensions: ["billfishpack"] }],
      );
      if (!selectedSourcePath) return undefined;
      const materialized = await materializeExternalLibrarySource({
    ...
    case "asset.import-linked.request": {
      const sourceRootPath = await selectOpenDirectory(
        createNativeDialogHost(),
        "linkFolder",
        process.env.SUPER_E2E_LINKED_SOURCE,
      );
      return sourceRootPath
        ? {
    ...
    case "linked-folder.relink.request": {
      const newRootPath = await selectOpenDirectory(
        createNativeDialogHost(),
        "relinkFolder",
        process.env.SUPER_E2E_LINKED_NEW_ROOT,
      );
      return newRootPath
        ? {
    ...
    case "collection.linked-folder.add.request":
      return {
        type: "collection.linked-folder.add",
        libraryId: request.libraryId,
        collectionId: request.collectionId,
        linkedFolderId: request.linkedFolderId,
        relativePath: request.relativePath,
    ...
    case "collection.source-browse-entries.request":
      return {
        type: "collection.source-browse-entries",
        libraryId: request.libraryId,
        collectionId: request.collectionId,
        recursive: request.recursive,
        parentFolderId: request.parentFolderId,
    ...
    case "asset.relink.request": {
      const newAbsolutePath = await selectOpenFile(
        createNativeDialogHost(),
        "locateMissingAsset",
        process.env.SUPER_E2E_RELINK_FILE,
      );
    ...
    case "asset.relink-batch.request": {
      const newRootPath = await selectOpenDirectory(
        createNativeDialogHost(),
        "selectRelinkRoot",
        process.env.SUPER_E2E_RELINK_ROOT,
      );
      if (newRootPath) {
        return {
          type: "asset.relink-batch.preview",
    ...
    case "library.export.request": {
      ...
      const defaultExportName = libraryExportDefaultName(
        request.libraryName ?? "super-library-export",
        request.format,
      );
      ...
      const destinationPath =
        request.format === "zip"
          ? await selectSavePath(
              host,
              "exportZip",
              process.env.SUPER_E2E_EXPORT_DEST_ZIP,
              {
          ...
          : await selectSavePath(
              host,
              "exportFolder",
              process.env.SUPER_E2E_EXPORT_DEST,
              { defaultPath: defaultExportPath },
            );
    ...
    case "library.import.request": {
      const sourceFolderPath = await selectOpenDirectory(
        createNativeDialogHost(),
        "importLibraryFolder",
        process.env.SUPER_E2E_IMPORT_SOURCE,
      );
    ...
    case "library.import-zip.request": {
      ...
      const sourceZipPath = await selectOpenFile(
        host,
        "importZip",
        process.env.SUPER_E2E_IMPORT_SOURCE_ZIP,
      ...
      const destinationParentPath = await selectOpenDirectory(
        ...
        process.env.SUPER_E2E_IMPORT_COPY_PARENT,
        { createDirectory: true },
      );
    ...
    case "sync.open-remote-library.request": {
      ...
      const selectedParentPath = await selectOpenDirectory(
        ...
        process.env.SUPER_E2E_OPEN_SYNC_LIBRARY_PARENT,
        { createDirectory: true },
      );
    ...
    case "model.resolve-companions.request":
      // Slice C (Super-qvc6): 3D viewer companion-texture index. The worker
    ...
    case "asset.preview.request":
      ...
      // a super:// URL after the Worker lookup.
      return {
        type: "media.get-preview-artifact",
        libraryId: request.libraryId,
        assetId: request.assetId,
...
async function handleLibraryRequest(input: unknown): Promise<RendererResult> {
  ...
  try {
    ...
    if (request.type === "library.delete-from-disk.request") {
      // Drop super:// file handles before the Worker tries to rm the root.
    ...
    // Super-xffq: 同步服务器与库绑定是 Main 本地配置（凭据经 safeStorage）。
    ...
    if (request.type === "sync.servers.upsert.request") {
      ...
      // Super-fatf: 保存时即校验/规范化 URL（缺协议自动补 http://），
    ...
    if (request.type === "sync.library.binding.save.request") {
      ...
      // Super-7405ef: 保存绑定（含开启自动同步）后立即触发一次同步，
    ...
    if (request.type === "library.open-recent.request") {
      ...
      try {
        image =
          !app.isPackaged &&
          process.env.SUPER_E2E === "1" &&
          process.env.SUPER_E2E_CLIPBOARD_IMAGE_PATH
            ? nativeImage.createFromBuffer(
                readFileSync(process.env.SUPER_E2E_CLIPBOARD_IMAGE_PATH),
              )
            : clipboard.readImage();
        const injectedNow =
          !app.isPackaged &&
          process.env.SUPER_E2E === "1" &&
          process.env.SUPER_E2E_CLIPBOARD_NOW
            ? new Date(process.env.SUPER_E2E_CLIPBOARD_NOW)
            : new Date();
      ...
      try {
        const injectedPaths =
          !app.isPackaged &&
          process.env.SUPER_E2E === "1" &&
          process.env.SUPER_E2E_CLIPBOARD_FILE_PATHS
            ? process.env.SUPER_E2E_CLIPBOARD_FILE_PATHS.split("\n").filter(
                Boolean,
              )
            : null;
        const sourcePaths =
    ...
    //
    // Super-v4jf: only the visible first screen (the sorted list head, i.e.
    ...
    if (workerResult.ok && workerResult.type === "library.opened") {
      ...
      // Super-140fe2 review: deleted libraries must not leave phantom
    ...
    // Super-xffq: 同步成功即记录绑定与上次同步时间，供“已同步”状态展示。
      ...
      const url =
        workerResult.status === "ready"
          ? workerResult.playbackMode === "source" &&
            ...
            ? `super://source/${request.libraryId}/${request.assetId}?revision=${encodeURIComponent(workerResult.sourceRevisionId)}`
            : workerResult.artifactId
              ? `super://${workerResult.playbackMode === "proxy" ? "proxy" : "preview"}/${request.libraryId}/${workerResult.artifactId}`
              : undefined
          : undefined;
      const posterUrl = workerResult.posterArtifactId
        ? `super://preview/${request.libraryId}/${workerResult.posterArtifactId}`
        : undefined;
      if (
    ...
    // the worker sees a temporary `super-external-library-*` directory.
    ...
    if (!result.ok) {
      if (operation) {
        ...
        // Super-s0oq: an invalid recent library (folder gone, corrupt, or
...
async function confirmDesktopAutomationWrite(): Promise<boolean> {
  ...
  if (!app.isPackaged
    && process.env.SUPER_E2E === '1'
    && process.env.SUPER_E2E_AUTOMATION_CONFIRM === '1') {
    return true;
  }
...
async function confirmDesktopAutomationFilePlan(
  ...
  if (!app.isPackaged
    && process.env.SUPER_E2E === '1'
    && process.env.SUPER_E2E_AUTOMATION_CANCEL_ONCE === '1') {
...
async function confirmCriticalLibraryDeletion(libraryId: string): Promise<boolean> {
  ...
  return manager.request({
    ...
    message: english
      ? `The library “${library.displayName}” and its Super-managed directory will be permanently deleted.`
      : `资源库“${library.displayName}”及其 Super 管理的目录将被永久删除。`,
    detail: english
...
async function confirmEnableAllMcpPermissions(label: string): Promise<boolean> {
  ...
  return manager.request({
    ...
    detail: english
      ? 'This applies only to this client credential. Super still enforces exact targets, path boundaries, version checks, idempotency and worker safety checks. Disable Full Access or revoke the credential to stop it immediately.'
      : '此设置只作用于当前客户端凭据。Super 仍会执行精确目标、路径边界、版本校验、幂等和 Worker 安全检查。关闭完全权限或删除凭据即可立即停止。',
    cancelLabel: english ? 'Cancel' : '取消',
    ...
    : operation === 'linked-folder'
      ...
      : operation === 'asset-permanent'
        ...
        : operation === 'trash-purge'
          ? (english ? 'Empty the Super trash permanently?' : '永久清空 Super 回收站？')
  ...
  const message = operation === 'folder'
    ...
    : operation === 'linked-folder'
      ...
      : operation === 'asset-permanent'
        ...
        : operation === 'trash-purge'
          ? (english ? 'All assets currently in the Super trash will be permanently deleted.' : 'Super 回收站中的全部资产将被永久删除。')
  ...
  automaticUpdateTimer = setTimeout(() => {
    ...
    void (async () => {
      ...
      if (
        ...
        || (checked.assetKind !== 'installer' && checked.assetKind !== 'delta-installer')
      ) {
        return;
      }
...
async function startApplication(): Promise<void> {
  ...
  // menu's first item shows "Super" from the very first frame.
  ...
  if (process.platform === "win32") {
    app.setAppUserModelId("com.superApi.app");
  }
  // Isolated E2E runs must not write the log into the real user profile
  // (~/Library/Logs/Super); pin it under the temp userData instead.
  if (process.env.SUPER_E2E === "1" && process.env.SUPER_E2E_USER_DATA_PATH) {
    app.setAppLogsPath(path.join(process.env.SUPER_E2E_USER_DATA_PATH, "logs"));
  } else {
    app.setAppLogsPath();
  }
    ...
    launchInstaller: async (installerPath, mode) => {
      if (process.platform === 'win32') {
        const args = mode === 'silent'
          ...
          : [];
        // “silent” only suppresses Inno's standard wizard. The user-approved
        // updater supplies its own foreground progress window, so hiding the
        // child process would also hide that window and leave Super closed.
        spawn(installerPath, args, { detached: true, stdio: 'ignore', windowsHide: false }).unref();
      } else {
  ...
  // Slice E (Super-hnmg): Main owns the shared offscreen window that renders
    ...
    createWindow: (options) => {
      ...
      offscreenWindow.webContents.on("did-finish-load", () => {
        ...
        void offscreenWindow.webContents
          .executeJavaScript(
            "({ readyState: document.readyState, body: document.body.innerHTML, hasBridge: Boolean(window.offscreenThumbnail), debug: window.__superOffscreenThumbnailDebug })",
          )
          .then((state) => {
            logger?.info(
  ...
  // Super-8ca259: HTML document thumbnails capture the source in a fresh
  ...
  automationCommandGateway = createAutomationCommandGateway(
    ...
    },
    {
      ...
      uiNotifyHandler: {
        notify: (input) => {
          if (!mainWindow || mainWindow.isDestroyed()) {
            throw new Error('The Super window is not available to show a notification.');
          }
          // Super review: ui.notify is strictly non-blocking (toast only);
      ...
      // Super-ihpx: automation imports (MCP/scripts) are no different from
  ...
  scriptRuntimeSupervisor = new ScriptRuntimeSupervisor({
    ...
    fork: (modulePath) => utilityProcess.fork(modulePath, [], {
      serviceName: 'Super Script Runtime',
      stdio: 'pipe',
    }),
    logger,
  });
  const executePluginHostCommand: PluginRuntimeHostCommandHandler = async (commandId, input, context) => {
    ...
    if (targetLibraryId === '__super_global_runtime__') {
      throw new Error('A global plugin must choose an open library with superApi.forLibrary().');
    }
    ...
    if (!result.ok) {
      // Super-8b5b.2: plugin host commands are never critical; a challenge
  ...
  pluginRuntimeSupervisor = new PluginRuntimeSupervisor({
    ...
    fork: (modulePath) => utilityProcess.fork(modulePath, [], {
      serviceName: 'Super Plugin Standard Host',
      stdio: 'pipe',
    }),
  ...
  pluginTrustedRuntimeSupervisor = new PluginTrustedRuntimeSupervisor({
    ...
    fork: (modulePath) => utilityProcess.fork(modulePath, [], {
      serviceName: 'Super Plugin Trusted Host',
      stdio: 'pipe',
    }),
    executeHostCommand: executePluginHostCommand,
    executeStorage: executePluginStorage,
  ...
  if (pluginCompatibility === undefined || !Number.isSafeInteger(nodeAbi) || nodeAbi <= 0) {
    ...
    pluginActivationCoordinator = new PluginActivationCoordinator({
      ...
      globalRuntimeContext: {
        libraryId: '__super_global_runtime__',
  ...
  try {
    embeddedMcpServer = new EmbeddedMcpServer({
      ...
      permissionBroker: mcpPermissionBroker,
      // Super-fmbr: MCP operations speak through the SAME human-facing toasts
    ...
    // Super review: an unrecoverable MCP construction failure (e.g. the
  ...
  // Super-bfsb 后续：自动同步调度器。打开同步资源库后自动绑定并开启
  ...
  // Register super:// custom protocol for serving thumbnail/preview artifacts.
  // The renderer uses super://preview/<libraryId>/<artifactId> URLs in <img> tags.
  // Main resolves the artifact path via Worker, reads the file, and returns bytes.
  protocol.handle("super", async (request) => {
    try {
      ...
      if (url.hostname === APP_ASSET_HOST) {
        ...
        if (!response) {
          logger?.info(
            "super-protocol.app-asset-missing",
      ...
      if (
        ...
        logger?.info(
          "super-protocol.invalid-host",
          "Rejected unsupported artifact protocol host.",
        );
        return new Response("Invalid super:// path", { status: 400 });
      }
      const parts = url.pathname.replace(/^\/+/, "").split("/");
      if (parts.length !== 2) {
        logger?.info(
          "super-protocol.invalid-path",
      ...
      if (
        ...
        logger?.info(
          "super-protocol.invalid-identifiers",
      ...
      if (!workerClient) {
        logger?.error(
          "super-protocol.worker-unavailable",
      ...
      if (url.hostname === "source") {
        ...
        if (!revisionId || !/^[A-Za-z0-9_-]{1,255}$/.test(revisionId)) {
          logger?.info(
            "super-protocol.invalid-revision",
          ...
          try {
            ...
            return await createArtifactResponse(
              ...
              authorizedSource.mimeType,
              {
                ...
                onStreamError: (error) =>
                  logger?.error("super-protocol.model-source-stream", error, {
                    libraryId,
                    assetId: artifactId,
                  }),
              },
            ...
            logger?.error("super-protocol.model-source-read", error, {
              libraryId,
              assetId: artifactId,
            });
        ...
        try {
          const response = await createArtifactResponse(
            ...
            sourceResult.mimeType,
            {
              ...
              signal: bindLibraryMediaReadSignal(libraryId, request.signal),
              onStreamError: (error) =>
                logger?.error("super-protocol.source-stream", error, {
                  libraryId,
                  assetId: artifactId,
                }),
            },
          ...
          sourcePathCache.delete(libraryId, artifactId, revisionId);
          logger?.error("super-protocol.source-read", error, {
            libraryId,
            assetId: artifactId,
          });
          return new Response("Source file missing", { status: 404 });
      ...
      // Super-1e3d4f: image previews are mirrored into userData on first
      ...
      try {
        const response = await createArtifactResponse(
          ...
          mimeType,
          {
            ...
            signal: bindLibraryMediaReadSignal(libraryId, request.signal),
            onStreamError: (error) =>
              logger?.error("super-protocol.stream", error, {
                libraryId,
                artifactId,
              }),
          },
        ...
        logger?.error("super-protocol.read", error, {
          libraryId,
          artifactId,
        });
      ...
      logger?.error("super-protocol", error, { url: request.url });
      return new Response("Internal error", { status: 500 });
    }
  });

  protocol.handle("super-plugin", async (request) => {
    ...
    try {
      ...
      const pluginOrigin = `super-plugin://${parsed.pluginId}`;
  ...
  ipcMain.handle(
    ...
    async (event): Promise<AppUpdateInstallResult> => {
      ...
      // The update handoff is intentional and user-approved. Hide the main
      // window before spawning Setup so the previous Super Lib UI never
      // lingers behind the foreground updater. Restore it only on failure.
  ...
  // Reveal superApi.log in the file manager without exposing the path to Renderer.
  ipcMain.handle(
    REVEAL_APP_LOG_CHANNEL,
  ...
  // Super-166q: text-field ⌘C fallback when Edit menu routes through renderer.
  ipcMain.handle(NATIVE_EDIT_COPY_CHANNEL, (event): void => {
    if (!mainWindow || event.sender !== mainWindow.webContents) return;
    event.sender.copy();
  });

  // Bootstrap dialog locale from OS before Renderer syncs (Super-bwb).
  ...
  // Super-q0b1: the renderer syncs native menu item enabled-state (business
  ...
  // View→Zoom accelerators that steal Cmd+=/-/0 (Super-46i9).
  // Windows: hides menu bar for frameless shell (Super-znex).
  installApplicationMenu({ locale: appLocale });
