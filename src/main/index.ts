 ...
 * the main process exits immediately after handing it off. Use the windowless
 * Windows Script Host as a last-mile cleanup fallback so an installer
 * directory that could not be removed before app.quit() is removed after setup
 * releases it. Do not use cmd.exe/ping here: they can flash a console window.
...
function scheduleWindowsInstallerCleanup(installerPath: string): void {
  ...
  const cleanupScriptPath = path.join(
    app.getPath('temp'),
    `super-update-cleanup-${randomUUID()}.vbs`,
  );
  const installerLiteral = quoteVbs(installerPath);
  const outputDirectoryLiteral = quoteVbs(outputDirectory);
  const script = [
    'On Error Resume Next',
    'Set fso = CreateObject("Scripting.FileSystemObject")',
    'For attempt = 1 To 30',
    '  WScript.Sleep 1000',
    `  fso.DeleteFile ${installerLiteral}, True`,
    `  fso.DeleteFolder ${outputDirectoryLiteral}, True`,
    `  If Not fso.FolderExists(${outputDirectoryLiteral}) Then Exit For`,
    'Next',
    'fso.DeleteFile WScript.ScriptFullName, True',
  ].join('\r\n');
  try {
    writeFileSync(cleanupScriptPath, script, { encoding: 'utf8', mode: 0o600 });
