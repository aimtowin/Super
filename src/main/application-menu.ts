...
/**
 * Super-q0b1: the template carries i18n catalog keys; the effective app
...
/**
 * Super-q0b1: the renderer drives item enabled-state (e.g. business undo is
 ...
 * (Super-q0b1). Commands with an explicit native accelerator retain it;
...
function enrichMenuTemplate(
  ...
  return items.map((item) => {
    ...
    if (item.command === "invert-selection" || item.command === "copy-selection") {
      // Super-166q/te8p: invert/copy keep their dedicated channels and
    ...
    if (item.submenu) {
      return {
        ...item,
        // Super review: keep hardcoded brand labels ("Super" app menu)
...
/**
 * Install Super's application menu (no page-zoom accelerator roles).
 * Call once during app ready so macOS defaults cannot steal
 * Cmd/Ctrl+=,-,0 from renderer zoom shortcuts (Super-46i9).
 *
 * Windows (Super-znex / Super-r7gu): hide the menu bar for frameless
 * shell unity, but keep a hidden accelerator menu for F2/Delete (Super-g8u9)
...
export function installApplicationMenu(options?: {
  ...
  const showDevTools =
    options?.showDevTools ?? (!app.isPackaged || process.env.SUPER_E2E === "1");
