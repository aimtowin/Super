...
export function mediaBinaryWorkerEnv(
  ...
  if (env.SUPER_FFMPEG_PATH === undefined) {
    const ffmpeg = firstRunnable(resourceCandidates("ffmpeg", "ffmpeg"));
    if (ffmpeg) env.SUPER_FFMPEG_PATH = ffmpeg;
  }
  if (env.SUPER_OIIO_PATH === undefined) {
    const oiiotool = firstRunnable(resourceCandidates("oiio", "oiiotool"));
    if (oiiotool) env.SUPER_OIIO_PATH = oiiotool;
  }
  // Ensure worker can still find sibling tools next to an env override.
  if (env.SUPER_FFMPEG_PATH && existsSync(env.SUPER_FFMPEG_PATH)) {
    const dir = path.dirname(env.SUPER_FFMPEG_PATH);
