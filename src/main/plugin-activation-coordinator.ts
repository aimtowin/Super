...
const DEFAULT_GLOBAL_RUNTIME_CONTEXT = {
  ...
  libraryDirectory: '__super_global_runtime__',
} as const;
...
export class PluginActivationCoordinator {
    ...
    for (const [pluginId, candidate] of desired) {
      ...
      try {
        ...
        try {
          ...
          appendFileSync(
            join(
              process.env.SUPER_E2E_USER_DATA_PATH
                ?? join(process.env.HOME ?? '/tmp', 'Library/Application Support/Super'),
  ...
  resolvePluginUiAsset(input: {
    ...
    // Settings / sidebar / inspector / viewer iframes share super-plugin:// with workspace views.
    const contribution = [
