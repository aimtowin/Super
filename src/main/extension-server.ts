...
/** Metadata for browser-fetched binary uploads (Super-1jyi). */
...
function isAllowedOrigin(origin: string | string[] | undefined): boolean {
  ...
  // - Firefox:  moz-extension://<UUID> (Super-54122a)
  // Auth is loopback-only (product decision: no pairing token — Super-1cxv).
...
const UPLOAD_STAGE_PREFIX = 'super-ext-upload-';
...
function parseUploadMetadata(headers: http.IncomingHttpHeaders):
  ...
  const kindRaw = headerValue(headers, 'x-super-kind');
  const sourcePageUrl = decodeHeaderText(headerValue(headers, 'x-super-source-page-url'));
  const mediaUrl = decodeHeaderText(headerValue(headers, 'x-super-media-url'));
  const localFileUpload = headerValue(headers, 'x-super-local-file') === 'true';
  const contentType = normalizeContentType(
    headerValue(headers, 'x-super-content-type') ?? headerValue(headers, 'content-type'),
  );
  const filename = decodeHeaderText(headerValue(headers, 'x-super-filename')) ?? 'download';
  const targetRaw = headerValue(headers, 'x-super-target-folder-id');
  ...
  if (!contentType || contentType === 'application/octet-stream') {
    ...
    if (!headerValue(headers, 'x-super-content-type')) {
  ...
  return {
    ...
    focusAppAfterSave: parseBooleanHeader(
      headerValue(headers, 'x-super-focus-app-after-save'),
      true,
    ),
    revealInLibrary: parseBooleanHeader(
      headerValue(headers, 'x-super-reveal-in-library'),
      true,
    ),
  };
}
 ...
 * (Super-1cxv): identity is loopback + optional chrome-extension Origin.
 */
export async function createExtensionServer(
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/ping') {
      jsonResponse(res, 200, { app: 'Super' });
      return;
    }
