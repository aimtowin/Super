...
export async function inspectPluginDirectory(
  ...
  await visit('');
  const manifestPath = safeContainedPath(directory, 'super-plugin.json');
  let manifestBytes: Buffer;
  try {
    manifestBytes = await readFile(manifestPath);
  } catch {
    throw new PluginPackageManagerError('PLUGIN_SOURCE_READ_FAILED', 'The plugin package must contain super-plugin.json.');
  }
