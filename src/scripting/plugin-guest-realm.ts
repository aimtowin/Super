...
/**
 * Injects `superApi.events.on` and `superApi.hooks.onWill` as guest JS over host
...
export function buildPluginSetupSource(entryJavaScript: string, context: {
  ...
  return [
    ...
    '}',
    'if (superApi.events && typeof superApi.events.next === "function") {',
    '  superApi.events.on = function(kind, handler) {',
    '    void (async function() {',
    '      for (;;) {',
    '        const event = await superApi.events.next();',
    '        if (event === null) return;',
    '        if (kind !== "*" && event.kind !== kind) continue;',
    '        const chain = (event.causeChain || []).concat([event.eventId]);',
    '        if (typeof superApi.events.__setCause === "function") superApi.events.__setCause(chain);',
    '        try {',
    '          await handler(event);',
    '        } finally {',
    '          if (typeof superApi.events.__setCause === "function") superApi.events.__setCause([]);',
    '        }',
    '      }',
    '    })();',
    '  };',
    '}',
    'if (superApi.hooks && typeof superApi.hooks.__nextInvoke === "function") {',
    '  const __hookHandlers = Object.create(null);',
    '  superApi.hooks.onWill = function(event, handler) {',
    '    __hookHandlers[String(event)] = handler;',
    '  };',
    '  void (async function() {',
    '    for (;;) {',
    '      const invoke = await superApi.hooks.__nextInvoke();',
    ...
    '        }',
    '      }',
    '      await superApi.hooks.__respond(invoke.invokeId, decision);',
    '    }',
    '  })();',
    '}',
    'if (superApi.jobs && typeof superApi.jobs.__nextJob === "function") {',
    '  const __jobHandlers = Object.create(null);',
    '  superApi.jobs.registerHandler = function(id, handler) {',
    '    __jobHandlers[String(id)] = handler;',
    '  };',
    '  superApi.jobs.enqueue = function(input) {',
    '    const request = input && typeof input === "object" ? input : {};',
    '    return superApi.jobs.__enqueue({',
    '      handlerId: String(request.handlerId ?? ""),',
    '      payload: request.payload ?? {},',
    '      recoveryStrategy: request.recoveryStrategy,',
    '    });',
    '  };',
    '  superApi.jobs.reportProgress = function(input) {',
    '    const request = input && typeof input === "object" ? input : {};',
    ...
    '    return superApi.jobs.__reportProgress({',
    ...
    '    });',
    '  };',
    '  superApi.jobs.cancel = function(input) { return superApi.jobs.__control({ ...(input || {}), action: "cancel" }); };',
    '  superApi.jobs.pause = function(input) { return superApi.jobs.__control({ ...(input || {}), action: "pause" }); };',
    '  superApi.jobs.resume = function(input) { return superApi.jobs.__control({ ...(input || {}), action: "resume" }); };',
    '  superApi.jobs.retry = function(input) { return superApi.jobs.__control({ ...(input || {}), action: "retry" }); };',
    '  const __jobSignal = function(jobId) {',
    '    const signal = {};',
    '    Object.defineProperty(signal, "aborted", { enumerable: true, get: function() { return superApi.jobs.__isAborted(String(jobId)); } });',
    ...
    '  void (async function() {',
    '    for (;;) {',
    '      const job = await superApi.jobs.__nextJob();',
    '      if (job === null) return;',
    ...
    '        }',
    '      }',
    '      await superApi.jobs.__respond(job.jobId, {',
    '        status,',
    '        ...(errorCode ? { errorCode } : {}),',
    ...
    '  })();',
    '}',
    'if (superApi.providers && typeof superApi.providers.__nextInvoke === "function") {',
    '  const __providerHandlers = Object.create(null);',
    '  superApi.providers.register = function(kind, provider) {',
    ...
    '  void (async function() {',
    '    for (;;) {',
    '      const invoke = await superApi.providers.__nextInvoke();',
    '      if (invoke === null) return;',
    ...
    '        }',
    '      }',
    '      await superApi.providers.__respond(invoke.invokeId, result);',
    '    }',
    '  })();',
    '}',
    'if (superApi.providers && typeof superApi.providers.__nextSearchEvent === "function") {',
    '  const __searchHandlers = Object.create(null);',
    '  const __searchSignals = Object.create(null);',
    '  superApi.providers.registerSearch = function(provider) {',
    ...
    '      await superApi.providers.__respondSearchChunk({',
    '        invokeId,',
    ...
    '      await superApi.providers.__respondSearchComplete({',
    ...
    '  void (async function() {',
    '    for (;;) {',
    '      const event = await superApi.providers.__nextSearchEvent();',
    '      if (event === null) return;',
    ...
    '  })();',
    '}',
    'if (superApi.commands && typeof superApi.commands.__nextCommand === "function") {',
    '  const __commandHandlers = Object.create(null);',
    '  superApi.commands.register = function(id, handler) {',
    '    __commandHandlers[String(id)] = handler;',
    '  };',
    '  void (async function() {',
    '    for (;;) {',
    '      const invoke = await superApi.commands.__nextCommand();',
    '      if (invoke === null) return;',
    ...
    '        }',
    '      }',
    '      await superApi.commands.__respond(invoke.invokeId, {',
    '        status,',
    '        ...(errorCode ? { errorCode } : {}),',
    '        ...(errorDetail ? { errorDetail } : {}),',
    '      });',
    ...
    '  })();',
    '}',
    'if (superApi.input && typeof superApi.input.__start === "function") {',
    '  superApi.input.capture = function(options) {',
    '    return superApi.input.__start(options || {}).then(function(session) {',
    '      const events = {',
    '        next: function() { return superApi.input.__nextEvent(session.sessionId); },',
    '      };',
    ...
    '        events: events,',
    '        release: function() { return superApi.input.__release(session.sessionId); },',
    '      };',
    '    });',
    '  };',
    '}',
    ...
    '};',
    'const signal = {};',
    'Object.defineProperty(signal, "aborted", { enumerable: true, get: function() { return typeof superApi.__isDeactivated === "function" && superApi.__isDeactivated(); } });',
    'signal.addEventListener = function(type, listener) {',
    '  if (type !== "abort" || typeof listener !== "function" || typeof superApi.__waitUntilDeactivate !== "function") return;',
    '  void superApi.__waitUntilDeactivate().then(function() { try { listener({ type: "abort", target: signal }); } catch (_error) {} });',
    '};',
    ...
    'try {',
    `  await setup(Object.assign({}, superApi, { pluginId: ${JSON.stringify(context.pluginId)}, pluginInstanceId: ${JSON.stringify(context.pluginInstanceId)}, installationScope: ${JSON.stringify(context.installationScope)}, instanceScope: { kind: ${JSON.stringify(context.instanceScope)} }, subscriptions: subscriptions, signal: signal, super: superApi }));`,
    '  if (typeof superApi.__waitUntilDeactivate === "function") {',
    '    await superApi.__waitUntilDeactivate();',
    '  }',
    '  if (typeof dispose === "function") {',
    '    await dispose(typeof superApi.__getDeactivateReason === "function" ? superApi.__getDeactivateReason() : undefined);',
    '  }',
    '} finally {',
