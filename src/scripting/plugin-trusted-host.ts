...
function createSuperBridge(
  ...
  const events = {
    ...
    on: (kind: unknown, handler: unknown): void => {
      if (typeof handler !== 'function') {
        throw new Error('superApi.events.on requires a handler function.');
      }
  ...
  const hooks = {
    onWill: (event: unknown, handler: unknown): void => {
      if (typeof handler !== 'function') {
        throw new Error('superApi.hooks.onWill requires a handler function.');
      }
  ...
  const createJobs = (targetLibraryId?: string) => {
    ...
    return {
      ...
      registerHandler: (id: unknown, handler: unknown): void => {
        if (typeof handler !== 'function') {
          throw new Error('superApi.jobs.registerHandler requires a handler function.');
        }
  ...
  const providers = {
    register: (kind: unknown, provider: unknown): void => {
      if (typeof provider !== 'object' || provider === null
        ...
        throw new Error('superApi.providers.register requires { id, compute }.');
      }
    ...
    registerSearch: (provider: unknown): void => {
      if (typeof provider !== 'object' || provider === null
        ...
        throw new Error('superApi.providers.registerSearch requires { id, search }.');
      }
  ...
  const commands = {
    register: (id: unknown, handler: unknown): void => {
      if (typeof handler !== 'function') {
        throw new Error('superApi.commands.register requires a handler function.');
      }
  ...
  const guestApi = createSuperGuestApi({ executeCommand: callHost });
  return {
    ...guestApi,
    ...
    forLibrary: (libraryId: string): Record<string, unknown> => {
      ...
      return {
        ...createSuperGuestApi({ executeCommand: callHost }, parsed.data),
...
export function createPluginTrustedHostHandler(options: {
  ...
  const activate = async (
    ...
    try {
      ...
      const superApi = createSuperBridge(active, options.postMessage);
      await exported.setup?.(Object.assign({}, superApi, {
        pluginId: request.pluginId,
