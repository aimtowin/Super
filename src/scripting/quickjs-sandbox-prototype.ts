...
import {
  SUPER_GUEST_COMMANDS,
} from './super-guest-api';
 ...
 * filesystem, network, or database access. The real `superApi` API belongs to
...
function assertSourceIsAllowed(source: string, fileName: string): void {
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isImportDeclaration(node) || ts.isImportEqualsDeclaration(node) || ts.isExportDeclaration(node)) {
      rejected = 'Modules are not available in Super scripts.';
      return;
    }
    if (
      ...
      rejected = 'Dynamic import is not available in Super scripts.';
      return;
    }
    if (
      ts.isCallExpression(node) &&
      ...
      rejected = 'Dynamic code construction is not available in Super scripts.';
      return;
    }
    if (ts.isIdentifier(node) && node.text === 'globalThis') {
      rejected = 'Global-object reflection is not available in Super scripts.';
      return;
    }
    if (
      ...
      rejected = 'Async generators are not available in Super scripts.';
      return;
    }
 ...
 */
export function transpileQuickJsSandboxPrototypeSource(source: string, fileName = 'script.super.ts'): string {
...
const PENDING_PROMISE_LIMIT_MARKER = '__SUPER_PENDING_PROMISE_LIMIT__';
...
function privateIdentifier(prefix: string): string {
  ...
  return `__super_${prefix}_${entropy[0]!.toString(36)}${entropy[1]!.toString(36)}${entropy[2]!.toString(36)}`;
}
...
export async function runQuickJsSandboxPrototype(
  ...
  try {
    ...
    const superApi = context.newObject();
    ...
    if (host.readText !== undefined) {
      ...
      context.setProp(superApi, 'readText', readText);
      readText.dispose();
    }
    if (host.waitUntilDeactivate !== undefined) {
      ...
      context.setProp(superApi, '__waitUntilDeactivate', waitUntilDeactivate);
    ...
    if (host.getDeactivateReason !== undefined) {
      ...
      context.setProp(superApi, '__getDeactivateReason', getDeactivateReason);
    ...
    if (host.signal !== undefined) {
      ...
      context.setProp(superApi, '__isDeactivated', isDeactivated);
    ...
    if (host.waitForDomainEvent !== undefined) {
      ...
      context.setProp(superApi, 'events', events);
      events.dispose();
    }
    if (host.waitForHookInvoke !== undefined && host.respondHookDecision !== undefined) {
      ...
      respond.dispose();
      context.setProp(superApi, 'hooks', hooks);
      hooks.dispose();
    }
    if (
      ...
      context.setProp(superApi, 'jobs', jobs);
      jobs.dispose();
    ...
    if (host.waitForCommandInvoke !== undefined && host.respondCommandComplete !== undefined) {
      ...
      context.setProp(superApi, 'commands', commands);
      commands.dispose();
    }
    if (host.executeStorageOperation !== undefined) {
      ...
      context.setProp(superApi, 'storage', storage);
      get.dispose();
      set.dispose();
      deleteKey.dispose();
      listKeys.dispose();
      ...
      context.setProp(superApi, 'data', data);
    ...
    if (
      ...
      release.dispose();
      nextEvent.dispose();
      context.setProp(superApi, 'input', input);
      input.dispose();
    }
    if (host.executeAutomationCommand !== undefined) {
      ...
      const createGuestCommandApi = (targetLibraryId?: string): QuickJSHandle => {
        ...
        try {
          for (const definition of SUPER_GUEST_COMMANDS) {
      ...
      for (const namespace of ['assets', 'library', 'folders', 'tags', 'collections', 'smartCollections', 'linkedFolders', 'files', 'trash', 'palettes', 'ui']) {
        ...
        try {
          context.setProp(superApi, namespace, target);
        } finally {
          target.dispose();
        }
      }
      const forLibrary = context.newFunction('forLibrary', (libraryIdHandle) => {
        ...
        if (!parsed.success || parsed.data === '__super_global_runtime__') {
          throw new Error('Invalid target library id.');
        }
        return createGuestCommandApi(parsed.data);
      });
      context.setProp(superApi, 'forLibrary', forLibrary);
    ...
    context.setProp(context.global, 'superApi', superApi);
