...
export interface CanvasPreferences {
  ...
  readonly fields: {
    ...
    /** Grid tile captions: width × height when metadata exists (Super-rzqj). */
    readonly dimensions: boolean;
    /** GIF / VIDEO / TEXT type chip (Super-cs1). */
    readonly badgeType: boolean;
    readonly badgeDuration: boolean;
    readonly badgeSource: boolean;
    /** Non-image extension chip, bottom-left (Super-cs1). */
    readonly badgeExtension: boolean;
  };
  /** Audio assets play in-place on hover (Super hover 音频). */
...
// Super-akz originally used this as the range-input step (2px). Super-7ny
...
export const PREF_KEY = 'superApi.canvas-prefs.v1';
const LEGACY_VIEW_MODE_KEY = 'superApi.asset-view-mode';
const LEGACY_CARD_SIZE_KEY = 'superApi.asset-card-size';
 ...
 * range at a given step (Super-akz). Exported so a regression test can
...
/**
 * Attempt to migrate from the legacy flat keys (`superApi.asset-view-mode` and
 * `superApi.asset-card-size`).  Returns `undefined` when neither key is present
 ...
 *   legacy pair `superApi.asset-view-mode` + `superApi.asset-card-size`.
