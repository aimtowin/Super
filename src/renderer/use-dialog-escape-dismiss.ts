...
export type UseDialogEscapeDismissParams = {
  ...
  closeLinkedFolderCollectionDialog: () => void;
  setDialog: (value: null) => void;
  setShowCollectionInput: (open: boolean) => void;
  setConflicts: (value: ImportConflictPlan | null) => void;
  setError: (message: string | null) => void;
  /** Super-c2rm: Escape dismisses the pending library-plugin trust prompt as Later. */
  onDismissPluginTrustPrompt?: () => void;
  /** Super-99lv: Escape dismisses the blocking fatal alert. */
  onDismissFatalAlert?: () => void;
  /** Super-kdnm: Escape on connection-failure dialog aborts remaining AI jobs. */
  onAbortAiConnectionFailure?: () => void;
};

/**
 * Document-level Escape dismiss for stacked App dialogs (Super-uye).
...
export function useDialogEscapeDismiss({
  ...
  closeLinkedFolderCollectionDialog,
  setDialog,
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      switch (action.kind) {
        ...
        case "close-convert-linked":
          resetConvertLinkedDialog();
          return;
        case "close-linked-folder-collection":
          closeLinkedFolderCollectionDialog();
          return;
        case "close-dialog":
          setDialog(null);
