/**
 * Masonry card preview sizing for portrait assets (Super-woa / Super-5p45).
 *
...
export function estimateMasonryPreviewHeightPx(
  ...
  if (!width || !height || width <= 0 || height <= 0) {
    ...
    // cannot drift away from the windowed slot height (Super-1s3d).
    return col / 1.3;
  }
 ...
 *  and leave a truncated empty band (Super-1s3d).
 */
