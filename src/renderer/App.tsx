...
import { LinkedFolderCollectionDialog } from "./LinkedFolderCollectionDialog";
...
import type { SuperMcpSettingsApi } from "../shared/mcp";
...
import type { SuperPluginManagerApi } from '../shared/plugin-manager-api';
...
type RendererWindow = Window & {
  super?: {
    library?: SuperLibraryApi;
    shell?: SuperShellApi;
    appUpdate?: SuperAppUpdateApi;
    automation?: SuperAutomationScriptApi;
    plugins?: SuperPluginManagerApi;
    mcp?: SuperMcpSettingsApi;
    e2e?: {
...
function AppInner() {
  ...
  const api = (window as RendererWindow).super?.library;
  const shellApi = (window as RendererWindow).super?.shell;
  ...
  const [linkedFolderCollectionDialog, setLinkedFolderCollectionDialog] = useState<{
  ...
  // Super-d0nv: bump to re-fetch folder browse entries when a cover
  ...
  /** Full trash tombstone list for hierarchy browse (Super-6pcd). */
  ...
  // Toast + fatal alert (REQ-SHELL-010 / Super-99lv): controller owns
  ...
  /** Virtual linked directory currently opened inside a collection (read-only grouping). */
  const [collectionSourceFolderId, setCollectionSourceFolderId] = useState<string | null>(null);
  ...
  // Super-eaxs: tag-management AND search ("包含 N 个标签") splits the tag
  ...
  /** Shape/aspect preset OR ranges (Super-gp4). */
  ...
  /** Super-hm28: null = normal sort; otherwise client shuffle seed. */
  ...
  // Super-ws4k: paginated browse/search loading. First pages render
  ...
  // Super-c9r3: bridge the multi-edit rebuilder into the ai.content.cleared
  ...
  useEffect(() => {
    const api = (window as RendererWindow).super?.plugins;
  ...
  const pluginSidebarViews = usePluginSidebarViews(
    (window as RendererWindow).super?.plugins,
    library?.libraryId,
  ...
  const appUpdateApi = (window as RendererWindow).super?.appUpdate;
  ...
  const checkForAppUpdates = useCallback(async () => {
    if (appUpdateApi === undefined || appUpdateChecking || appUpdateDownloading) return;
  ...
  async function refreshAppLog(automationCorrelationId = appLogAutomationCorrelationId): Promise<void> {
    const bridge = (window as RendererWindow).super?.shell;
  ...
  // Super-njoy: owned here (not inside AssetPreviewModal, which remounts
  ...
  // Super-l67w: folder cards share the flush masonry column width so the
  ...
  // Super-wgl2: the marquee box div is always mounted and moved directly
  ...
  // Super-b963a9: masonry/justified canvases use the full-scope layout index
  ...
  // Super-6pcd: assets at the current trash hop only (no source-folder grouping).
  ...
  // REQ-FOLDER-010 / Super-nu6o: selection order must match the canvas,
  ...
  // Super-ws4k: select-all / invert cover the *whole* browse scope, not just
  ...
  useEffect(() => {
    ...
    return shellApi.onCommandCompleted((payload) => {
      // Super-fmbr: MCP operations show the same toasts as manual ones —
  ...
  // browse parent is a managed folder or the managed root.  A collection with
  // linked members additionally gets read-only source-directory cards derived
  // from those members; this never reads arbitrary sibling source assets.
  useEffect(() => {
    let cancelled = false;
    async function loadFolderBrowseEntries() {
      if (api && library && activeCollectionId) {
        if (searchValue.trim()) {
          if (!cancelled) setFolderBrowseEntries([]);
          return;
        }
        const result = await api.listCollectionSourceBrowseEntries({
          libraryId: library.libraryId,
          collectionId: activeCollectionId,
          recursive: collectionRecursive,
          parentFolderId: collectionSourceFolderId,
      ...
      // Super-7a9e89: 「递归显示子文件夹内容」的浏览语义是把子级资产
    ...
    showIgnoredItems,
    folderRecursive,
    // Super-d0nv: a cover candidate's thumbnail.ready bumps this token so
    // the row re-fetches and the generated cover appears without navigation.
    folderBrowseRefreshToken,
  ]);

  // Super-d0nv: keep the cover-candidate asset set (from the last browse
  ...
  const resizeAssetCards = useCallback(
    (requestedSize: number, clientX?: number, clientY?: number) => {
      ...
      // Super-32p: always re-anchor after settle; width/size reflow may reset
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      // into pixels for the continuous high-gain path (Super-fvpi / Super-7ny).
      const delta =
  ...
  // (Super-46i9 / Super-7ny). Viewer owns the chord while preview is open.
  useEffect(() => {
    if (previewAsset) return;
  ...
  const openAssetPreview = useCallback((asset: AssetSummary) => {
    ...
    // Super-ayf: entering the viewer always shows chrome, regardless of
  ...
  const loadContent = useCallback(
    async (
      ...
      // Super-ws4k: register the paginated query so the scroll sentinel can
  ...
  // Super-y0au: keep recent libraries warm on the no-library start surface.
  useEffect(() => {
    if (!api || library) return;
  ...
  // Super-kipk: no-library surface is the shared create dialog (start phase),
    ...
    return api.onLifecycle((event) => {
      ...
      void (async () => {
        try {
          ...
          // super://preview URLs would be rebuilt with the NEW libraryId and
    ...
    const unsubscribe = api.onThumbnailEvent((event) => {
      ...
      if (event.type === "asset.thumbnail.ready") {
        ...
        if (event.artifactId) {
          ...
          // Super-d0nv: when a cover candidate of the current folder-card
  ...
  useEffect(() => {
    ...
    const unsubscribeProgress = api.onAiProgress((event) => {
      if (event.libraryId !== library.libraryId) return;
      // Super-u0tn: do not arm analyzing UI for background/import auto jobs
    ...
    const unsubscribeCompleted = api.onAiCompleted((event) => {
      if (event.libraryId !== library.libraryId) return;
      // Refresh only — completion toast is owned by queue-drain (Super-4i18).
      void reloadCurrentContentRef.current();
      if (selectedAssetIdRef.current === event.assetId) {
        // Super-c9r3: refreshAfterAi refreshes the primary's metadata and
    ...
    const unsubscribeCleared = api.onAiCleared((event) => {
      ...
      // Super-c9r3: clearing AI must NOT disturb the browsing view, selection
  ...
  async function runLibraryOpenPipeline(
    ...
    try {
      ...
      // read-only library (Super-e0dw). Keep the new library even if close
      ...
      // Super-s0oq: opening an invalid library removes it from the recent
  ...
  // Super-eaxs: tag-management AND/OR jump — leave management, scope to all
  ...
  async function chooseCollection(
    ...
    setCollectionSourceFolderId(sourceFolderId);
    // Collections have no physical destination directory. Prevent a later
    // external drop from inheriting the last managed-folder browsing scope.
  ...
  function applySearchResult(
    ...
    // Super-87pd: canvas slots come from beginPage. Replacing the list with
  ...
  function currentSearchScope(): SearchScope | undefined {
    if (activeCollectionId)
      return {
        ...
        collectionId: activeCollectionId,
        recursive: collectionRecursive,
        ...(collectionSourceFolderId ? { sourceFolderId: collectionSourceFolderId } : {}),
      };
    if (assetScope === "root")
  ...
  // Super-关联刷新: asset/folder deletions must clear cards immediately
  ...
  const applyLocalAssetRemoval = useCallback(
    (assetIds: string[], options?: { removedCount?: number }) => {
      ...
      // Super-关联刷新: fold the deletion into the pagination bookkeeping so
  ...
  async function commitInlineCollectionRename() {
    ...
    try {
      ...
      // Blur and pointer-down both commit the inline row. Closing a failed
      // session here prevents the same stale request from reporting forever.
      setInlineCollectionRename(null);
      setError(toOrganizationMessage(caught, "collection", "rename", locale));
    }
  }
  ...
  // Super-vf8x: folder create/rename/trash chords (mac ⌘ / Windows Ctrl).
  ...
  async function executeSearchDefinition(definition: SearchDefinition) {
    ...
    applySearchResult(result.value);
    // Super-ws4k: subsequent pages must reuse the same query/scope/sort.
    registerBrowseSearchPage(beginBrowsePage, {
      libraryId: library.libraryId,
      query: definition.search ?? null,
      filters: definition.filters,
  ...
  async function runSearch(
    ...
    try {
      ...
      // Super-huvw: discovery debounce / reload must not toast "搜索完成"
  ...
  useEffect(() => {
    ...
    if (
      !library ||
      showTrash ||
      // Super-eaxs: entering tag management clears discovery controls; the
  ...
  refreshAfterAiRef.current = async (assetId: string) => {
    ...
    // primary alone was refreshed above (Super-c9r3 regression: batch AI
  ...
  async function addLinkedFolderAssetsToCollection() {
    if (!api || !library || !linkedFolderCollectionDialog?.collectionId) return;
    const dialogState = linkedFolderCollectionDialog;
    setLinkedFolderCollectionDialog((current) =>
      current ? { ...current, submitting: true } : current,
    );
    setUiState("loading");
    try {
      const result = await api.addLinkedFolderAssetsToCollection({
        libraryId: library.libraryId,
        collectionId: dialogState.collectionId,
        linkedFolderId: dialogState.linkedFolderId,
        relativePath: dialogState.relativePath,
        recursive: dialogState.recursive,
      });
      if (!result.ok) throw new LibraryOperationError(result.error);
      const collectionResult = await api.listCollections({
        libraryId: library.libraryId,
      ...
      if (collectionResult.ok) setCollections(collectionResult.value);
      setLinkedFolderCollectionDialog(null);
      setNotice(
        t("toast.linkedFolderAddedToCollection", {
          count: result.value.addedCount,
        }),
        result.value.historyEntryId,
      );
      await reloadCurrentContent();
      ...
      setError(
        toMessage(
          caught,
          t("toast.linkedFolderAddToCollectionFailed"),
          locale,
        ),
      );
    } finally {
  ...
  async function confirmDeleteLibraryFromDisk() {
    ...
    try {
      await closeAssetPreview(false);
      // Super-dfgg: Chromium keeps super:// thumbnail/source files mapped
      ...
      // Super-qgm1: a failed disk deletion must NOT masquerade as success.
  ...
  async function exportLibrary(format: "folder" | "zip", includeLinkedContent: boolean) {
    ...
    try {
      ...
      if (!result.ok) {
        if (result.error.code === "CANCELLED") {
          // Super-tye: folder/save dialog cancel must clear the optimistic strip.
  ...
  async function completeImportCopy() {
    ...
    // Super-1tio: the validated dialog must disappear the moment the import
  ...
  async function completeImportInPlace() {
    if (!api || !importValidated) return;
    // Super-1tio: same immediate-dismissal contract as completeImportCopy —
  ...
  useEffect(() => {
    ...
    const unsubscribe = api.onAssetsChanged((event) => {
      ...
      void refreshOperationHistory();
      // Super-yqrl: while a user import is applying, each committed asset
      // triggers a silent canvas refresh so cards appear one-by-one.
      if (uiStateRef.current === "importing" || importProgressRef.current) {
        scheduleSilentReload();
        return;
      }
      // Super-关联刷新: batch mutations (delete/trash/relink) broadcast one
    ...
    const unsubscribeLibraryChanged = api.onLibraryChanged((event) => {
      ...
      // canvas after large imports (Super-yti0). Asset-set mutations still
  ...
  const dialogEscapeSnapshot = useMemo((): DialogEscapeSnapshot => {
    return {
      ...
      linkedFolderCollectionOpen: linkedFolderCollectionDialog !== null,
  ...
  useDialogEscapeDismiss({
    ...
    closeLinkedFolderCollectionDialog: () => setLinkedFolderCollectionDialog(null),
    setDialog: (value) => {
      // Super-kipk: required no-library surface cannot dismiss; Escape returns
  ...
  useEffect(() => {
    // Super-0rk: freeze shell pointer targets while any modal is open.
    document.body.classList.toggle("super-modal-open", dialogFocusTrapActive);
    return () => {
      document.body.classList.remove("super-modal-open");
    };
  }, [dialogFocusTrapActive]);
  ...
  usePluginShortcutKeyboard({
    enabled: Boolean(library) && !showTagManagement && !showPluginSidebarView,
    platform: SHORTCUT_PLATFORM,
    pluginApi: (window as RendererWindow).super?.plugins,
    libraryId: library?.libraryId,
  ...
  // Super-166q: macOS Edit → Copy accelerator (custom menu, not role:copy).
  useEffect(() => {
    if (!shellApi) return;
  ...
  // Super-oy07: sync BrowserWindow focus to document (native macOS traffic lights
  ...
  useEffect(() => {
    ...
    return shellApi.onApplicationMenuCommand((command: ApplicationMenuCommand) => {
      ...
      // Super-q0b1: honor the same disabled state the Windows in-app menu
  ...
  useEffect(() => {
    ...
    // Super-接管: the native menu accelerator must NOT intercept Cmd/Ctrl+Z
  ...
  // Super-接管: Windows has no native menu bar (its accelerators are gone),
  ...
  return (
    ...
    >
      ...
      <section className="workspace">
        ...
        >
          <DimensionFilterBar
            ...
            onTagNamesChange={(names) => {
              ...
              // (Super-w9c6 / resolveFolderBrowseParentId).
        ...
        >
          {/* Super-wgl2: rendered once, mutated directly via ref — a
          ...
          {library && showPluginSidebarView ? (
            ...
            browseCanvasBodyLayout.mode !== "empty" ? (
              <>
                {browseCanvasBodyLayout.showFolders && (
                  ...
                  >
                    {canvasFolderBrowseEntries.map((entry) => (
                      <FolderCard
                        draggable={!showTrash && !activeCollectionId}
                        entry={entry}
                        key={entry.folderId}
                        libraryId={library.libraryId}
                        trashed={showTrash}
                        {...(showTrash || activeCollectionId
                          ? {}
                  ...
                  {(() => {
                    ...
                    <CardTag
                      ...
                      onDragStart={(event) => {
                        ...
                        const isE2e = Boolean(
                          (window as RendererWindow).super?.e2e,
                        );
                        ...
                        // Super-aa3: Option/Alt during dragover selects copy
                        ...
                        // (asset-drag-preview.ts); the same super:// URL as
                        ...
                        const preview = showAssetDragPreview({
                          thumbnailUrl:
                            ...
                            library
                              ? `super://preview/${library.libraryId}/${asset.thumbnailArtifactId}`
                              : null,
                      ...
                      >
                        {(() => {
                          ...
                          if (isCardHoverPreviewable(asset)) {
                            ...
                            // Super-2ajm: unify with AssetCardMedia so a
                ...
                {/* Super-87pd: invisible tail probe. A jump to the end fills
                    the last window, not pages 0 → 100 → 200. Super-6z5r:
        ...
        {/* Drop overlay lives outside the scrollable canvas so it stays put
            while the browse grid scrolls (Super-ns3r). */}
        {externalDropActive && (
      ...
      {linkedFolderCollectionDialog && (
        <LinkedFolderCollectionDialog
          collectionId={linkedFolderCollectionDialog.collectionId}
          collections={collections}
          folderName={linkedFolderCollectionDialog.name}
          recursive={linkedFolderCollectionDialog.recursive}
          submitting={linkedFolderCollectionDialog.submitting}
          onCancel={() => setLinkedFolderCollectionDialog(null)}
          onCollectionChange={(collectionId) =>
            setLinkedFolderCollectionDialog((current) =>
              current ? { ...current, collectionId } : current,
            )
          }
          onConfirm={() => void addLinkedFolderAssetsToCollection()}
          onRecursiveChange={(recursive) =>
            setLinkedFolderCollectionDialog((current) =>
              current ? { ...current, recursive } : current,
            )
          }
        />
      )}
      ...
      <AppSettingsDialog
        ...
        aiConfigPanel={
          <AiConfigDialog
            ...
            onClose={() => {
              ...
              // (Super-rsbt); re-sync from stored credentials after draft edits.
              void probeStoredAiConnection();
            }}
        ...
        onOpenExtensionReleases={() => {
          void (window as RendererWindow).super?.shell?.openExternalUrl(
            "https://github.com/aimtowin/Super/releases/latest",
          );
        }}
        pluginApi={(window as RendererWindow).super?.plugins}
        pluginContributionRefreshKey={pluginContributionRefreshKey}
        libraryId={library?.libraryId}
        mcpApi={(window as RendererWindow).super?.mcp}
      ...
      <ScriptSandboxPreviewDialog
        automation={(window as RendererWindow).super?.automation}
        libraryId={library?.libraryId ?? null}
        onClose={() => {
      ...
      <AssetContextMenu
        ...
        onAddLinkedFolderToCollection={(linkedFolderId, relativePath, name) => {
          setLinkedFolderCollectionDialog({
            linkedFolderId,
            relativePath,
            name,
            collectionId: "",
