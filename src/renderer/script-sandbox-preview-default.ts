...
while (true) {
  const page = await superApi.assets.search({ query: 'name:Ser | tag:Ser', limit: 200, offset });
...
for (let index = 0; index < matchingIds.length; index += 500) {
  batches.push(await superApi.assets.setRating(matchingIds.slice(index, index + 500), 4));
}
