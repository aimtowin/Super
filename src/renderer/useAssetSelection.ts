...
export interface UseAssetSelectionParams {
  ...
  /** Super-wgl2: the always-mounted marquee box div, mutated directly via ref. */
  ...
  folderIds?: string[];
  /**
   * Optional asset id order for Shift/marquee (Super-oz1t). When omitted,
...
export interface UseAssetSelectionReturn {
  ...
  /**
   * Attach to folder cards' onClick. Plain click selects (Super-829);
...
export function useAssetSelection({
  ...
  // Super-wgl2: the marquee must not run per pointermove (~120Hz) — the
  ...
  // Super-wgl2: direct-DOM marquee box writer. Keep the callback stable so
  ...
  const handleCanvasMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      // Super-wgl2: the hits diff key must not leak across marquees — a
      // second drag over the same region would otherwise skip every update.
      marqueeLastHitsKeyRef.current = "";
      // Super-wgl2: suppress the per-card selection transition while a
  ...
  useEffect(() => {
    ...
    const collectHits = (box: MarqueeRect) => {
      ...
      // Super-wgl2: cache card rects in canvas-content coordinates — they
    ...
    const applyMarqueeHits = (hits: {
      ...
      // Super-wgl2: only push to React when the hit set actually changed —
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Super-wgl2: park the pointer and run the marquee once per animation
      ...
      if (marqueeRafRef.current === null) {
        marqueeRafRef.current = requestAnimationFrame(() => {
          ...
          // Super-wgl2: a mouseup between pointermove and this frame must
