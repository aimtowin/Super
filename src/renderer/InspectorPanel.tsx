...
import type { PreviewResolution, SuperLibraryApi } from "../shared/library-api";
...
// Super-4bdd26 收编：Inspector 每次渲染格式化多个日期，缓存 formatter。
...
export interface InspectorPanelProps {
  ...
  /** Super-t8sw: when false, hide AI corner badges (data retained). */
...
function InspectorHeroSinglePreview({
  ...
  asset: AssetSummary;
  library: RendererLibrarySummary | null;
  /** Ready GIF/video resolution to loop-play (Super-a9n); null = static only. */
...
function InspectorHero({
  ...
  /** Super-a9n: only ever passed/used for single selection. */
  livePreview?: PreviewResolution | null;
  api?: SuperLibraryApi | null;
  cardFeelEnabled?: boolean;
}) {
  ...
  return (
    <div className={`inspector-hero-compact${isMulti ? " is-multi" : ""}`}>
      ...
      <strong className="inspector-hero-title" title={title}>
        {isMulti ? (
          <>
            {/* Super-poly1: name 可缩略、计数后缀不缩略，保证「等 N 个文件」始终可见。 */}
...
export function InspectorPanel(props: InspectorPanelProps) {
  ...
  // Super-a9n: single-selection GIF/video loops in the Inspector hero;
  ...
  // REQ-VIEW-003 / CU-D8 / Super-i07: fetch extracted metadata for video /
  ...
  // 描述输入框高度自动包裹内容（Super-qto）：默认单行，受控值变化（输入/
        ...
        <div className="inspector-content">
          ...
          {displayMetadata || isMultiEdit ? (
            <>
              ...
              {(() => {
                // Super-uz1: palette chrome is image/video only — never show
              ...
              <div className="editor-field editor-field-palette">
                {/* Super-l79c: no「色卡 · 自动」label — swatches alone. */}
