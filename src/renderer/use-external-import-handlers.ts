...
/**
 * A collection is an organisational relation, not a physical folder.  An
 * external drop onto it must therefore import into the managed library root
 * and then attach the imported assets to the collection.  In particular, it
 * must not inherit the last folder selected elsewhere in the workspace.
 */
export function resolveExternalImportDestination(input: {
  targetFolderId: string | null | undefined;
  targetCollectionId: string | undefined;
  fallbackFolderId: string | undefined;
}): { targetFolderId: string | undefined; targetCollectionId: string | undefined } {
  ...
  return {
    targetFolderId: input.targetFolderId ?? input.fallbackFolderId,
 ...
 * (Super-uye). Dialog/file-picker import stays in App.
 */
export function useExternalImportHandlers({
  ...
  const applyDesktopImportResult = useCallback(
    async (
      result: Awaited<ReturnType<SuperLibraryApi["importDropped"]>>,
    ): Promise<void> => {
      if (!result.ok) {
  ...
  const importDroppedFiles = useCallback(
    async (
      ...
      const destination = resolveExternalImportDestination({
        targetFolderId,
        targetCollectionId,
        fallbackFolderId: managedImportTargetFolderIdRef.current,
      });
      ...
      try {
        const result = await api.importDropped({
          ...
          targetCollectionId: destination.targetCollectionId,
          autoDetectImageSequences,
          files,
  ...
  const handleExternalDragOver = useCallback(
    (event: DragEvent<HTMLElement>) => {
      ...
      // Super-pagf: resolve folder-card under pointer from the canvas
  ...
  const createFolderCardDropHandlers = useCallback(
    (folderId: string) => ({
      ...
      onDragLeave: (event: DragEvent<HTMLButtonElement>) => {
        // Super-4gyr: cross-app drags often report relatedTarget=null while
      ...
      onDragOver: (event: DragEvent<HTMLButtonElement>) => {
        // Super-4gyr: keep target warm on dragover — unfocused windows may
