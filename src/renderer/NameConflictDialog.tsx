...
/** Same-folder name conflicts only (Super-9iyi / zp8q / 79c7 / 793k). */
export function NameConflictDialog({
  ...
  // Super-793k: show the colliding (already-existing) asset's name and
        ...
        {nameConflicts.map((item, index) => {
          const thumb =
            item.existingThumbnailArtifactId && libraryId
              ? `super://preview/${libraryId}/${item.existingThumbnailArtifactId}`
              : null;
          return (
            <li
