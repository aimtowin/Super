...
interface AssetContextMenuProps {
  ...
  /** Folder tombstones in trash (Super-b3kf / empty-trash enablement). */
...
export function AssetContextMenu(props: AssetContextMenuProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (window as RendererWindow).super?.library
  ...
  // Super-接管: the workspace canvas menu is a plugin extension point
    ...
    <ContextMenuBackdrop>
      ...
      >
        ...
        {activeContextMenu.descriptor.type === "smart-collection" && (() => {
          ...
          const commandContext: SidebarCommandContext = {
            ...
            actions: {
              ...
              addLinkedFolderToCollection: onAddLinkedFolderToCollection,
        ...
        {activeContextMenu.descriptor.type === "folder" && (() => {
          ...
          const addLinkedFolderToCollectionItem = resolvedById.get(
            "folder.linked-folder.add-to-collection",
          );
            ...
            <>
              ...
              <ContextMenuSection label={t("command.group.folders")}>
                ...
                {addLinkedFolderToCollectionItem && (
                  <ContextMenuItem
                    icon={<Icon name="collection" size={14} />}
                    label={addLinkedFolderToCollectionItem.label}
                    onAction={() =>
                      runSidebarCommand("folder.linked-folder.add-to-collection")
                    }
                  />
                )}
                {copyItem && (
        ...
        {activeContextMenu.descriptor.type === "multi-asset" &&
          (() => {
            ...
            // REQ-MENU-004 / Super-koy: process/skip counts + reasons from a
