...
export type ContextMenuDescriptor =
  ...
  | {
      type: "multi-asset";
      assetIds: string[];
      /** Canvas folder cards in the same multi/mixed selection (Super-koy). */
      folderIds?: string[];
      count: number;
    }
  | {
  ...
  | {
      /** Sidebar trash row context menu (Super-gaoi). */
      type: "trash";
    }
  | {
      /** Deleted managed-folder tombstone in trash browse (Super-qufh). */
      type: "trashed-folder";
      tombstoneId: string;
      name: string;
      relativePath: string;
