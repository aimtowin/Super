 ...
 * (Super-5fq; Escape extract from App for Super-uye wave).
 *
...
export type UseSelectionKeyboardArgs = {
  ...
  /** Loaded browse-scope asset ids (Super-6w7n); gates select-all/invert on a non-empty scope. */
  readonly browseScopeAssetIds: readonly string[];
  readonly selectedAssetIds: readonly string[];
  readonly clearAssetSelection: () => void;
  /** Super-ws4k: async select-all covering the whole browse scope (idsOnly). */
  readonly onSelectAll?: () => void;
  /** Super-ws4k: async invert covering the whole browse scope (idsOnly). */
