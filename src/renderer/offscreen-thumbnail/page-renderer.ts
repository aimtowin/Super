/**
 * Offscreen thumbnail frame pipeline (slice E, Super-hnmg).
 *
...
export async function renderModelThumbnailFrame(
  ...
  try {
    ...
    if (preset) {
      try {
        ...
        environment = await promiseWithTimeout(
          loadHdrEnvironmentForFrame(`super://app-assets/hdri/${preset.fileName}`, {
    ...
    if (job.data.views && job.data.views.length > 0) {
      // Super-6w40: multi-view render (AI four views) — each direction
