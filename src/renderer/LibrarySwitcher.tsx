...
export type LibrarySwitcherProps = {
  ...
  onCloseLibrary: () => void;
  /** Soft remove: close + drop from recents; disk untouched (Super-ucx). */
  onRemoveLibrary?: () => void;
  /** Irreversible delete of the currently open library root (Super-9i8). */
  ...
  libraryOpen?: boolean;
  /** 同步连接状态（Super-ae7257 后续）：link / link-off 图标提示。 */
  ...
  return (
    <div className="library-switcher" ref={rootRef}>
      <button
        ...
        type="button"
      >
        <span className="library-switcher-glyph" aria-hidden="true">
              ...
              >
                ...
                {recentLibraries.map((entry) => (
                  <div className="library-switcher-recent-row" key={entry.path}>
                    {/* Super-s0oq: hovering a recent library reveals its full
