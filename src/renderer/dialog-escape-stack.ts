/**
 * Pure priority for Escape dismiss across stacked App dialogs (Super-uye).
...
export type DialogEscapeSnapshot = {
  ...
  linkedFolderCollectionOpen?: boolean;
  dialogOpen: boolean;
  /** Calm prompt when library plugins appear that still need per-device trust. */
  pluginTrustPromptOpen: boolean;
  /** Super-99lv: blocking fatal alert — Escape acknowledges/dismisses. */
  fatalAlertOpen: boolean;
  /** Super-kdnm: AI connection lost — Escape aborts remaining jobs. */
...
export type DialogEscapeAction =
  ...
  | { kind: "close-linked-folder-collection" }
  | { kind: "close-dialog" }
...
export function resolveDialogEscapeAction(
  ...
  // Generic fatal alert sits above other layers (Super-99lv).
  if (snapshot.fatalAlertOpen) {
    return { kind: "dismiss-fatal-alert" };
  }
  // Fatal AI connection dialog sits above remaining layers (Super-kdnm).
  ...
  if (snapshot.linkedFolderCollectionOpen) {
    return { kind: "close-linked-folder-collection" };
  }
