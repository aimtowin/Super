 ...
 * URL. Model sources are `super://source/<libraryId>/<assetId>` — a flat
 ...
 * `super://source/<libraryId>/<assetId>?revision=<revisionId>`. The
 ...
 *
 * References that are already absolute (super://, http(s), data:) pass
...
/** Build a `super://preview/<libraryId>/<artifactId>` artifact URL. */
export function superPreviewUrl(libraryId: string, artifactId: string): string {
  return `super://preview/${libraryId}/${artifactId}`;
}

/** Build a `super://source/<libraryId>/<assetId>?revision=<id>` source URL. */
export function superSourceUrl(
  libraryId: string,
  assetId: string,
  revisionId?: string,
): string {
  const base = `super://source/${libraryId}/${assetId}`;
 ...
 * Returns the `super://source` URL, or null when the reference is
...
export function remapCompanionUrl(input: {
  ...
  return companion
    ? superSourceUrl(libraryId, companion.assetId, companion.revisionId)
    : null;
}
...
export function remapCompanionUrlByBasename(input: {
  ...
  if (exact) return superSourceUrl(input.libraryId, exact.assetId, exact.revisionId);
  const basename = normalized.split('/').pop();
  if (!basename) return null;
  for (const [relativePath, companion] of input.companionMap) {
    if (relativePath.split('/').pop() === basename) {
      return superSourceUrl(input.libraryId, companion.assetId, companion.revisionId);
    }
  }
  return null;
}
 ...
 * `super://source` companion URLs. Only `uri` strings are touched;
 ...
 * preserved; only the path tail is swapped for its super://preview URL.
 */
