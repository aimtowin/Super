 ...
 *                         rewritten to super://preview URLs;
 ...
 *                         re-pointed at remapped super://preview URLs;
...
async function loadFbx(input: LoadModelSceneInput): Promise<LoadedModelScene> {
  // Super-a5ic: ufbx conversion is the primary path. The bridge now uses
    ...
    try {
      ...
      if (result.status === 'ready') {
        ...
        const gltf = await loader.loadAsync(
          superPreviewUrl(input.libraryId, result.glbArtifactId),
        );
        const scene = gltf.scene ?? new Group();
        enableShadowCasting(scene);
        return {
  ...
  // by relative names fail to fetch against super://source; remap them
 ...
 * loads that failed against the flat super://source URL at their remapped
 * super://preview companions (3D-11 / 3D-12).
 *
