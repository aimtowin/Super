/**
 * ModelViewerSurface — the interactive 3D viewport (slice C, Super-qvc6).
 *
...
export interface ModelViewerSurfaceProps {
  readonly api: SuperLibraryApi;
  readonly asset: AssetSummary;
  readonly libraryId: string;
  /** `super://source/...` URL of the model asset. */
...
export function ModelViewerSurface(props: ModelViewerSurfaceProps) {
  ...
  useEffect(() => {
    ...
    // (Super-v4jt / Super-xjcy), so the right mouse button is not a camera
  ...
  // Right-drag environment rotation (Super-v4jt): rotates the light source
  ...
  // Display mode (Super-fkhe): applied to the live scene; re-applied after
  ...
  useEffect(() => {
    ...
    const url = preset ? `super://app-assets/hdri/${preset.fileName}` : null;
  ...
  // MODEL-004 / Super-osr0: route non-blocking notices to the shell Info
