...
interface AssetCardMediaProps {
   ...
   * the silent video preview and never plays audio (Super hover 音频工单).
   */
  hovering?: boolean;
  preview: PreviewResolution | null;
  libraryId?: string;
  sequence?: ImageSequenceSummary | null;
  /** Super-2ajm: generation failed — show the cracked-file icon fallback. */
  failed?: boolean;
  /** Audio assets play in-place on hover (Super hover 音频工单). */
  ...
  mediaMuted?: boolean;
  /**
   * Live video failed to play in-place (Super-c8a1a3). The card calls this
...
export function AssetCardMedia({
  ...
  // Super-2ajm: a failed image load must never paint the browser's broken
