...
export interface SidebarCommandActions {
  ...
  /** Adds the available linked directory's indexed assets as a static snapshot. */
  readonly addLinkedFolderToCollection: (
...
export const sidebarCommandDefinitions: readonly SidebarCommandDefinition[] = [
  ...
  },
  {
    ...
    group: 'organize',
    // Finder/Explorer new-folder chord; Windows Ctrl twin (Super-vf8x).
    shortcut: {
  ...
  {
    id: 'folder.linked-folder.add-to-collection',
    title: (ctx) =>
      translateForLocale(ctx.locale, 'command.folder.addLinkedFolderToCollection'),
    group: 'organize',
    visible: (ctx) =>
      ctx.menuKind === 'folder' &&
      ctx.locationKind === 'linked' &&
    ...
    run: (ctx) => {
      const linkedFolderId = ctx.linkedFolder?.folderId;
      if (linkedFolderId !== undefined) {
        ctx.actions.addLinkedFolderToCollection(
          linkedFolderId,
