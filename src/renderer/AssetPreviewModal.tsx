...
interface AssetPreviewModalProps {
  api: SuperLibraryApi;
  asset: AssetSummary;
  /** Owned by a parent that survives per-asset remounts (Super-ayf). */
  chromeIdle: boolean;
  libraryId: string;
...
>(function AssetPreviewModal(
  ...
  // Super-e56a1f: 代理回退的可见状态。生成中/加载中显示普通状态提示
  // （非警告），只有生成失败才显示警告。
  ...
  const ensureProxyFallback = useCallback(
    async (errorCode: string) => {
      ...
      void api
        ...
        .catch(() => undefined);
      // Super-e56a1f: 生成中显示状态提示（非警告），失败才警告。
  ...
  async function retry() {
    ...
    requestedProxyFallbackRef.current = null;
    setRetrying(true);
    // Super-e56a1f: 手动重试同样进入生成中状态，失败前不显示警告。
    setProxyFallbackState("generating");
    ...
    // second error event on Chromium's custom `super://` scheme.
  ...
  function handlePlaybackError(event: SyntheticEvent<HTMLMediaElement>) {
    ...
    if (shouldProxyFallback) {
      ...
      // Super-e56a1f: 进入代理生成流程后不再显示「视频无法播放」警告——
  ...
  const copyViewerAsset = useCallback(async () => {
    // Super-f8e175: 与右键「复制」一致，把当前资产源文件复制到剪贴板。
  ...
  useEffect(() => {
    // Super-f8e175: 图片/视频/PDF/音频等非文本查看器聚焦时 Ctrl/Cmd+C
      ...
      <ShellSurface className="preview-modal">
        ...
        <div className={`preview-content${isTextViewer ? " is-text-mode" : ""}${isDocumentViewer ? " is-document-mode" : ""}`}>
          {primarySurface === "loading" && !placeholderUrl ? (
            ...
            // Slice C (Super-qvc6): the 3D viewport surface. Keyed so a
