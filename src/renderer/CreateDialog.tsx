...
export interface CreateDialogProps {
  open: boolean;
  /**
   * Super-kipk: `start` is the no-library surface; `form` is the name
   * prompt for creating a Super library; `eagle`/`billfish` are the same
  ...
  onBackToStart: () => void;
  /** Open an existing library via the native folder picker (Super-y0au). */
  onOpenExisting: () => void;
  /** Import a library folder/ZIP (same entry as the former empty-state CTA). */
  onImportLibrary: () => void;
  /** One-click open from the recent list. */
  onOpenRecent: (path: string) => void;
  /** Super-kqgy: remove a library from the recent list (not from disk). */
...
/**
 * Unified create / no-library start dialog (Super-kipk / y0au).
 *
 ...
 * import CTAs. Menu 「新建资源库」 opens the name form only (Super-pte2).
 */
export function CreateDialog({
  ...
  // Super-s0oq: the create dialog shows at most five most-recent libraries;
            ...
            <ul className="create-dialog-recent-list">
              {visibleRecents.map((entry) => (
                <li className="create-dialog-recent-row" key={entry.path}>
                  {/* Super-kqgy follow-up: the forget affordance lives inside
