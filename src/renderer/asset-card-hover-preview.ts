...
export function isCardHoverPreviewable(asset: {
  ...
  // Audio plays in-place on hover (Super hover 音频工单).
...
export function coverSrc(
  ...
  return `super://preview/${libraryId}/${thumbnailArtifactId}`;
}
...
export function sourceSrc(libraryId: string, assetId: string): string {
  return `super://source/${libraryId}/${assetId}`;
}
 ...
 * (Super-a9n) so both use one tested rule: only an active, ready
 ...
 *
 * Super-43d32f: animated GIFs no longer resolve through a WebM proxy; the
...
/**
 * Video hover preview muted state (Super hover 音频工单): sound only plays
