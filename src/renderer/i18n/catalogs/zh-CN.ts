...
export const zhCN = {
  ...
  shell: {
    ...
    networkStorageNotice:
      "该资源库位于网络共享（NAS/SMB）上。Super 会在此使用回滚日志；文件锁和断线恢复取决于 NAS。请保持同一时间只有一个 Super 实例写入，并做好备份。",
    readOnlyBanner: "该资源库由更新版本的 Super 创建，当前为只读模式。升级 Super 后可继续编辑。",
    readOnlyBannerMigrationStuck: "该资源库多次尝试迁移均失败，现为只读模式。请保留当前版本的 Super，待新版可用后再升级。",
    readOnlyBannerVersioned:
      "该资源库由 Super v{libraryVersion} 创建（当前版本支持 v{supportedVersion}），当前为只读模式。升级 Super 后可继续编辑。",
    recoveryBackup1: "资源库已从第一份有效备份恢复，最近的部分数据库变更可能丢失。",
    recoveryBackup2: "资源库已从第二份有效备份恢复，最近的部分数据库变更可能丢失。",
    recoveryReadonly: "资源库数据库受损，Super 已以只读模式打开，以便导出剩余数据。",
    recoveryRescue: "资源库已根据 Assets 文件夹抢救重建，合集、标签和评分可能需要重新恢复。",
    recoveryRescueDetails: "已找到 {count} 个源文件；合集、标签、评分、描述和源链接未从损坏数据库恢复。",
    recoveryReportAvailable: "已生成恢复报告，可在资源库的 .super/corrupt-backup 目录查看。",
  ...
  automation: {
    preview: {
      ...
      fileErrors: {
        cancelled: "没有打开或保存脚本。",
        "invalid-script-file": "请选择扩展名为 .superApi.js 或 .superApi.ts 的脚本文件。",
  ...
  filter: {
    ...
    palette: "调色盘",
    paletteSelected: "已选 {count} 色",
    paletteSelectHint: "点击选择 · Shift 多选",
    color: {
      red: "红",
      scarlet: "朱红",
      orange: "橙",
  ...
  command: {
    ...
    folder: {
      ...
      moveTo: "移动到…",
      linkedRules: "链接规则…",
      addLinkedFolderToCollection: "将文件夹资产加入合集…",
      moveToTrash: "移入回收站",
      deleteFromDisk: "从硬盘中删除…",
  ...
  dialog: {
    ...
    openEagleLibrary: {
      title: "打开 Eagle 资源库",
      body: "将所选 Eagle 资源库转换为新的 Super 资源库。",
      name: "名称",
      help: "",
      submit: "选择保存位置"
    },
    openBillfishLibrary: {
      ...
      body: "将所选 Billfish 资源库转换为 Super 资源库。",
      name: "名称",
      help: "",
      submit: "选择保存位置"
    },
    ...
    restore: {
      ...
      nameConflict: "同名冲突",
      replaceHelp: "替换会删除冲突资产的 Super 记录及其托管文件，恢复资产会保留原有 ID 和元数据。",
      submit: "确认恢复"
    },
    undoMove: {
      title: "原位置已有新内容",
      ...
      replace: "替换原位置的新内容",
      skip: "跳过冲突资产",
      help: "Super 没有覆盖原位置。请选择明确的冲突处理方式后再撤销。",
      submit: "按所选策略撤销",
      conflictLabel: "冲突处理"
    },
    permanentDelete: {
    ...
    imageSequence: {
      ...
      fps: "帧率（FPS）",
      help: "可设置 1–240 FPS。序列只改变 Super 中的浏览与播放方式，不会修改源图片。",
      create: "创建序列图",
      creating: "正在创建…",
      update: "保存帧率",
      updating: "正在保存…"
    ...
    linkedFolderCollection: {
      title: "将「{name}」加入合集",
      cancelAria: "取消加入合集",
      help: "将当前可用、未被过滤的链接资产加入所选合集。",
      target: "目标合集",
      targetPlaceholder: "请选择合集",
      includeSubfolders: "包含子文件夹中的资产",
      snapshotNotice: "这是一次性成员快照：以后源文件夹新增、删除或移动文件时，合集不会自动同步；不会复制源文件。",
      submit: "加入合集"
    },
    collectionEditor: {
      title: "编辑合集详情",
    ...
    mediaJobs: {
      ...
      retryFailed: "重试失败项",
      linkedRemovalSection: "链接素材库移除任务",
      linkedRemovalSummary: "{name} · 已处理 {removed}/{total} 项 · {status}",
      pauseLinkedRemoval: "暂停",
      resumeLinkedRemoval: "恢复",
      linkedIndexSection: "链接素材库索引任务",
      linkedIndexSummary: "已索引 {indexed} 项 · 已扫描 {scanned} 个文件夹 · 待扫描 {pending} 个 · {status}",
      pauseLinkedIndex: "暂停",
      resumeLinkedIndex: "恢复",
      aiSection: "AI 分析任务",
      pauseAi: "暂停 AI",
      resumeAi: "继续 AI",
      cancelAi: "取消 AI",
    ...
    about: {
      ...
      close: "关闭",
      version: "版本 {version}",
      description: "免费、开放的本地素材库。让文件、标签、合集和预览保持井然有序。",
      openSourceNote: "遇到问题或希望参与改进？欢迎在 GitHub 提交反馈。",
      checkForUpdates: "检查更新",
      downloadAndInstall: "下载并安装更新",
      cancelDownload: "取消下载",
      updateNotChecked: "检查 Super 的新版本。",
    ...
    importLibraryChooser: {
      title: "导入资源库",
      help: "可以从资源库文件夹或导出的 ZIP 导入，也可以打开 Eagle 资源库并转换为新的 Super 资源库。",
    ...
    openLibraryChooser: {
      title: "打开资源库",
      help: "打开已有的 Super 资源库、从同步服务器拉取，或将 Eagle / Billfish 转换为新的 Super 资源库。",
      super: "打开 Super 资源库",
      external: "打开外部资源库"
    }
  },
  ...
  /** REQ-PREF-001 / Super-9es: 浏览区通用设置面板；主题/语言复用 shell.theme 与 shell.language 下的键。文件名/大小/日期共用一条 cardFieldsHint。 */
  plugin: {
  ...
  settings: {
    ...
    pluginThemeTrustedCssHint: "此插件可向 Super 宿主界面注入自定义 CSS。宿主 CSS 可能破坏布局、无障碍与升级兼容性。",
    pluginThemeTrustedCssTrustHint: "非受限模式下的宿主 CSS 可修改插件 iframe 之外的 Super 界面。仅信任你接受其样式变更的插件。",
    ...
    pluginHostSettingsHint: "由宿主根据插件清单渲染的设置项。值在重新打开资源库后仍保留；已激活插件可通过 Super.storage.get('settings.<id>') 读取镜像值。",
    ...
    backgroundUseDefault: "使用默认背景",
    backgroundRestoreDefault: "恢复默认背景",
    backgroundBuiltInImage: "内置默认背景 · 二次元主题",
    backgroundOpacity: "不透明度",
    appFontSizeSection: "应用字体大小",
    appFontSizeHint: "仅调整 Super Lib 内的文字大小；不影响系统缩放、浏览器缩放或其他程序。",
    appFontSizeLevel1: "1 较小",
    appFontSizeLevel2: "2 标准",
    appFontSizeLevel3: "3 较大",
    appFontSizeLevel4: "4 最大",
    ...
    browserExtensionStepDownload:
      "点击「前往下载」打开扩展发布页，按浏览器下载对应文件：Chrome/Edge 用 Super-extension-<版本>.zip，Firefox 用 Super-extension-firefox-<版本>-signed.xpi。",
    browserExtensionStepExtract:
      "Chrome / Edge：解压 zip 到固定文件夹（如「文档/Super-extension」），文件夹内需直接包含 manifest.json。",
    ...
    mcpIntroHint: "允许 MCP 客户端通过仅限本机回环地址的 Streamable HTTP 服务控制当前 Super 桌面实例。",
    mcpEnabled: "启用 MCP 服务",
    mcpEnabledHint: "服务默认关闭，永远不会监听局域网地址。",
    mcpAutoStart: "自动启动 MCP",
    mcpAutoStartHint: "Super 打开后自动启动本机回环服务。",
    ...
    mcpAgentHandoffHint: "点击“复制给 Agent”后，把剪贴板文本直接粘贴给 Agent 即可连接并操控 Super。",
    mcpRevoke: "撤销",
    mcpRevoked: "已撤销",
    ...
    sync: {
      ...
      webdavUrl: "WebDAV 地址",
      webdavUrlHint: "可为服务端的任意子路径，例如 https://nas.local/dav/share/Super/；省略 http:// 时会自动补全。",
      credentials: "凭据",
      ...
      serversTitle: "同步服务器",
      serversHint: "添加、测试并管理全局 WebDAV 服务器。服务器地址可包含路径（如 …/Share/Super），每个资源库会以库名为目录追加到其后。",
  ...
  preview: {
    ffmpegRequired: "当前安装缺少可用的媒体组件，无法生成视频播放代理。请重新安装或修复 Super 后重试。",
  ...
  toast: {
    bridgeUnavailable: "无法连接到 Super 桌面服务。请重新启动应用。",
    ...
    linkedFolderAddedToCollection: "已将 {count} 项链接资产加入合集（后续不会自动同步）。",
    linkedFolderAddToCollectionFailed: "无法将链接文件夹资产加入合集。",
    ...
    linkedFolderRemovalQueued: "已开始后台移除「{name}」的索引（共 {count} 项）；可在后台媒体任务中查看进度、暂停或恢复。源目录未改动。",
    ...
    sourceUrlOpenUnauthorized: "无法从此窗口打开源链接。请重新加载 Super 后再试。",
  ...
  error: {
    code: {
      ...
      WEB_MEDIA_DROP_TOO_LARGE: "网页拖放元数据过大，Super 已拒绝解析。",
      ...
      NOT_A_LIBRARY: "所选文件夹不是 Super 资源库（缺少资源库数据库）。",
      LIBRARY_CORRUPT: "无法读取该资源库的数据库（完整性检查或迁移记录失败）。请重新打开资源库，Super 会尝试从备份恢复。若需留底，请先把整个资源库文件夹复制到本机磁盘。",
      LIBRARY_ENGINE_UNAVAILABLE: "无法加载 SQLite 数据库引擎。这是应用安装问题，不是资源库文件损坏。请重装 Super 或重建 native 模块；不要恢复备份，也不要删除资源库文件夹。",
      LIBRARY_VERSION_TOO_NEW: "该资源库由更新版本的 Super 创建，当前应用无法打开。请安装最新版 Super 后再打开。",
      ...
      LIBRARY_MIGRATION_STUCK: "该资源库多次尝试迁移均失败。Super 已按上次可用的结构打开，你仍可继续使用。请等待更新后再升级；不要删除资源库文件夹。",
      LIBRARY_STRUCTURE_MISMATCH: "该资源库的数据库结构与当前 Super 版本不匹配。请升级到最新版 Super 后重新打开。",
      ...
      LIBRARY_BUSY: "该资源库正被另一个 Super 窗口占用，或数据库暂时锁定。请等待几秒后重试；不要在两台电脑上同时打开同一资源库。",
      LIBRARY_CLEANUP_FAILED: "创建资源库失败，且临时文件无法自动清理。请删除所选位置旁的 `.super-create-*.partial` 文件夹，然后换一个可写文件夹重试。",
      ...
      ASSET_MOVE_CONFLICT: "资产移动无法完成：源位置或目标位置已经变化，Super 未执行静默覆盖。",
    ...
    reason: {
      ...
      SOURCE_TRASH_FAILED: "操作系统拒绝将源文件移入系统回收站；源文件与 Super 记录均已保留。",
      ...
      NOT_A_LIBRARY: "所选目标不是有效的 Super 资源库。",
      ...
      AI_RATE_LIMIT: "请求过于频繁，Super 将稍后重试。",
      AI_NETWORK: "无法连接 AI 供应商，请检查网络。",
      AI_TIMEOUT: "AI 请求超时，Super 将稍后重试。",
      ...
      CONTACT_SHEET_REQUIRED: "视频分镜/封面尚未就绪，Super 将在就绪后自动重试。",
      ...
      LIBRARY_PARENT_INSIDE_SOURCE: "不能把新 Super 资源库保存在 Eagle/Billfish 源库内部。请选择源库以外的文件夹。",
