...
export const SUPER_PORTS = [19876, 19877, 19878] as const;

const SUPER_HOST = 'http://127.0.0.1';
...
async function requestSuper(
  path: string,
  init: RequestInit,
  fetchFn: FetchFunction,
): Promise<FetchResponse | null> {
  for (const port of SUPER_PORTS) {
    try {
      return await fetchFn(`${SUPER_HOST}:${port}${path}`, init);
    } catch {
      // Super may have selected the next fallback port.
    }
  }
  return null;
}

export async function probeSuperConnection(
  fetchFn: FetchFunction = fetch,
): Promise<ConnectionOutcome> {
  const ping = await requestSuper('/ping', { method: 'GET' }, fetchFn);
  if (!ping) return { kind: 'offline' };
  if (ping.status !== 200) return { kind: 'offline' };

  const folders = await requestSuper('/folders', { method: 'GET' }, fetchFn);
...
function parseFolderList(body: string): {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return { folders: [], recentBrowsedFolderIds: [], libraryDisplayName: 'Super' };
  }
...
export async function fetchSuperFolders(
  fetchFn: FetchFunction = fetch,
): Promise<FolderListOutcome> {
  const response = await requestSuper('/folders', { method: 'GET' }, fetchFn);
...
/** Align URL-derived filenames with the response Content-Type (Super-1jyi). */
...
/**
 * Fetch media in the browser with cookies + page referrer (Super-1jyi).
 * This is the anti-hotlink path; Super no longer re-downloads the URL.
 */
...
export async function deliverSaveUpload(
  ...
  const headers: Record<string, string> = {
    ...
    'X-Super-Kind': intent.kind,
    'X-Super-Content-Type': media.contentType,
    'X-Super-Filename': encodeURIComponent(media.filename),
    'X-Super-Focus-App-After-Save': String(behavior.focusAppAfterSave),
    'X-Super-Reveal-In-Library': String(behavior.revealInLibraryAfterSave),
  };
  if (intent.sourcePageUrl) {
    headers['X-Super-Source-Page-Url'] = encodeURIComponent(intent.sourcePageUrl);
  }
  if (intent.mediaUrl) {
    headers['X-Super-Media-Url'] = encodeURIComponent(intent.mediaUrl);
  }
  if (!intent.sourcePageUrl && !intent.mediaUrl) {
    headers['X-Super-Local-File'] = 'true';
  }
  if (intent.targetFolderId !== undefined) {
    headers['X-Super-Target-Folder-Id'] =
      intent.targetFolderId === null ? 'null' : encodeURIComponent(intent.targetFolderId);
  }

  const response = await requestSuper(
    '/save-upload',
    {
      method: 'POST',
      headers,
...
export function notificationForOutcome(outcome: SaveOutcome): UserNotification {
  switch (outcome.kind) {
    ...
    case 'unreachable':
      return {
        title: '无法连接 Super',
        message: '请先启动 Super 桌面应用并打开资源库，然后重新保存。',
      };
    case 'fetch_failed':
      return {
        title: '浏览器无法下载该媒体',
