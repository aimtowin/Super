...
export function folderMenuId(folderId: string): string {
  return `super-save-folder:${folderId}`;
}

/** 有子文件夹的文件夹子菜单第一行的「保存到此文件夹」项 id。 */
export function folderMenuSelfId(folderId: string): string {
  return `super-save-self:${folderId}`;
}

/** 「根目录」父菜单（子项「保存至此」= 保存到库根）。 */
export const MENU_ROOT_PARENT_ID = 'super-save-root-parent';

export function parseFolderMenuId(
  menuItemId: string | number,
): string | null | undefined {
  if (menuItemId === 'super-save-root') return null;
  if (typeof menuItemId !== 'string') return undefined;
  if (menuItemId.startsWith('super-save-folder-path:')) {
    // 纯容器节点（仅用于展开子级，自身不可保存）
    return undefined;
  }
  if (menuItemId.startsWith('super-save-self:')) {
    // 「保存到此文件夹」→ 保存到该文件夹自身
    return menuItemId.slice('super-save-self:'.length);
  }
  if (!menuItemId.startsWith('super-save-folder:')) return undefined;
  return menuItemId.slice('super-save-folder:'.length);
}
...
export function folderMenuPathId(path: string): string {
  return `super-save-folder-path:${path}`;
}
