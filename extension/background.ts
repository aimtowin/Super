...
const MENU_ROOT_ID = 'super-save';
const MENU_ROOT_FOLDER_ID = 'super-save-root';
const MENU_SEPARATOR_ID = 'super-save-sep';
const CONNECTION_ALARM = 'super-connection-check';
...
async function refreshFolderMenus(): Promise<void> {
  ...
  folderMenuRefreshPromise = (async () => {
    ...
    const outcome = await fetchSuperFolders();
    if (outcome.kind !== 'ok') {
      await removeDynamicFolderMenus();
      return;
    }
...
async function refreshConnectionState(): Promise<void> {
  const outcome = await probeSuperConnection();
 ...
 * `super-connection-status` miss sendResponse on large libraries
 * (message port closed → drag menu always showed 未连接 while the
 * toolbar icon was already bright). Super-a8mm.
 */
...
function showNotification(notification: UserNotification): void {
  void readExtensionSaveBehavior().then((behavior) => {
    ...
    chrome.notifications.create(
      `super-save-${Date.now()}-${notificationSequence}`,
      {
        type: 'basic',
...
async function saveIntentAndNotify(intent: SaveIntent): Promise<UserNotification> {
  ...
  if (outcome.kind === 'accepted') {
    const foldersOutcome = await fetchSuperFolders();
...
function notifySaveFeedback(
  ...
  chrome.tabs.sendMessage(
    tab.id,
    { type: 'super-save-feedback', ...payload },
    () => {
      void chrome.runtime.lastError;
    },
  );
...
async function handleContextMenuClick(
  info: SuperContextMenuClickData,
): Promise<void> {
  if (connectionState !== 'connected') {
    showNotification({
      title: '无法保存到 Super',
      message: '请先启动 Super 桌面应用并打开资源库。',
    });
  ...
  if (!intent) {
    if (info.tab?.id !== undefined && isFileUrl(info.srcUrl)) {
      ...
      chrome.tabs.sendMessage(
        info.tab.id,
        {
          type: 'super-local-context-save-request',
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (type === 'super-connection-status') {
  ...
  if (type === 'super-local-save-request') {
    void (async () => {
      if (connectionState !== 'connected') {
        const notification = {
          title: '无法保存到 Super',
