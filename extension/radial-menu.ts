...
/**
 * Super-c0ml / REQ-EXT-005 思维导图树状拖放保存菜单。
...
const STYLE_TEXT = `
  ...
  .panel {
    ...
    overflow: visible;
    animation: super-tree-in 140ms ease-out;
  }
  @keyframes super-tree-in {
...
function ensureShadowRoot(): ShadowRoot {
  ...
  host.setAttribute('data-super-radial-menu', 'true');
...
export async function startRadialSaveMenu(
  ...
  try {
    ...
    if (!connected) {
      if (!dragEndedEarly) {
        showSaveBubble(clientX, clientY, '未连接 Super', '请先启动应用并打开资源库');
      }
      return;
    }
