...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (type === 'super-local-context-save-request') {
    void (async () => {
      ...
      chrome.runtime.sendMessage({
        ...
        if (chrome.runtime.lastError) {
          showSaveToast('无法保存到 Super', '本地图片上传失败。');
        } else {
  ...
  if (type !== 'super-save-feedback') return;
  if (Reflect.get(message, 'state') === 'saving') {
    showSaveToast('正在保存到 Super', '发送中…');
  } else {
...
// 避免浮层菜单替换整站原生菜单（Super-ak94 / 用户反馈 2026-07-26）。

// Super-c0ml / REQ-EXT-005：拖拽图片/视频时展开树状保存菜单（全站点生效）。
