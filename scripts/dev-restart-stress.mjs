 ...
 *
 * Pass per round:
 *   - super.log contains main.window.mount-verified
 *   - no main.window.mount-failed / ERR_CONNECTION_REFUSED
 *   - after SIGINT, Super Electron exits (no orphan)
 */
...
const port = Number(process.env.SUPER_VITE_PORT || 5173);
const electronLog = path.join(homedir(), "Library/Logs/Electron/super.log");
const electronApp =
  process.platform === "win32"
    ? "node_modules\\electron\\dist\\electron.exe"
    : "node_modules/electron/dist/Electron.app/Contents/MacOS/Electron";

function superElectronRunning() {
...
function startNpmStart() {
  return spawn("npm", ["start"], {
    cwd: projectRoot,
    env: { ...process.env, SUPER_VITE_PORT: String(port) },
    stdio: "ignore",
  });
}
...
async function runRound() {
  ...
  try {
    ...
    const electronAlive = superElectronRunning();
    return {
