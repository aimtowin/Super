...
let headless = false;
let libraryPath = process.env.SUPER_MCP_LIBRARY_PATH ?? '';
let writeAccess = process.env.SUPER_MCP_WRITE_ACCESS === '1';
let userData = process.env.SUPER_MCP_USER_DATA_PATH ?? '';
let allowUnbound = process.env.SUPER_MCP_ALLOW_UNBOUND === '1';
...
if (libraryPath && !path.isAbsolute(libraryPath)) {
  process.stderr.write('SUPER_MCP_LIBRARY_PATH / --library must be an absolute path.\n');
  usage();
}
...
if (headless) {
  env.SUPER_MCP = '1';
  env.SUPER_MCP_WRITE_ACCESS = writeAccess ? '1' : '0';
  if (libraryPath) env.SUPER_MCP_LIBRARY_PATH = libraryPath;
  if (allowUnbound) env.SUPER_MCP_ALLOW_UNBOUND = '1';
  if (userData) env.SUPER_MCP_USER_DATA_PATH = path.resolve(userData);
} else {
  commandArgs.push(
    process.execPath,
...
process.stderr.write(
  headless
    ? '[super-mcp] launching Electron headless MCP host…\n'
    : '[super-mcp] attaching to Super Desktop…\n',
);
