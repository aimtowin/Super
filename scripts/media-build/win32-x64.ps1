...
# checkout on Windows; `SUPER_MEDIA_BUILD_DIR` remains available for CI.
# 注意：不得使用 `%LOCALAPPDATA%\Super` 下的子目录——Squirrel 安装目录
# 恰为 `%LOCALAPPDATA%\<包名>`（super，不区分大小写），撞名会让 Squirrel
# 全量安装时尝试删除 vcpkg 构建树而失败（PathTooLongException）。
$DefaultWork = Join-Path $env:LOCALAPPDATA 'SuperMediaBuild\win32-x64'
$Work = if ($env:SUPER_MEDIA_BUILD_DIR) { $env:SUPER_MEDIA_BUILD_DIR } else { $DefaultWork }
...
$Triplet = 'super-x64-windows-static'
...
$Archive = Join-Path $ArtifactRoot 'super-media-win32-x64.zip'
$ManifestChecksum = Join-Path $ArtifactRoot 'super-media-win32-x64.manifest.sha256'
