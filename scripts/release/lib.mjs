...
export function packageDirectoryName() {
  return `Super-${process.platform}-${process.arch}`;
}
...
export function resolvePackagedExecutable(packageRoot = defaultPackageRoot()) {
  if (process.platform === 'darwin') {
    const executable = path.join(packageRoot, 'Super.app', 'Contents', 'MacOS', 'Super');
  ...
  if (process.platform === 'win32') {
    const executable = path.join(packageRoot, 'Super.exe');
    if (!existsSync(executable)) {
      throw new Error(`Packaged executable not found: ${executable}`);
    }
    return executable;
...
export function writeChecksumManifest(files, outputPath, version = currentVersion()) {
  ...
  writeFileSync(
    outputPath,
    `# Super v${version} (${currentPlatformKey()})\n${lines.join('\n')}\n`,
    'utf8',
  );
