#!/usr/bin/env node
/**
 * 上传媒体 bundle 到 Super-Build Release（单一 Release + 标准 asset 名）。
 *
 * 策略（2026-08-08 产品确定）：
 *   - 只保留一个 Release（tag media-v0.1.1，标题 SuperBuildDependencies）
 *   - asset 用标准名（super-media-<platform>.zip），更新时删除旧资产再上传
 *
 * 用法：
 *   node scripts/release/publish-media-bundle.mjs \
 *     --platform win32-x64 --version v0.1.1 \
 *     --zip artifacts/media-binaries/super-media-win32-x64.zip
 *
...
const BUILD_REPO = 'dolag233/Super-Build';
...
function main() {
  ...
  const base = `super-media-${platform}`;
  const assetFiles = [
    [zipPath, `${base}.zip`],
