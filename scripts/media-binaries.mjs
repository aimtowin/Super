...
async function acquire({ platform, root, lockPath }) {
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'super-media-'));
  try {
...
async function ensure({ platform, root, lockPath }) {
  try {
    ...
    if (process.env.SUPER_MEDIA_AUTO_ACQUIRE === '0') {
      throw error;
    }
    console.warn(
      `[media-binaries] Local ${platform} bundle is missing or invalid; ` +
      'acquiring the pinned Super-Build release before packaging.\n' +
...
async function main() {
  ...
  if (command === 'verify') {
    ...
    if (process.env.SUPER_MEDIA_SKIP_PROVENANCE === '1') {
      console.warn(
        'Skipping release provenance check (SUPER_MEDIA_SKIP_PROVENANCE=1). ' +
