 ...
 * `SUPER_E2E_ISOLATED=1`, so `src/main/index.ts` places the main window on
 ...
 * to Super-a1b rather than solved here.
 *
...
if (useXvfbRun) {
  ...
  console.log(
    '[run-e2e-isolated] macOS: the E2E window will use a secondary display when one is connected; ' +
      'on a single-display Mac it falls back to the primary display (see super.log for the fallback notice).',
  );
}
...
const child = spawn(command, args, {
  cwd: projectRoot,
  stdio: 'inherit',
  env: { ...process.env, SUPER_E2E_ISOLATED: '1' },
});
