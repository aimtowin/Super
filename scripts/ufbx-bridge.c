 ...
 *   int         super_parse(const uint8_t *fbx, size_t fbx_size, const char *opts_json)
 *   const char *super_error(void)        // JSON error descriptor after failure
 *   uint8_t    *super_out_ptr(void)
 *   size_t      super_out_size(void)
 *   void        super_free_out(void)
 *
...
#define SUPER_MAX_OUTPUT_BYTES (1u << 30) /* 1 GiB hard cap, defense in depth */
#define SUPER_ERROR_BUF_SIZE 4096
#define SUPER_MAX_PARTS 1024
#define SUPER_MAX_TEXTURES 256
...
typedef struct {
	super_buf json;   /* descriptor text (working buffer) */
	super_buf bin;    /* binary blobs (working buffer) */
	super_buf packed; /* final contiguous [u32 len][JSON][blobs] */
} super_result;
...
static super_result g_result;
static char g_error[SUPER_ERROR_BUF_SIZE];
static int g_error_used;
static int g_texture_emitted[SUPER_MAX_TEXTURES];
static const ufbx_texture *g_missing_emitted[64];

/* Forward declarations (defined at the bottom; super_parse uses them). */
int super_parse(const uint8_t *fbx_data, size_t fbx_size, const char *opts_json);
const char *super_error(void);
uint8_t *super_out_ptr(void);
size_t super_out_size(void);
void super_free_out(void);
...
static void buf_reserve(super_buf *buf, size_t extra)
{
	if (buf->size + extra <= buf->cap) return;
	size_t cap = buf->cap ? buf->cap : 4096;
	while (cap < buf->size + extra) cap *= 2;
	if (cap > SUPER_MAX_OUTPUT_BYTES) cap = SUPER_MAX_OUTPUT_BYTES;
...
static void buf_append(super_buf *buf, const void *data, size_t size)
{
...
static void jappend(super_buf *buf, const char *text)
{
	buf_append(buf, text, strlen(text));
}

static void jappend_int(super_buf *buf, long long value)
{
	char tmp[32];
...
static void jappend_double(super_buf *buf, double value)
{
...
static void jappend_string(super_buf *buf, const char *data, size_t len)
{
...
static void jappend_str_c(super_buf *buf, const char *text)
{
...
static void blob_append(super_buf *bin, const void *data, size_t size)
{
...
static void parse_opts(const char *json, super_opts *opts)
{
	memset(opts, 0, sizeof(*opts));
	opts->max_output_bytes = SUPER_MAX_OUTPUT_BYTES;
	if (!json) return;
...
static void emit_material(super_buf *j, const ufbx_material *mat, const ufbx_scene *scene)
{
	...
	/* Super-a5ic: separate-file metalness/roughness maps cannot ride the
...
static void emit_texture(super_buf *j, super_buf *bin, const ufbx_texture *tex, int index, int *missing_count)
{
	jappend(j, "{\"index\":");
	jappend_int(j, index);
	jappend(j, ",\"name\":");
...
int super_parse(const uint8_t *fbx_data, size_t fbx_size, const char *opts_json)
{
	...
	super_buf *j = &g_result.json;
	super_buf *bin = &g_result.bin;
	...
	for (size_t i = 0; i < scene->meshes.count; i++) {
		...
		size_t part_off[SUPER_MAX_PARTS];
		size_t part_tri[SUPER_MAX_PARTS];
		size_t num_parts = mesh->material_parts.data ? mesh->material_parts.count : 0;
		if (num_parts > SUPER_MAX_PARTS) num_parts = SUPER_MAX_PARTS;
		...
		if (has_uv) {
			for (size_t c = 0; c < corner_count; c++) {
				...
				/* Super-a5ic: FBX UV origin is bottom-left, glTF is
		...
		for (size_t p = 0; p < num_parts; p++) {
			...
			size_t emit = 0;
			/* Super-a5ic: use ufbx's real triangulator instead of a hand
	...
	for (size_t i = 0; i < scene->materials.count; i++) {
		...
		for (size_t t = 0; t < mat->textures.count; t++) {
			...
			if (already) continue;
			if (tex_ix >= SUPER_MAX_TEXTURES) break;
