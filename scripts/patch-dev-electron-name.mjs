#!/usr/bin/env node
// Super-tluf/q0b1: in dev mode macOS shows "Electron" in the Dock and the
...
// re-register with LaunchServices so dev shows "Super" everywhere.
...
if (currentDisplay === 'Super') {
  process.exit(0);
}

const plistArgs = [
  '-c', 'Delete :CFBundleDisplayName',
  '-c', `Add :CFBundleDisplayName string Super`,
  '-c', 'Delete :CFBundleName',
  '-c', `Add :CFBundleName string Super`,
  plistPath,
];
...
console.log(`[patch-dev-electron-name] Dev Electron.app renamed to Super (menu bar immediate; Dock restarted to pick up the name${ls.status !== 0 ? '; lsregister exit ' + ls.status : ''}).`);
